...
void Mesh::initNormals(int useArea) {
    if (!hasVertNormals) {
        if (tstrips.size())
...
vector<int> &Mesh::getTstrips(void) {
    if (!tstrips.size()) {
#if VERBOSE
...
void Mesh::copyTriFrom(Mesh *meshSrc, int src, int dst) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (3 * dst + i < this->tris.size()) {
            ...
            if (meshSrc->hasVoxels)
                this->fromVoxels[3 * dst + i] =
                    meshSrc->fromVoxels[3 * src + i];
...
void Mesh::copyVertFrom(Mesh *meshSrc, int src, int dst) {
...
void Mesh::mark_boundary_verts(void) {
    if (bdry.size())
        return;
...
Mesh *Mesh::Decimate(int numFaces, int optLevel, float errLevel,
                     float boundWeight, ResolutionCtrl::Decimator dec) {
    Mesh *outMesh = new Mesh();

    switch (dec) {
    case ResolutionCtrl::decQslim:
        quadric_simplify(vtx, getTris(), outMesh->vtx, outMesh->tris, numFaces,
                         optLevel, errLevel, boundWeight);
        break;

    case ResolutionCtrl::decPlycrunch:
        plycrunch_simplify(vtx, getTris(), outMesh->vtx, outMesh->tris,
                           numFaces);
        break;
...
static PlyProperty face_props[] = { { "vertex_indices", PLY_INT, PLY_INT, 0, 1,
                                      PLY_UCHAR, PLY_UCHAR, 0 }, };

static PlyProperty face_prop_from_voxel_x = { "from_voxel_x", PLY_FLOAT,
                                              PLY_FLOAT,      0,
                                              0,              PLY_START_TYPE,
                                              PLY_START_TYPE, 0 };
static PlyProperty face_prop_from_voxel_y = { "from_voxel_y", PLY_FLOAT,
                                              PLY_FLOAT,      0,
                                              0,              PLY_START_TYPE,
                                              PLY_START_TYPE, 0 };
static PlyProperty face_prop_from_voxel_z = { "from_voxel_z", PLY_FLOAT,
                                              PLY_FLOAT,      0,
                                              0,              PLY_START_TYPE,
                                              PLY_START_TYPE, 0 };
...
int Mesh::readPlyFile(const char *filename) {
    int i, j;
    int nelems;
    char **elist;
    char **obj_info;
    ...
    for (i = 0; i < nelems; i++) {
        ...
        if (equal_strings("vertex", elem_name)) {
            ...
            if (ply.is_valid_property("vertex", vert_prop_confidence.name)) {
            ...
            for (j = 0; j < num_elems; j++) {
                ...
                if (hasVertNormals)
                    pushNormalAsShorts(
                        nrm, Pnt3(plyVert.nx, plyVert.ny, plyVert.nz));
        ...
        if (equal_strings("face", elem_name)) {
            ...
            if (ply.is_valid_property("face", "from_voxel_x") ||
                ...
                printf("No voxel information stored...\n");
            ...
            for (j = 0; j < num_elems; j++) {
                ...
                addTri(plyFace.verts[0], plyFace.verts[1], plyFace.verts[2]);
                if (hasVoxels) {
                    ...
                    addVoxelInfo(plyFace.verts[0], plyFace.verts[1],
                                 plyFace.verts[2], plyFace.fromVoxelX,
                                 plyFace.fromVoxelY, plyFace.fromVoxelZ);
                }
            ...
            for (int iv = 0; iv < info.nverts; iv++) {
                ...
                if (ok) {
                    ...
                    cerr << "\n\nRed alert: invalid index " << vert
                         << " in tstrip (index = " << iv << ", valid range=0.."
                         << vtx.size() - 1 << ")\n" << endl;
                    // remove last strip
                    if (lastend) {
                        while (--lastend)
...
int Mesh::writePlyFile(const char *filename, int useColorNotTexture,
                       int writeNormals) {
#if 1
...
void Mesh::subsample_points(float rate, vector<Pnt3> &pts) {
    subsample_points(int(rate * vtx.size()), pts);
}

void Mesh::subsample_points(float rate, vector<Pnt3> &pts, vector<Pnt3> &nrms) {
    subsample_points(int(rate * vtx.size()), pts, nrms);
}

bool Mesh::subsample_points(int n, vector<Pnt3> &pts) {
    pts.clear();
    pts.reserve(n);
    double nv = vtx.size();
    if (n > nv)
...
bool Mesh::subsample_points(int n, vector<Pnt3> &pts, vector<Pnt3> &nrms) {
    pts.clear();
    pts.reserve(n);
    nrms.clear();
    nrms.reserve(n);
...
void Mesh::remove_unused_vtxs(void) {
    ...
    if (nrm.size())
        nrm.erase(nrm.begin() + 3 * cnt, nrm.end());
...
void Mesh::remove_stepedges(int percentile, int factor) {
...
void Mesh::simulateFaceNormals(vector<short> &facen) {
    facen.clear();
...
void Mesh::Warp() {
    ...
    for (vector<Pnt3>::iterator i = vtx.begin(); i != vtx.end(); i++) {
        (*i)[2] +=
            7 * cos(0.017674205 * (*i)[1]) + 5 * cos(0.030921161 * (*i)[0]);
    }
}
...
void Mesh::dequantizationSmoothing(double maxDisplacement) {
    ...
    for (int i = 0; i < vtx.size(); i++) {
        if (i % mc == 0)
            printf("Vertex Number %d tri count %ld\n ", i, vtxTris[i].size());
...
***************************************************************************/
void Mesh::saveOrigVerts() { orig_vtx = vtx; }
