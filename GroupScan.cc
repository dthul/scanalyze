...
bool GroupScan::AddScan(DisplayableMesh *scan) {
...
bool GroupScan::RemoveScan(DisplayableMesh *scan) {
    FOR_EACH_CHILD(it) {
        if (*it == scan) {
            children.erase(it);
            rebuildResolutions();
...
bool GroupScan::write(const string &fname) {
    if (fname.empty()) {
...
MeshTransport *GroupScan::mesh(bool perVertex, bool stripped, ColorSource color,
                               int colorSize) {
...
void GroupScan::computeBBox(void) {
    bbox.clear();
    rot_ctr = Pnt3();
    FOR_EACH_CHILD(it) {
        RigidScan *rs = (*it)->getMeshData();
...
bool GroupScan::is_modified(void) { return bDirty; }

bool GroupScan::get_children(vector<RigidScan *> &_children) const {
    _children.reserve(children.size());
    FOR_EACH_CONST_CHILD(it) { _children.push_back((*it)->getMeshData()); }

    return true;
}

bool GroupScan::get_children_for_display(
    vector<DisplayableMesh *> &_children) const {
    _children = children;
    return true;
}

void GroupScan::rebuildResolutions(void) {
    ...
    for (int i = 0; i < nRes; i++) {
        ...
        FOR_EACH_CHILD(it) {
            ...
            if (i == 0) // only do this once for each child
                iPolysVis +=
                    childres[rs->current_resolution_index()].abs_resolution;
        }
...
bool GroupScan::switchToResLevel(int iRes) {
...
bool GroupScan::load_resolution(int iRes) {
...
bool GroupScan::release_resolution(int nPolys) {
...
bool GroupScan::filter_inplace(const VertexFilter &filter) { return false; }

RigidScan *GroupScan::filtered_copy(const VertexFilter &filter) {
    BailDetector bail;
    ...
    FOR_EACH_CHILD(it) {
        ...
        if (newchild) {
            if (newchild->num_resolutions() > 0) {
                string newname;
                newname = oldchild->get_basename() + "Clip." +
                          oldchild->get_nameending();
        ...
        cout << "." << flush;
        if (bail()) {
            cerr << "Warning: group clip interrupted; results are partial"
                 << endl;
            break;
        }
    }
...
void GroupScan::subsample_points(float rate, vector<Pnt3> &p, vector<Pnt3> &n) {
    vector<Pnt3> cp, cn;
    p.clear();
    n.clear();
...
bool GroupScan::closest_point(const Pnt3 &p, const Pnt3 &n, Pnt3 &cl_pnt,
                              Pnt3 &cl_nrm, float thr, bool bdry_ok) {
    Pnt3 cp, cn;
    Pnt3 mp, mn;
    float closest = 1e33;
    bool bAny = false;
...
bool GroupScan::write_metadata(MetaData data) {
    bool success = true;
