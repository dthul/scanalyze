...
static RigidScan *GetMeshFromCmd(Tcl_Interp *interp, int argc, char *argv[],
                                 int minArgC = 2,
                                 DisplayableMesh **pOutDisp = NULL,
                                 int iMeshName = 1) {
    char buf[1000];
...
int PlvMeshResolutionCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                         char *argv[]) {
    DisplayableMesh *dm;
...
int PlvMeshDecimateCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                       char *argv[]) {
...
int PlvMeshResListCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                      char *argv[]) {
    ...
    for (int i = 0; i < resList.size(); i++) {
        if (bShort)
            ...
            sprintf(
                tempstr, "(%c%c):%d ", resList[i].desired_in_mem ? 'A' : '-',
                resList[i].in_memory ? 'L' : '-', resList[i].abs_resolution);
...
int PlvSetMeshResPreloadCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                            char *argv[]) {
...
int PlvCurrentResCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                     char *argv[]) {
    char info[PATH_MAX];

    RigidScan *meshSet = GetMeshFromCmd(interp, argc, argv, 2);
    if (!meshSet) {
        Tcl_SetResult(interp, "Unable to find mesh in PlvCurrentResCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }

    if (!meshSet->num_resolutions()) {
        Tcl_SetResult(interp, "Given mesh has no data in PlvCurrentResCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
int PlvMeshInfoCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                   char *argv[]) {
    RigidScan *meshSet = GetMeshFromCmd(interp, argc, argv, 2);
    if (meshSet == NULL)
        return TCL_ERROR;
    ...
    if (argc > 2 && 0 == strcmp(argv[2], "graphical"))
        Tcl_SetResult(interp, (char *)meshSet->getInfo().c_str(), TCL_VOLATILE);
    else
...
int PlvMeshDeleteResCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                        char *argv[]) {
    ...
    if (scan->num_resolutions() == 1) {
        Tcl_SetResult(interp, "Scan has only 1 resolution; can't be deleted",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
int PlvMeshUnloadResCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                        char *argv[]) {
    DisplayableMesh *dm;
    RigidScan *scan = GetMeshFromCmd(interp, argc, argv, 3, &dm);
    if (scan == NULL) {
    ...
    if (!scan->release_resolution(resLevel)) {
        cerr << "scan " << dm->getName() << " failed to release resolution"
             << endl;
        return TCL_ERROR;
    }
...
int PlvMeshSetDeleteCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                        char *argv[]) {
...
static void DeleteHelper(DisplayableMesh *mesh) {
    ...
    if (group) {
        ...
        for (vector<DisplayableMesh *>::iterator it = children.begin();
             it < children.end(); it++) {
            DeleteHelper(*it);
        }
...
int PlvTransMeshCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                    char *argv[]) {
    if (argc != 5) {
        printf("%s: mesh x y z\n", argv[0]);
        return TCL_OK;
...
int PlvMeshRemoveStepCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                         char *argv[]) {
...
int PlvScanColorCodeCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                        char *argv[]) {
    DisplayableMesh *dispMesh;
    ...
    if (!strcmp(argv[1], "set")) {
        ...
        sprintf(color, "#%02x%02x%02x", (unsigned)diffuse[0],
                (unsigned)diffuse[1], (unsigned)diffuse[2]);
...
int PlvFlipMeshNormalsCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                          char *argv[]) {
...
int PlvBlendMeshCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                    char *argv[]) {
    DisplayableMesh *mesh;
...
int PlvGroupScansCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                     char *argv[]) {
    if (argc < 3) {
    ...
    if (!strcmp(argv[1], "create")) {
        ...
        for (int i = 3; i < argc - 1; i++) {
            ...
            if (!dm) {
                ...
                Tcl_SetResult(interp, "Missing scan in PlvGroupScansCmd",
                              TCL_STATIC);
                return TCL_ERROR;
            }
        ...
        if (!group) {
            Tcl_SetResult(interp, "The group could not be created.",
                          TCL_STATIC);
            return TCL_ERROR;
        }
        ...
        Tcl_SetResult(interp, "Unrecognized option to PlvGroupScansCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
int PlvHiliteScanCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                     char *argv[]) {
    // unhilite previous selection in meshcontrols window
    for (int i = 0; i < g_hilitedScans.size(); i++)
        Tcl_VarEval(interp, "hiliteScanEntry ", g_hilitedScans[i]->getName(),
                    " 0", NULL);
...
int SczXformScanCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                    char *argv[]) {
    ...
    if (!strcmp(argv[2], "reload")) {
        if (!scan->readXform(scan->get_basename())) {
            cerr << "Scan " << scan->get_name() << " failed to read xform."
                 << endl;
...
int SczGetScanXformCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                       char *argv[]) {
    RigidScan *scan = GetMeshFromCmd(interp, argc, argv, 2);
    if (!scan)
        return TCL_ERROR;
...
int PlvSmoothMesh(ClientData clientData, Tcl_Interp *interp, int argc,
                  char *argv[]) {
    if (argc < 4) {
        Tcl_SetResult(interp,
                      "plvSmoothMesh <meshName> <iterations> <maxMotion_mm>",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
//
int PlvRunExternalProgram(ClientData clientData, Tcl_Interp *interp, int argc,
                          char *argv[]) {
    ...
    if (!strlen(command) || !strlen(inputMesh) || !strlen(outputMesh)) {
        Tcl_SetResult(interp,
                      "Please enter a command, inputMesh or outputMesh name",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (writeCBox) {
        ...
        if (theSel.pts.size() < 3) {
            Tcl_SetResult(
                interp, "At least 3 points are needed for clipping information",
                TCL_STATIC);
            return TCL_ERROR;
        }
        ...
        for (int i = 0; i < theSel.pts.size(); i++) {
            ...
            gluUnProject(theSel.pts[i].x, theSel.pts[i].y, 0, mvmatrix,
                         projmatrix, viewport, &objx, &objy, &objz);
            gluUnProject(theSel.pts[i].x, theSel.pts[i].y, 1, mvmatrix,
                         projmatrix, viewport, &objFarX, &objFarY, &objFarZ);
        ...
        for (int i = 0; i < theSel.pts.size(); i++) {
            ...
            Pnt3 crs;
            Pnt3 up(obj[j][0] - obj[i][0], obj[j][1] - obj[i][1],
                    obj[j][2] - obj[i][2]);
            ...
            // make the point list and concat it with the current clip region
            // string
            sprintf(tempVertBuf, "%f %f %f %f %f %f ", (double)obj[i][0],
                    (double)obj[i][1], (double)obj[i][2], (double)crs[0],
                    (double)crs[1], (double)crs[2]);
    ...
    // if we're clipping the object against some box, put the object in it's
    ...
    char cmdBuffer[512];
    if (writeCBox)
        sprintf(cmdBuffer, "%s %s %s %s", command, inTName.c_str(),
                outTName.c_str(), clipBoxCoord);
    else
        sprintf(cmdBuffer, "%s %s %s ", command, inTName.c_str(),
                outTName.c_str());
    ...
    if (newScan != NULL) {
        ...
        Tcl_SetResult(interp, "A filtered scan could not be created",
                      TCL_STATIC);
