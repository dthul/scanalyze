...
void CyberWorkingVolume::use(RigidScan *rscan) {
    ...
    bounds(wvVert, wvHorInc, wvHorMin, wvHorMax, wvRotMin, wvRotMax, wvMotMin,
           wvMotMax);
}
...
void CyberWorkingVolume::bounds(float vert, float hor_inc, float hor_min,
                                float hor_max, float rot_min, float rot_max,
                                float mot_min, float mot_max) {
    CyberXform xf;
    float h, r, m;
    int mc, rc;
    float vertDelta;
    ...
    for (h = wvHorMin;
         h <= wvHorMin + wvHorInc * ceilf((wvHorMax - wvHorMin) / wvHorInc);
         h += wvHorInc)
        ++wvNumShells;
...
void CyberWorkingVolume::drawthis(void) {
    int i, j, k;
