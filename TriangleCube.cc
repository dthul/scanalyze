...
static long face_plane(const Pnt3 &p) {
    long outcode = 0;
    if (p[0] > .5)
        outcode |= 0x01;
    if (p[0] < -.5)
...
static long bevel_2d(const Pnt3 &p) {
    long outcode = 0;
    if (p[0] + p[1] > 1.0)
        outcode |= 0x001;
    if (p[0] - p[1] > 1.0)
...
static long bevel_3d(const Pnt3 &p) {
    long outcode = 0;
    if ((p[0] + p[1] + p[2]) > 1.5)
        outcode |= 0x01;
    if ((p[0] + p[1] - p[2]) > 1.5)
...
static bool point_on_face(const Pnt3 &p1, const Pnt3 &p2, float alpha,
                          long mask) {
    Pnt3 plane_point;
...
static bool line_on_face(const Pnt3 &p1, const Pnt3 &p2, long outcode_diff) {

    if (0x01 & outcode_diff)
        if (point_on_face(p1, p2, (.5 - p1[0]) / (p2[0] - p1[0]), 0x3e))
            return true;
    if (0x02 & outcode_diff)
        if (point_on_face(p1, p2, (-.5 - p1[0]) / (p2[0] - p1[0]), 0x3d))
            return true;
    if (0x04 & outcode_diff)
        if (point_on_face(p1, p2, (.5 - p1[1]) / (p2[1] - p1[1]), 0x3b))
            return true;
    if (0x08 & outcode_diff)
        if (point_on_face(p1, p2, (-.5 - p1[1]) / (p2[1] - p1[1]), 0x37))
            return true;
    if (0x10 & outcode_diff)
        if (point_on_face(p1, p2, (.5 - p1[2]) / (p2[2] - p1[2]), 0x2f))
            return true;
    if (0x20 & outcode_diff)
        if (point_on_face(p1, p2, (-.5 - p1[2]) / (p2[2] - p1[2]), 0x1f))
            return true;
    return false;
}
...
static bool point_triangle_intersection(const Pnt3 &p, const Pnt3 &v1,
                                        const Pnt3 &v2, const Pnt3 &v3) {
    /* First, a quick bounding-box test:                               */
    /* If P is outside triangle bbox, there cannot be an intersection. */

    if (p[0] > MAX3(v1[0], v2[0], v3[0]))
        return true;
    if (p[1] > MAX3(v1[1], v2[1], v3[1]))
        return true;
    if (p[2] > MAX3(v1[2], v2[2], v3[2]))
        return true;
    if (p[0] < MIN3(v1[0], v2[0], v3[0]))
        return true;
    if (p[1] < MIN3(v1[1], v2[1], v3[1]))
        return true;
    if (p[2] < MIN3(v1[2], v2[2], v3[2]))
        return true;
    ...
    long sign12 =
        SIGN3(cross(v2, p, v1)); // Extract X,Y,Z signs as 0...7 integer
...
//
bool tri_outside_of_cube(const Pnt3 &c, float s, const Pnt3 &t1, const Pnt3 &t2,
                         const Pnt3 &t3) {
    ...
    if ((v1_test & v2_test) == 0)
        if (line_on_face(v1, v2, v1_test | v2_test))
            return false;
    if ((v1_test & v3_test) == 0)
        if (line_on_face(v1, v3, v1_test | v3_test))
            return false;
    if ((v2_test & v3_test) == 0)
        if (line_on_face(v2, v3, v2_test | v3_test))
            return false;
