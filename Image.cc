...
void bwtorgb(unsigned char *b, unsigned char *l, int n) {
    while (n--) {
        l[0] = *b;
        l[1] = *b;
        l[2] = *b;
...
void rgbtorgba(unsigned char *r, unsigned char *g, unsigned char *b,
...
void rgbtorgb(unsigned char *r, unsigned char *g, unsigned char *b,
              unsigned char *l, int n) {
    while (n--) {
        l[0] = r[0];
        l[1] = g[0];
...
unsigned *read_textureRGB(char *name, int *width, int *height) {
    unsigned *base, *lptr;
    unsigned char *rbuf, *gbuf, *bbuf, *abuf;
    ImageRec *image;
    int y;
...
void Image::create(const int w, const int h, const int d) {
...
Image::Image() : xsize(0), ysize(0), data(NULL), _data(NULL), dim(0) {}
...
void Image::toRGBA(void) {
    ...
    for (int i = 0; i < ysize; i++)
        data[i] = &_data[xsize * dim * i];
}

void Image::toRGB(void) {
    assert(data && _data);
        ...
        for (int u = 0; u < xsize; u++) {
            ...
            if (s == 0)
                ...
                if (tmp[0] == max)
                    ...
                    h = 2 + (tmp[2] - tmp[0]) / delta;
                else if (tmp[2] == max)
                    h = 4 + (tmp[0] - tmp[1]) / delta;
                if (h < 0.0)
                    h += 6.0;
                h /= 6.0;
            }
...
void Image::distances(int iter) {
    for (int i = 0; i < iter; i++) {
        int vi = i % 2;
        for (int v = vi ? 0 : ysize - 1; vi ? (v < ysize) : (v >= 0);
             vi ? v++ : v--) {
            int ui = (i % 4) >> 1;
            for (int u = ui ? 0 : xsize - 1; ui ? (u < xsize) : (u >= 0);
                 ui ? u++ : u--) {
                if (getR(u, v) == 0)
                    continue;
                int min = 255;
...
int Image::read(const char *fname, Format format, int w, int h) {
    int i;
    if (w == 0) {
        w = xsize;
        h = ysize;
...
int Image::write(const char *fname, Format format) {
    switch (format) {
    case WSU: {
...
Image Image::shrink(int factor) const {
    int sx = xsize / factor;
    int sy = ysize / factor;
...
void Image::shrink_by_half(Image &simg) const {
    int sx = xsize >> 1;
    int sy = ysize >> 1;
...
// [x' y' w'] = [u v w] M
Image Image::warp(float m[3][3]) const {
...
Image Image::crop(int x, int y, int dx, int dy) {
    Image im(dx, dy, dim);
    for (int v = 0; v < dy; v++) {
...
Image &Image::fill(uchar a, uchar b, uchar c, uchar d) {
