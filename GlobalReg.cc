...
inline float swap_float(float x) {
    ...
    y = (y << 24) | ((y << 8) & 0x00ff0000) | ((y >> 8) & 0x0000ff00) |
        (y >> 24);
    return (*((float *)&y));
}

inline short FixShort(short x) { return (swap_short(x)); }

inline int FixInt(int x) { return (swap_int(x)); }

inline float FixFloat(float x) { return (swap_float(x)); }

// otherwise set up file loading routines for a big endian machine
#else

inline short FixShort(short x) { return (x); }

inline int FixInt(int x) { return (x); }

inline float FixFloat(float x) { return (x); }
...
/* There is no ReadChar since no special byte ordering is required for
 * characters */
...
inline int scan_idx(const TbObj *a, const vector<TbObj *> &scan) {
...
inline float dist2(const vector<Pnt3> &P, const vector<Pnt3> &Q) {
    float d = 0.0;
...
static RigidScan *createProxyScanNamed(const string &name) {
    if (strcmp(Tcl_GetVar(g_tclInterp, "allowProxiesWithGR", TCL_GLOBAL_ONLY),
               "0") == 0) {
        return NULL;
    }
    ...
    Tcl_VarEval(g_tclInterp, "addMeshToWindow ", dm->getName(), " 0", NULL);
    Tcl_VarEval(g_tclInterp, "changeVis ", dm->getName(), " 0", NULL);

    return rs;
}

static void calcPairwiseRmsErr(const vector<Pnt3> &ptsa,
                               const vector<Pnt3> &nrma,
                               const vector<Pnt3> &ptsb,
                               const vector<Pnt3> &nrmb,
                               const Xform<float> &rel_xf,
                               float &pw_point_rmsErr, float &pw_plane_rmsErr) {
    assert(ptsa.size());
...
void GlobalReg::mapEntry::stats(void) {
...
static bool write_points(ofstream &out, vector<Pnt3> &pts, TbObj *scan) {
    ...
    if (cs && GetTclGlobalBool("writeGRsRaw")) {
        ...
        for (int i = 0; i < n; i++) {
            if (cs->get_raw_data(pts[i], cw[i]) == false) {
                cerr << "Problem in GlobalReg::write_points()" << endl;
                return false;
            }
        }
...
static bool read_points(ifstream &in, vector<Pnt3> &pts) {
    // magic number (general point data 1)
    char magic[4];
    in.read(magic, 4);
...
void GlobalReg::mapEntry::export_to_file(const std::string &gr_dir) {
...
// just overwrite the file if it exists
    ...
    cout << "Writing correspondences between " << name1 << " and " << name2
         << " ... ";
...
void GlobalReg::mapEntry::export_cyber_raw(const char *fname) {
...
void GlobalReg::get_pts(TbObj *x, vector<Pnt3> &P, vector<Pnt3> &Q,
                        TbObj *partner) {
    P.clear();
    Q.clear();
...
void GlobalReg::get_pts_within_group(TbObj *x, vector<Pnt3> &P, vector<Pnt3> &Q,
                                     vector<TbObj *> &group) {
    P.clear();
    Q.clear();
...
void GlobalReg::evaluate(const vector<TbObj *> &group) {
    for (int i = 0; i < group.size(); i++) {
        FOR_MATCHING_KEYS(group[i], data) {
            // TbObjs in a pair must be in the same group!
            assert(data->xfa == group[i] || CONTAINS(group, data->xfa));
            assert(data->xfb == group[i] || CONTAINS(group, data->xfb));
...
float GlobalReg::evaluate(TbObj *one) {
...
static string nameFromTbObj(TbObj *tbo) {
    if (tbo) {
...
void GlobalReg::dump(void) {
    ...
    FOR_MAP_ENTRIES(key, data) {
        ...
        cout << "-" << endl;
        copy(ptsa.begin(), ptsa.end(), ostream_iterator<Pnt3>(cout, "\n"));
        vector<Pnt3> &ptsb = data->ptsb;
        cout << "-" << endl;
        copy(ptsb.begin(), ptsb.end(), ostream_iterator<Pnt3>(cout, "\n"));
        cout << "-" << endl;
...
void GlobalReg::move_back(const vector<TbObj *> &group,
    ...
    // apply it
    for (i = 0; i < group.size(); i++) {
        group[i]->setXform((xfp * group[i]->getXform()).enforce_rigidity());
    }
    cout << "done." << endl;
}

Xform<float> GlobalReg::compute_xform(vector<Pnt3> &P, vector<Pnt3> &Q) {
    Xform<float> xf;
    double q[7];
    horn_align(&P[0], &Q[0], P.size(), q);
...
void GlobalReg::align_one_to_others(TbObj *one, TbObj *two) {
    vector<Pnt3> P, Q;
    ...
    do {
        ...
        one->setXform(compute_xform(P, Q) * one->getXform(), false);

        sum = evaluate(one);
        cout << "The average point-to-point distance is " << sum << endl;
...
void GlobalReg::align_group(const vector<TbObj *> &group) {
    vector<Pnt3> P, Q;
    int i;
    ...
    if (active.size() == 0) {
        ...
        for (i = 0; i < group.size(); i++) {
            int links = 0;
            FOR_MATCHING_KEYS(group[i], data) { links++; }
            END_FOR_KEYS;
            if (links > seed_links) {
                seed_links = links;
                seed = group[i];
    ...
    while (!dormant.empty()) {
        ...
        for (it = dormant.begin(); it != dormant.end(); it++) {
            int links = 0;
            FOR_ACTIVE_NBORS(*it, nbor) { links++; }
            END_FOR_NBORS;
            if (links > seed_links) {
                seed_links = links;
                pos = it;
        ...
        while (!que.empty()) {
            ...
            nbors.clear();
            FOR_ACTIVE_NBORS(seed, nbor) { nbors.push_back(nbor); }
            END_FOR_NBORS;
            ...
            if (diff > ftol * end_dist && diff > 1.0e-6) {
                if (cnt++ < 200) {
...
bool GlobalReg::getPairError(TbObj *a, TbObj *b, float &pointError,
                             float &planeError, float &globalError,
                             bool &manual) {
...
// the older one
GlobalReg::mapEntry *GlobalReg::import(const std::string &fname, bool manual) {
    ...
    FOR_MATCHING_KEYS(tbobj[0], data) {
        if ((data->xfa == tbobj[0] && data->xfb == tbobj[1]) ||
            ...
            if (manual && data->manually_aligned == false) {
                ...
                cerr << "Warning: there seem to be several entries " << endl
                     << "involving meshes " << name[0].c_str() << " and "
                     << name[1].c_str() << endl;
...
#endif
    if (!in) {
        cerr << "Couldn't open file " << fname.c_str() << " for reading"
             << endl;
        return NULL;
    }
    ...
    case 0:
    default:
        cerr << fname.c_str() << " does not have the correct signature" << endl;
    ...
    mapEntry *entry =
        addPair(tbobj[0], tbobj[1], ap, bp, pw_point_rmsErr, pw_plane_rmsErr,
...
inline void unlink_gr_file(std::string path, std::string n1, std::string n2) {
    path += "/";
    path += n1;
...
void GlobalReg::unlink_gr_files(TbObj *a, TbObj *b, bool only_auto) {
...
GlobalReg::GlobalReg(void) : initial_import_done(false) {
...
void GlobalReg::initial_import(void) {
    if (!initial_import_done) {
        ...
        if (!check_file_access(gr_auto_dir.c_str(), 1, 1, 1, 1, 1)) {
            cerr << "Creating autoreg directory " << gr_auto_dir.c_str()
                 << endl;
...
void GlobalReg::perform_import(void) {
    cerr << "Importing *.gr... " << flush;

    int nTotal = 0, nYes = 0;
    int nTotalAuto = 0, nYesAuto = 0;
    for (DirEntries de_auto(gr_auto_dir, ".gr"); !de_auto.done();
         de_auto.next()) {
        ++nTotalAuto;
    ...
    cerr << "imported " << nYesAuto << " / " << nTotalAuto << " auto; " << nYes
         << " / " << nTotal << " manual." << endl;
...
GlobalReg::mapEntry *GlobalReg::addPair(
    TbObj *a, TbObj *b, const vector<Pnt3> &ap, const vector<Pnt3> &nrma,
    const vector<Pnt3> &bp, const vector<Pnt3> &nrmb, Xform<float> rel_xf,
    bool manually_aligned, int max_pairs, bool save, int saveQual) {
    ...
    calcPairwiseRmsErr(ap, nrma, bp, nrmb, rel_xf, me->pw_point_rmsErr,
                       me->pw_plane_rmsErr);
    // add it twice
    ...
    if (save)
        me->export_to_file(gr_dir);
    if (cyber_raw_name)
        me->export_cyber_raw(cyber_raw_name);

    return me;
}

GlobalReg::mapEntry *GlobalReg::addPair(
    TbObj *a, TbObj *b, const vector<Pnt3> &ap, const vector<Pnt3> &bp,
    float pw_point_rmsErr, float pw_plane_rmsErr, Xform<float> rel_xf,
    bool manually_aligned, int max_pairs, bool save, int saveQual) {
    assert(ap.size() == bp.size());
    // delete previous instance
    deletePair(a, b, save);
...
bool GlobalReg::markPairQuality(TbObj *a, TbObj *b, int saveQual) {
    FOR_MATCHING_KEYS(a, data) {
...
void GlobalReg::deletePair(TbObj *a, TbObj *b, bool deleteFile) {
...
void GlobalReg::deleteAllPairs(TbObj *a, bool deleteFile) {
    bool again;
    do {
        again = false;
        ...
        FOR_MATCHING_KEYS(a, data) {
            ...
            if (deleteFile)
                unlink_gr_files(data->xfa, data->xfb);
...
void GlobalReg::deleteAutoPairs(float thr, TbObj *a) {
    bool again;
    int cnt = 0;
    do {
        again = false;
        ...
        FOR_MAP_ENTRIES(key, data) {
            ...
            if (a != NULL && data->xfa != a && data->xfb != a)
                continue;

            if (!data->manually_aligned && data->pw_plane_rmsErr > thr) {
    ...
    cerr << "Nuked " << cnt << " auto pairs greater than " << thr << endl;
}

void GlobalReg::showPointpairCount(TbObj *a, TbObj *b) {
    assert(a);
    if (b == NULL) {
        cout << endl << "From " << nameFromTbObj(a) << " to" << endl;
    ...
    FOR_MATCHING_KEYS(a, data) {
        if (b == NULL) {
            ...
            cout << data->maxErr << "\t";
            if (data->xfa == a)
                cout << nameFromTbObj(data->xfb);
            else
                cout << nameFromTbObj(data->xfa);
            cout << endl;
            found = true;
...
bool GlobalReg::pairRegistered(TbObj *a, TbObj *b, bool &manual,
    ...
    if (transitiveAllowed) {
        ...
        FOR_MAP_ENTRIES(key, data) {
            if (key == data->xfb)
                continue;
            cc.connect(scan_idx(data->xfa, scan), scan_idx(data->xfb, scan));
        }
        END_FOR_MAP;
...
string GlobalReg::list_partners(TbObj *mesh, bool transitiveAllowed) {
    string response;
...
int GlobalReg::count_partners(TbObj *mesh) {
    int partners = 0;
    FOR_MATCHING_KEYS(mesh, data) { ++partners; }
    END_FOR_KEYS;

    return partners;
}

void GlobalReg::dump_connected_groups(void) {
    // create a vector of the TbObj pointers
    vector<TbObj *> scan;
    for (ITS is = all_scans.begin(); is != all_scans.end(); is++) {
        scan.push_back(*is);
...
void GlobalReg::align(float _ftol, TbObj *scanToMove, TbObj *scanToMoveTo) {
    ftol = _ftol;
...
void GlobalReg::align(float _ftol, Bbox worldBbox, TbObj *scanToMove,
                      TbObj *scanToMoveTo) {
    ftol = _ftol;
...
void GlobalReg::normalize_samples(Bbox worldBbox, TbObj *scanToMoveTo) {
    ...
    int numBuckets = init_vol_buckets(&vb, worldBbox, scanToMoveTo, VB_SIZE);
    cerr << "done" << endl;
    ...
    cerr << "iterating through points..." << scans.size() << " scans" << endl;
...
bool GlobalReg::should_delete_point(Pnt3 pt, VolumeBucket *vb, int bucketInd) {
    if (vb[bucketInd].points.size() > THRESHOLD)
        return (true);
...
int GlobalReg::init_vol_buckets(VolumeBucket **vb, Bbox worldBbox,
...
int GlobalReg::find_bucket(Pnt3 pt, VolumeBucket *vb, float bucket_side,
                           int numBuckets) {
...
std::string GlobalReg::dump_meshpairs(int choice, float listThresh) {
    vector<string> names;
    vector<TbObj *> objs;
    ...
    for (vector<DisplayableMesh *>::iterator dm = theScene->meshSets.begin();
        ...
        if ((*dm)->getVisible()) {
            ...
            cout << "Excluding invisible mesh " << (*dm)->getName() << endl;
        }
    }
    ...
    for (i = 0; i < names.size(); i++) {
        ...
        for (j = 0; j < names.size(); j++) {
            char code[3];
            sprintf(code, " -");
            bool paired = getPairError(objs[i], objs[j], pointError, planeError,
                                       globalError, manual);
            float error;
            if (choice == 0)
                error = pointError;
            ...
            if (paired) {
                ...
                if (i > j)
                    sortedList.insert(
                        pair<const float, int>(error, i * 65000 + j));
            }
    ...
    if (listThresh < FLT_MAX) {
        ...
        for (it = sortedList.begin(); it != sortedList.end(); it++) {
            ...
            if (err > listThresh) {
                ...
                bool paired = getPairError(objs[i], objs[j], pointError,
                                           planeError, globalError, manual);
                ans += names[i].c_str();
                ans += " ";
                ans += names[j].c_str();
                if (manual) {
                    printf("%s to %s: %g \t(manual %f)\n", names[i].c_str(),
                           names[j].c_str(), err, planeError);
                    ans += " manual ";
                } else {
...
static void update_getPairingSummary_fields(GlobalReg::mapEntry *data,
                                            GlobalReg::ERRMETRIC metric,
                                            int count[3], float err[3],
                                            int qual[4]) {
    ++count[0];
    ...
    float errSrc;
    switch (metric) {
    case GlobalReg::errmetric_max:
        errSrc = data->maxErr;
        break;
    case GlobalReg::errmetric_avg:
        errSrc = data->avgErr;
        break;
    case GlobalReg::errmetric_rms:
        errSrc = data->rmsErr;
        break;
    case GlobalReg::errmetric_pln:
        errSrc = data->pw_plane_rmsErr;
        break;
    default: // this is the only one populated by a single icp
    case GlobalReg::errmetric_pnt:
        errSrc = data->pw_point_rmsErr;
        break;
    }
...
bool GlobalReg::getPairingSummary(TbObj *mesh, ERRMETRIC metric, int count[3],
                                  float err[3], int qual[4]) {
...
bool GlobalReg::getPairingStats(TbObj *from, TbObj *to, bool &bManual,
                                int &iQuality, int &nPoints, time_t &date,
                                float err[5]) {
...
void main(void) {
    ...
    for (i = 0; i < nviews; i++) {
        ...
        xf[i].translate((rnd() - .5) * fact, (rnd() - .5) * fact,
                        (rnd() - .5) * fact);
    }
    ...
    for (i = 0; i < nviews; i++) {
        ...
        for (j = 1; j < 4; j++) {
            for (k = 0; k < 3000; k++) {
                ...
                P[k] += Pnt3(rnd() * .001 - .0005, rnd() * .001 - .0005,
                             rnd() * .001 - .0005);
                Q[k] += Pnt3(rnd() * .001 - .0005, rnd() * .001 - .0005,
                             rnd() * .001 - .0005);
            }
    ...
    copy(xf.begin(), xf.end(), ostream_iterator<Xform<float> >(cout, "\n"));
