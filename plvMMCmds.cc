...
int MmsAbsorbXformCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                      char *argv[]) {
    ...
    if (argc == 3)
        mm->absorb_xforms(argv[2]);
    else
        mm->absorb_xforms(argv[1]);
...
int MmsGetScanFalseColorCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                            char *argv[]) {
...
int MmsNumScansCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                   char *argv[]) {
...
int MmsVripOrientCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                     char *argv[]) {
    if ((argc != 3) && (argc != 4)) {
    ...
    if (argc == 4) {
        if (strcmp(argv[3], "-noxform") == 0) {
            printf("cancel that... no alignment with Z-axis, only saving "
                   "pieces.\n");
...
int PlvWriteMMForVripCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                         char *argv[]) {
    if (argc < 3) {
        Tcl_SetResult(
            interp,
...
int MmsIsScanVisibleCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                        char *argv[]) {
    if (argc != 3)
        return TCL_ERROR;
...
int MmsSetScanVisibleCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                         char *argv[]) {
    if (argc != 4)
        return TCL_ERROR;
...
int MmsResetCacheCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                     char *argv[]) {
    if (argc != 2)
        return TCL_ERROR;
...
int MmsDeleteScanCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                     char *argv[]) {
    if (argc != 3)
        return TCL_ERROR;
...
int MmsFlipScanNormsCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                        char *argv[]) {
    if (argc != 3)
        return TCL_ERROR;
