...
int PlvWriteSDForVripCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                         char *argv[]) {
    if (argc < 5) {
    ...
    for (; dm < theScene->meshSets.end(); dm++) {
        ...
        for (vector<CyberSweep *>::iterator pSweep = sweeps.begin();
            ...
            int numSubSweeps =
                useSubSweeps ? (sweep->num_frames() - 1) / framesPerSweep + 1
                             : 1;
            ...
            for (int k = 0; k < numSubSweeps; k++) {
                ...
                if (access(name, R_OK)) {
                    ...
                    cout << "Skipping " << name << " (already exists) ... "
                         << endl;
                ...
                if (!bConfOnly) { // create mesh
                    if (useSubSweeps) {
                        ...
                        int frameFinish =
                            MIN(sweep->num_frames() - 1,
                                (k + 1) * framesPerSweep + overlap + 1);
                        newScan = (CyberScan *)cs->get_piece(
                            sweepNum, frameStart, frameFinish);
                        ...
                        vector<CyberSweep *> newSweeps =
                            newScan->get_sweep_list();
                        ...
                        // more of its sweeps; harmless enough to call more than
                        // once
                ...
                if (res) {
                    ...
                    Xform<float> xfRotate =
                        newSweep->vrip_reorientation_frame();
                    xfRotate.invert();

                    if (!bConfOnly) {
                        // write plyfile
                        if (!newSweep->write_resolution_mesh(res, name,
                                                             xfRotate)) {
                        ...
                        /* Started to write something that would save bboxes for
                           pvrip, but realized that the world bboxes would not
                           be
                           tight.  We can add tight world bboxes later.  For
                           now,
                    ...
                    sprintf(bmesh, "bmesh %s %g %g %g %g %g %g %g",
                            1 + strrchr(name, '/'), t[0], t[1], t[2], -q[1],
                            -q[2], -q[3], q[0]);
...
int PlvCyberScanSelfAlignCmd(ClientData clientData, Tcl_Interp *interp,
    ...
    // check input
    if (argc < 2) {
        Tcl_SetResult(interp, "No scan passed to PlvCyberScanSelfAlignCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (!dm) {
        Tcl_SetResult(interp, "Missing scan in PlvCyberScanSelfAlignCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    vector<vector<CyberSweep *> > shells = scan->get_ordered_sweeps();
    ...
    for (i = 0; i < shells.size(); i++) {
        ...
        for (j = 0; j < shell.size(); j++) {
            if (!shell[j]->write_metadata(RigidScan::md_xform)) {
                cerr << "Couldn't write the xform for " << shell[j]->get_name()
                     << endl;
            } else {
                cerr << "Saved xform for " << shell[j]->get_name() << endl;
            }
        }
    }
...
int ScnDumpLaserPntsCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                        char *argv[]) {
    // check input
    if (argc < 4) {
        Tcl_SetResult(interp,
    ...
    DisplayableMesh *dm = FindMeshDisplayInfo(argv[1]);
    if (!dm) {
        Tcl_SetResult(interp, "Missing scan in ScnDumpLaserPntsCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    CyberScan *scan = dynamic_cast<CyberScan *>(dm->getMeshData());
    if (!scan) {
        Tcl_SetResult(interp, "ScnDumpLaserPntsCmd: that's not a CyberScan!",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
int PlvDiceCyberDataCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                        char *argv[]) {
            ...
            for (int k = 0; k < numSubSweeps; k++) {
                ...
                if (meshTo) {
                    if (meshTo->num_resolutions() > 0) {
                        ...
                        Tcl_VarEval(interp, "changeVis ", oldDisp->getName(),
                                    " 0", NULL);
                    }
                }
            }
...
int PlvWorkingVolumeCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                        char *argv[]) {
    ...
    if (argc == 2 && !strcmp(argv[1], "off")) {
        ...
        Tcl_SetResult(interp, "Bad arguments to plv_working_volume",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
int PlvWorldCoordToSweepCoord(ClientData clientData, Tcl_Interp *interp,
    ...
    if (argc != 5) {
        // Usage
        Tcl_SetResult(interp, "Usage: plv_worldCoordToSweepCoord scanname x y "
                              "z\n  returns four numbers that can be used to "
                              "resontruct a new world coord \n  after "
                              "callibration or realignment.\n",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    // Return results
    if (res) {
        sprintf(Tcl_GetStringResult(interp), "%d %f %f %f", sweepInd,
                (float)scanPt[0], (float)scanPt[1], (float)scanPt[2]);
    } else {
        Tcl_SetResult(interp, "Did not find a sweep containing the Pt.",
                      TCL_STATIC);
    }
...
int PlvSweepCoordToWorldCoord(ClientData clientData, Tcl_Interp *interp,
    ...
    // Usage
    if (argc != 6) {
        Tcl_SetResult(interp, "Usage: plv_sweepCoordToWorldCoord scanname "
                              "sweepindex val1 val2 val3",
                      TCL_STATIC);
        return TCL_ERROR;
    }
