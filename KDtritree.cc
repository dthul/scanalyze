...
static int mediansort(KDsphere *data, int n, int dim, float med) {
    // move values <= med to left, the rest to right
    int left = 0, right = n - 1;
    while (1) {
        while (data[left].ctr[dim] <= med && right > left)
            left++;
        while (data[right].ctr[dim] > med && right > left)
            right--;
        KDsphere tmp = data[left]; // swap
        data[left] = data[right];
        data[right] = tmp;
        if (left == right) {
            if (data[right].ctr[dim] <= med)
                right++;
            break;
        }
        left++;
...
void KDtritree::collapse_spheres(void) {
...
KDtritree::KDtritree(const Pnt3 *pts, const int *triinds, KDsphere *spheres,
                     int n, int first) {
    int i;
    ...
    if (n > 1) {
        ...
        Bbox bbox;
        for (i = 0; i < n; i++)
            bbox.add(spheres[i].ctr);
        Pnt3 diag = bbox.max();
        diag -= bbox.min();
        if (diag[2] > diag[1] && diag[2] > diag[0])
            ind = 2;
        else
            ind = diag[1] > diag[0];
...
void KDtritree::_search(const Pnt3 *pts, const int *inds, const Pnt3 &p,
                        Pnt3 &cp, float &d2) const {
    if (child[0] == NULL) {
        // terminal node, check the triangle and return
        closer_on_tri(p, cp, pts[inds[ind]], pts[inds[ind + 1]],
                      pts[inds[ind + 2]], d2);
        return;
    }
...
KDtritree *create_KDtritree(const Pnt3 *pts, const int *inds, int n) {
    ...
    for (int i = 0; i < ns; i++) {
        int ti = i * 3;
        spheres[i].radius = spheres[i].ctr.smallest_circle(
            pts[inds[ti]], pts[inds[ti + 1]], pts[inds[ti + 2]]);
        spheres[i].ind = ti;
        // SHOW(spheres[i].radius);
        // SHOW(spheres[i].ctr);
    }
