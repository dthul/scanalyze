...
CameraParams *CameraParams::Read(const char *tsaifile) {
    ...
    printf("Reading camera parameters file... ");
    fflush(stdout);
    ...
    if ((dummy1 != pixsize) || (dummy2 != pixsize) || (dummy3 != pixsize)) {
    ...
    if (dummy1 != 0) {
        fprintf(
            stderr,
            "Yikes! The distortion is not zero.\n I can't deal with that.\n");
    }
    ...
    return new CameraParams(imgwidth, imgheight, Tx, Ty, Tz, Rx, Ry, Rz, Cx, Cy,
                            S, pixels_per_radian);
}
...
//
const double *CameraParams::GLmodelmatrix() const { return matrix; }
