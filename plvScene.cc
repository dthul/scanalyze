...
class dmNameSort
    : public binary_function<DisplayableMesh *, DisplayableMesh *, bool> {
  public:
    bool operator()(DisplayableMesh *d1, DisplayableMesh *d2);
};

DisplayableMesh *Scene::addMeshSet(RigidScan *scan, bool bRecenterCameraOK,
                                   char *nameToUse) {
    ...
    if (bRecenterCameraOK) {
        char *pszAutoRecenter =
            Tcl_GetVar(interp, "autoCenterCamera", TCL_GLOBAL_ONLY);
...
void Scene::addMeshInternal(DisplayableMesh *dm) {
    auto pos = lower_bound(meshSets.begin(), meshSets.end(), dm, dmNameSort());
...
void Scene::deleteMeshSet(DisplayableMesh *deadMesh) {
    ...
    vector<DisplayableMesh *>::iterator mydm =
        find(g_hilitedScans.begin(), g_hilitedScans.end(), *dm);
    if (mydm != g_hilitedScans.end())
        g_hilitedScans.erase(mydm);
...
bool Scene::renameMeshSet(DisplayableMesh *mesh, const char *nameToUse) {
    // TODO
    return false;
}
...
void Scene::computeBBox(bboxSyncMode synchronous) {
    ...
    if (synchronous == async) {
        _nextBboxUpdate =
            Tcl_CreateTimerHandler(20, ReallyUpdateBbox, (ClientData) this);
        return;
    }
...
void Scene::setMeshResolution(int res) {
    ...
    bool bInvisibleToo =
        atoi(Tcl_GetVar(interp, "selectResIncludesInvisible", TCL_GLOBAL_ONLY));
...
int Scene::getMeshResolution(void) { return resOverride; }

void Scene::invalidateDisplayCaches(void) {
    for (int k = 0; k < meshSets.size(); k++) {
        meshSets[k]->invalidateCachedData();
    }
}

bool Scene::wantMeshBBox(DisplayableMesh *mesh) {
    return (theRenderParams->boundSelection && mesh == theActiveScan);
}

void Scene::setHome(void) { homePos = *tbView; }

void Scene::goHome(void) {
    *tbView = homePos;
    computeBBox();
}

float Scene::sceneRadius(void) const { return bbox.diag() * .5; }

Pnt3 Scene::worldCenter(void) { return bbox.center(); }

void Scene::flattenCameraXform(void) {
...
bool Scene::meshes_written_stripped(void) {
...
int Scene::getSlowPolyCount(void) { return slowPolyCount; }

void Scene::setSlowPolyCount(int count) { slowPolyCount = count; }
...
int AddMeshSetToHash(DisplayableMesh *mesh) {
    ...
    int isnew;
    Tcl_HashEntry *ent =
        Tcl_CreateHashEntry(&meshSetHash, mesh->getName(), &isnew);
    if (!isnew)
...
DisplayableMesh *FindMeshDisplayInfo(const char *name) {
    if (!mesh_set_hash_initted)
        return NULL;
...
DisplayableMesh *FindMeshDisplayInfo(RigidScan *scan) {
...
DisplayableMesh *GetMeshForRigidScan(RigidScan *scan) {
    DisplayableMesh *dm;
...
bool Scene::readSessionFile(char *sessionName) {
...
bool Scene::writeSessionFile(char *sessionName) {
    ...
    for (vector<DisplayableMesh *>::iterator pdm = meshSets.begin();
        ...
        file << scan->get_name() << ": " << bbox.min() << " * " << bbox.max()
             << endl;
        if (file.fail())
            return false;
    }
...
bool Scene::setScanLoadedStatus(DisplayableMesh *dm, bool load) {
    RigidScan *scan = NULL;
...
void Scene::sortSceneMeshes(vector<DisplayableMesh *> &sortedList,
                            vector<sortCriteria> &criteria,
                            bool bDictionarySort, bool bListInvisible) {
    if (bListInvisible) {
        sortedList = meshSets;
    } else {
...
bool sceneMeshSort::operator()(DisplayableMesh *d1, DisplayableMesh *d2) {
    int res;
