...
class MMScan : public RigidScan {
  ...
  public:
    ...
    struct mmResLevel {
        ...
        int num_tris(void) { return tris.size() / 3; }
        int num_vertices(void) { return vtx.size(); }
    };
    ...
    struct mmScanFrag {
        ...
        int num_stripes(void) { return stripes.size(); }
        int num_tris(void) {
            if (meshes.size() == 0)
                return 0;
            if (!isVisible)
    ...
    bool is_modified(void) { return isDirty_disk; }
    ...
    int write_xform_ply(int whichRes, char *dir, string &confLines,
                        bool applyXform = true);
    ...
    void setScanVisible(int scanNum, int shouldBeVisible) {
        if ((scanNum >= scans.size()) || (scanNum < 0))
            return;
