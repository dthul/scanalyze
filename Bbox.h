...
class Bbox {
  ...
  public:
    Bbox(void) { clear(); }
    Bbox(Pnt3 const &_lo, Pnt3 const &_hi) : lo(_lo), hi(_hi), cdone(0) {}
    ~Bbox(void) {}
    ...
    void add(const double *a) { add(a[0], a[1], a[2]); }
    void add(const float *a) { add(a[0], a[1], a[2]); }
    ...
    int outside(const Pnt3 &p, float d = 0.0) const {
        return (p[0] > hi[0] + d || p[0] < lo[0] - d || p[1] > hi[1] + d ||
                p[1] < lo[1] - d || p[2] > hi[2] + d || p[2] < lo[2] - d);
    }
    ...
    Pnt3 corner(int i) const {
        return Pnt3((i & 4) ? hi[0] : lo[0], (i & 2) ? hi[1] : lo[1],
                    (i & 1) ? hi[2] : lo[2]);
    }
...
inline ostream &operator<<(ostream &out, const Bbox &a) {
