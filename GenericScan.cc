...
GenericScan::GenericScan() { _Init(); }

void GenericScan::_Init() {
    bDirty = true;
    bNameSet = false;
    pushcount = 0;
    myRangeGrid = NULL;
...
MeshTransport *GenericScan::mesh(bool perVertex, bool stripped,
...
void GenericScan::setMTColor(Mesh *mesh, MeshTransport *mt, bool perVertex,
    ...
    if (source == colorBoundary) {
        ...
        if (!mesh->vertMatDiff && !mesh->vertIntensity &&
            ...
            return; // no color data at all, or only per-face but per-vert
                    // requested
    }
    ...
    if (source == colorConf) {
            ...
            for (int i = 0; i < mesh->tris.size(); i += 3) {
                pushConf(*colors, colorsize,
                         (mesh->vertConfidence[mesh->tris[i + 0]] +
                          ...
                          mesh->vertConfidence[mesh->tris[i + 2]]) /
                             3);
            }
        }
        ...
        if (perVertex) {
            ...
            for (int i = 0; i < mesh->tris.size(); i += 3) {
                ...
                } else { // prefer intensity, then rgb from faces, then rgb from
                         // verts
                    if (mesh->vertIntensity)
                        pushColor(*colors, colorsize,
...
int GenericScan::num_vertices(void) { return currentMesh()->num_verts(); }

void GenericScan::subsample_points(float rate, vector<Pnt3> &p,
                                   vector<Pnt3> &n) {
    currentMesh()->subsample_points(rate, p, n);
}

bool GenericScan::read(const string &fname) {
    bool success = FALSE;
...
void GenericScan::setd(const string &dir, bool bCreate) {
...
bool GenericScan::readSet(const string &fn) {
    // read .set file
...
bool GenericScan::readSingleFile(const string &fn) {
...
Mesh *GenericScan::readMeshFile(const char *name) {
    Mesh *mesh = new Mesh;
...
int GenericScan::create_resolution_absolute(int budget, Decimator dec) {
    ...
    Mesh *newMesh = origMesh->Decimate(budget, PLACE_OPTIMAL, 0, 1000, dec);
...
bool GenericScan::delete_resolution(int nPolys) {
...
void GenericScan::flipNormals(void) {
    for (int i = 0; i < meshes.size(); i++)
        meshes[i]->flipNormals();
    bDirty = true;
}

void GenericScan::insertMesh(Mesh *m, const string &filename, bool bLoaded,
                             bool bAlwaysLoad, int nRes) {
...
Mesh *GenericScan::currentMesh(void) {
...
bool GenericScan::load_resolution(int i) {
    if (resolutions[i].in_memory)
        return true;
...
bool GenericScan::release_resolution(int nPolys) {
    int iRes = findLevelForRes(nPolys);
    if (iRes < 0) {
...
bool GenericScan::getXformFilename(const char *meshName, char *xfName) {
...
KDindtree *GenericScan::get_current_kdtree() {
    ...
    Mesh *mesh = currentMesh();
    kdtree[iTree] =
        CreateKDindtree(mesh->vtx.data(), mesh->nrm.data(), mesh->vtx.size());

    return kdtree[iTree];
}

bool GenericScan::closest_point(const Pnt3 &p, const Pnt3 &n, Pnt3 &cp,
...
string GenericScan::getInfo(void) {
    char info[512];
    sprintf(info, "Polygon file: %ld resolutions\n"
                  "verts %d tris %ld strips %ld\n\n",
            resolutions.size(), currentMesh()->num_verts(),
            currentMesh()->tris.size() / 3, currentMesh()->tstrips.size());

    return string(info) + RigidScan::getInfo();
}

Mesh *GenericScan::getMesh(int level) {
...
inline Mesh *GenericScan::highestRes(void) { return getMesh(0); }
...
void GenericScan::PrintVoxelInfo() {
    ...
    for (int i = 0; i < triSize; i += 3) {
        ...
        printf("(%f, %f, %f), (%f, %f, %f), (%f, %f, %f)\n",
               ...
               mesh->vtx[mesh->tris[i]][2], mesh->vtx[mesh->tris[i + 1]][0],
               mesh->vtx[mesh->tris[i + 1]][1], mesh->vtx[mesh->tris[i + 1]][2],
               mesh->vtx[mesh->tris[i + 2]][0], mesh->vtx[mesh->tris[i + 2]][1],
               mesh->vtx[mesh->tris[i + 2]][2]);
        printf("came from voxel at\n");
        printf("(%f, %f, %f)\n", mesh->fromVoxels[i], mesh->fromVoxels[i + 1],
               mesh->fromVoxels[i + 2]);
    }
}
...
RigidScan *GenericScan::filtered_copy(const VertexFilter &filter) {
    int i, j;
...
bool GenericScan::filter_inplace(const VertexFilter &filter) {
...
bool GenericScan::filter_vertices(const VertexFilter &filter, vector<Pnt3> &p) {
    Mesh *mesh = currentMesh();
...
bool GenericScan::is_modified(void) { return bDirty; }

bool GenericScan::write_metadata(MetaData data) {
    bool success = false;
...
bool GenericScan::write(const string &fname) {
    ...
    for (int i = 0; i < meshes.size(); i++) {
        if (!bNameSet) {
            ...
            resolutions[i].filename = basename + "." + stupidbuf + ".ply";
        ...
        if (resolutions[i].in_memory && meshes[i]->bNeedsSave) {
            ...
            meshes[i]->writePlyFile(resolutions[i].filename.c_str(), true,
                                    false);
        }
    }
    popd();
...
bool GenericScan::write_resolution_mesh(int nPolys, const string &fname,
                                        Xform<float> xfBy) {
    if (!xfBy.isIdentity()) {
...
int PlvWritePlyForVripCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                          char *argv[]) {
    if (argc < 3) {
    ...
    char *oldStripVal = Tcl_GetVar(interp, "meshWriteStrips", TCL_GLOBAL_ONLY);
    Tcl_SetVar(interp, "meshWriteStrips", "never", TCL_GLOBAL_ONLY);
    ...
    for (; dm < theScene->meshSets.end(); dm++) {
        ...
        if (res) {
            ...
            if (!gs->write_resolution_mesh(res, name, Xform<float>())) {
                cerr << "Scan " << sd->get_name() << " failed to write self!"
                     << endl;
                return TCL_ERROR;
            }
            ...
            sprintf(bmesh, "bmesh %s %g %g %g %g %g %g %g",
                    1 + strrchr(name, '/'), t[0], t[1], t[2], -q[1], -q[2],
                    -q[3], q[0]);
            ...
            xffile << xf;
            if (xffile.fail()) {
                cerr << "Scan " << sd->get_name() << " failed to write xform!"
                     << endl;
                success = false;
                break;
            }
    ...
    // cleanup
    Tcl_SetVar(interp, "meshWriteStrips", oldStripVal, TCL_GLOBAL_ONLY);
...
void GenericScan::dequantizationSmoothing(int iterations,
