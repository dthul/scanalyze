...
void axis_to_quat(Pnt3 &a, float phi, float q[4]) {
    a.normalize();
    a *= sin(phi / 2.0);
    q[1] = a[0];
    q[2] = a[1];
...
Trackball::Trackball(void)
    : spinning_enabled(true), rotating(0), spinning(0), panning(0), zooming(0),
      orthographic(false), lastTarget(NULL) {
    trackball(currQuat, 0.0, 0.0, 0.0, 0.0);
    setTransScale();
}

void Trackball::setSize(int w, int h) {
    W = w;
    H = h;
}
...
void Trackball::setup(float _c[3], float _o[3], float _up[3], float r,
...
void Trackball::resetXform(void) {
    trans = Pnt3();
...
void Trackball::newRotationCenter(float _o[3], TbObj *target) {
    Pnt3 newCtr = Pnt3(_o);
...
void Trackball::changeFOV(float fov) { field_of_view = fov; }

float Trackball::getFOV(void) { return field_of_view; }

void Trackball::setObliqueCamera(float x, float y) {
    oblique_camera_offset_x = x;
    oblique_camera_offset_y = y;
}

void Trackball::resetAxes(void) {
...
void Trackball::setClippingPlanes(Pnt3 bb_ctr, float bb_r) {
...
void Trackball::pressButton(int button, int up, int x, int y, int t,
                            TbObj *target) {
    spinning_enabled =
        atoi(Tcl_GetVar(g_tclInterp, "spinningAllowed", TCL_GLOBAL_ONLY));
    lastTarget = target;
    y = (H - 1) - y;
...
void Trackball::move(int x, int y, int t, TbObj *target) {
    ...
    if (rotating) {
        trackball(lastQuat, 2.0 * beginx / W - 1.0, 2.0 * beginy / H - 1.0,
                  2.0 * x / W - 1.0, 2.0 * y / H - 1.0, constraint);
        /*
        ...
        */
        ...
        if (zooming) {
            ...
            if (orthographic) {
                ...
                float newFocalDist =
                    focalDistance * powf(2.0, (y - beginy) / 80.0);
...
void Trackball::getXform(float quat[4], float t[3]) {
    for (int i = 0; i < 4; i++)
        quat[i] = currQuat[i];
...
void Trackball::getXform(float m[4][4], float t[3]) {
    float q[4];
...
void Trackball::applyXform(int bReinitMatrix) {
    spin();
    reapplyXform(bReinitMatrix);
}

void Trackball::reapplyXform(int bReinitMatrix) {
    float m[4][4], t[3];
    getXform(m, t);
    if (bReinitMatrix) {
...
void Trackball::enable_spinning(void) { spinning_enabled = true; }

void Trackball::disable_spinning(void) { spinning_enabled = false; }

void Trackball::spin(void) {
...
bool Trackball::isSpinning(void) {
    if (!spinning_enabled)
        return false;
    return spinning && !rotating && !zooming && !panning;
}

bool Trackball::isManipulating(void) {
    return (spinning && spinning_enabled) || rotating || zooming || panning;
}

void Trackball::stop(void) { spinning = false; }

// return the viewing direction
Pnt3 Trackball::dir(void) {
    float m[4][4];
    build_rotmatrix(m, currQuat);
    return Pnt3(m[0][2], m[1][2], m[2][2]);
}

void Trackball::getRotationCenter(float center[3]) {
    for (int i = 0; i < 3; i++)
        center[i] = o[i];
}

void Trackball::getCameraCenter(float center[3]) {
    for (int i = 0; i < 3; i++)
        center[i] = c[i];
}
...
void Trackball::rotateAroundAxis(const Pnt3 &axis, float angle_rads,
                                 TbObj *target) {
    float q[4];
...
void Trackball::rotateQuat(float q[4], TbObj *target) {
...
void Trackball::constrainRotation(TbConstraint _constraint) {
    constraint = _constraint;
}

void Trackball::getBounds(float &left, float &right, float &bottom,
                          float &top) {
...
void Trackball::getFrustum(float &left, float &right, float &bottom, float &top,
...
void Trackball::setPerspXform(void) {
...
float Trackball::getOrthoHeight(void) {
...
void Trackball::setOrthoXform(void) {
    float aspect = (float)W / H;

    glOrtho(-orthoHeight / 2 * aspect, orthoHeight / 2 * aspect,
            -orthoHeight / 2, orthoHeight / 2, znear, zfar);
}

void Trackball::applyProjection() {
    int mm;
...
void Trackball::getProjection(float &pFov, float &pAspect, float &pNear,
                              float &pFar) {
    pFov = field_of_view;
    pAspect = (float)W / H;
    pNear = znear;
...
void Trackball::setOrthographic(bool bOrtho) {
...
bool Trackball::getOrthographic(void) { return orthographic; }

void Trackball::enforceZlimits(void) {
...
Xform<float> Trackball::getUndoXform(void) {
    Xform<float> xf;
...
void Trackball::setUndoXform(const Xform<float> &xf) {
...
void Trackball::translateInEyeCoords(Pnt3 &dt, TbObj *target, bool undoable) {
    if (target) {
        // moving an object
...
void Trackball::getState(Pnt3 &_c, Pnt3 &_o, Pnt3 &_t, float q[4], float &_fov,
                         float &_oblique_x, float &_oblique_y) {
    _c = c;
    _o = o;
...
void Trackball::setState(Pnt3 &_t, float q[4]) {
    for (int i = 0; i < 4; i++)
        currQuat[i] = q[i];
...
void Trackball::zoomToRect(int x1, int y1, int x2, int y2) {
...
void Trackball::setTransScale(const Pnt3 &clickPt) {
    if (clickPt[2] != 0.) {
...
void Trackball::resetTransScale(void) {
