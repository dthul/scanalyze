...
int PlvDeinitCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                 char *argv[]);
int PlvGetRendererStringCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                            char *argv[]);
int PlvGetWordSizeCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                      char *argv[]);
...
int Plv_Init(Tcl_Interp *interp) {
    ...
    PlvCreateCommand("plv_reset_rotation_center", PlvResetCenterOfRotation);
    ...
    if (plvDir == NULL) {
        Tcl_SetResult(interp, "Need to set SCANALYZE_DIR environment variable.",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (code != TCL_OK) {
        Tcl_SetResult(interp, Tcl_GetVar(interp, "errorInfo", TCL_GLOBAL_ONLY),
                      TCL_VOLATILE);
        return TCL_ERROR;
    }
