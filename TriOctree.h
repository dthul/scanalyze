...
class TriOctree {
  ...
  private:
    ...
    TriOctree *recursive_find(int face, stack<int> &path, bool bReachedTop);
    bool is_neighbor_cell(int face, int &iChild);
    TriOctree *find_neighbor(int face);

  public:
    TriOctree(const Pnt3 &c, float r, const Pnt3 *p, const vector<int> &tris,
              float minRadius, int pidx = 0, TriOctree *parent = NULL);
    ~TriOctree(void) {};
