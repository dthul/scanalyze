...
int CorrespondenceList::AddPoint(DisplayableMesh *mesh, const Pnt3 &pt) {
...
void CorrespondenceList::DeletePoint(vector<CorrespondencePt>::iterator cp) {
    points.erase(cp);
}

void CorrespondenceList::ToString(char *text) {
    sprintf(text, "[%d]", id);
    for (vector<CorrespondencePt>::iterator cp = points.begin();
         cp < points.end(); cp++) {
        sprintf(text + strlen(text), " %s:(%.4g,%.4g,%.4g)",
                cp->mesh->getName(), cp->pt[0], cp->pt[1], cp->pt[2]);
...
AlignmentInfo::AlignmentInfo() { tb = NULL; }
...
CorrespondenceList *AlignmentOverviewInfo::CompleteCorrespondence(bool bKeep,
                                                                  char *text) {
    if (correspInProgress == NULL) {
        strcpy(text, "[0]");
...
CorrespondenceList *AlignmentOverviewInfo::GetSelectedCorrespondence(void) {
    return GetCorrespondenceById(GetSelectedCorrespondenceId());
}

CorrespondenceList *AlignmentOverviewInfo::GetCorrespondenceInProgress(void) {
    return correspInProgress;
}

CorrespondenceList *AlignmentOverviewInfo::GetCorrespondenceById(int id) {
    if (id == 0)
        return NULL;

    for (vector<CorrespondenceList *>::iterator cl = correspondences.begin();
         cl < correspondences.end(); cl++) {
        if ((*cl)->id == id)
            return *cl;
    }
...
int AlignmentOverviewInfo::GetSelectedCorrespondenceId(void) {
...
void AlignmentToglInfo::SetTargetMesh(DisplayableMesh *_md) {
    meshDisplay = _md;
    if (meshDisplay == NULL) {
        meshData = NULL;
        return;
...
void DrawAlignmentPoint(const Pnt3 &pt, uchar *color, bool bSelected = false) {
...
void DrawAlignmentPoints(struct Togl *togl) {
...
void DrawAlignmentMesh(struct Togl *togl) {
...
void DrawAlignmentMeshToBack(struct Togl *togl) {
    AlignmentToglInfo *ati = (AlignmentToglInfo *)Togl_GetClientData(togl);
    assert(ati != NULL);
...
void SpinToglTrackball(struct Togl *togl) {
...
void FreeAlignmentInfo(struct Togl *togl) {
    AlignmentInfo *ai = (AlignmentInfo *)Togl_GetClientData(togl);
...
int PlvBindToglToAlignmentOverviewCmd(ClientData clientData, Tcl_Interp *interp,
                                      int argc, char *argv[]) {
    if (argc < 2) {
        Tcl_SetResult(interp,
                      "Bad arguments to PlvBindToglToAlignmentOverviewCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (togl == NULL) {
        Tcl_SetResult(interp,
                      "Missing togl widget in PlvBindToglToAlignmentViewCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
int PlvCorrespRegParmsCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                          char *argv[]) {
    if (argc < 3) {
        Tcl_SetResult(interp, "Bad arguments to PlvCorrespRegParmsCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    struct Togl *togl = toglHash.FindTogl(argv[1]);
    if (togl == NULL) {
        Tcl_SetResult(interp, "Missing togl widget in PlvCorrespRegParmsCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
int PlvBindToglToAlignmentViewCmd(ClientData clientData, Tcl_Interp *interp,
                                  int argc, char *argv[]) {
    if (argc < 3) {
        Tcl_SetResult(interp, "Bad arguments to PlvBindToglToAlignmentViewCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (mesh == NULL && strcmp(argv[2], "")) {
        Tcl_SetResult(interp, "Missing mesh in PlvBindToglToAlignmentViewCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
int PlvRegUIMouseCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                     char *argv[]) {
    ...
    struct Togl *togl = toglHash.FindTogl(argv[1]);
    if (togl == NULL) {
        Tcl_SetResult(interp, "Missing togl widget in PlvRegUIMouseCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
int PlvAddPartialRegCorrespondenceCmd(ClientData clientData, Tcl_Interp *interp,
                                      int argc, char *argv[]) {
    // argument string: togl overviewTogl x y
    if (argc < 5) {
        Tcl_SetResult(interp,
                      "Bad arguments to PlvAddPartialRegCorrespondenceCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (togl == NULL) {
        Tcl_SetResult(interp,
                      "Missing togl widget in PlvCreateRegCorrespondenceCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
        ...
        if (clSelected != NULL) {
            if (clSelected->points.size() > 2) {
                vector<CorrespondencePt>::iterator cpOld =
                    clSelected->FindPoint(ati->meshDisplay);
                if (cpOld !=
                    clSelected->points.end()) { // prompt to delete point
                    Tcl_VarEval(interp,
                                "tk_dialog .reg.confirm {Delete point?} "
                                "{Delete meshset ",
...
int PlvDeleteRegCorrespondenceCmd(ClientData clientData, Tcl_Interp *interp,
                                  int argc, char *argv[]) {
    if (argc != 3) {
        Tcl_SetResult(interp, "Bad arguments to PlvDeleteRegCorrespondenceCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (!toglOV) {
        Tcl_SetResult(interp,
                      "Missing overview togl widget in DeleteRegCorrespondence",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    AlignmentOverviewInfo *aoi =
    ...
    for (vector<CorrespondenceList *>::iterator cl =
             aoi->correspondences.begin();
         cl < aoi->correspondences.end(); cl++) {
        if ((*cl)->id == iCorresp) {
    ...
    Tcl_SetResult(interp, "Index not found in PlvDeleteRegCorrespondenceCmd",
                  TCL_STATIC);
    return TCL_ERROR;
}

int PlvConfirmRegCorrespondenceCmd(ClientData clientData, Tcl_Interp *interp,
                                   int argc, char *argv[]) {
    if (argc < 2) {
        Tcl_SetResult(interp, "Bad arguments to PlvConfirmRegCorrespondenceCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (!toglOV) {
        Tcl_SetResult(interp,
                      "Missing overview togl widget in CreateRegCorrespondence",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    AlignmentOverviewInfo *aoi =
...
int PlvGetCorrespondenceInfoCmd(ClientData clientData, Tcl_Interp *interp,
                                int argc, char *argv[]) {
    if (argc != 3) {
        Tcl_SetResult(interp, "Bad arguments to PlvGetCorrespondenceInfoCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (toglOV == NULL) {
        Tcl_SetResult(interp,
                      "Missing overlay togl in PlvGetCorrespondenceInfoCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (idCorresp == 0) {
        Tcl_SetResult(interp,
                      "Bad correspondence id in PlvGetCorrespondenceInfoCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (cl == NULL) {
        Tcl_SetResult(interp,
                      "Missing correspondence in PlvGetCorrespondenceInfoCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
int PlvCorrespondenceRegistrationCmd(ClientData clientData, Tcl_Interp *interp,
                                     int argc, char *argv[]) {
    // arguments are: overviewTogl mode from to
    // from and to are irrelevant for all2all mode, but must exist
    if (argc != 5) {
        Tcl_SetResult(interp,
                      "Bad arguments to PlvCorrespondenceRegistrationCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (toglOV == NULL) {
        Tcl_SetResult(interp,
                      "Missing overview togl in PlvCorrespRegistrationCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    AlignmentOverviewInfo *aoi =
    ...
    case from2all: {
        ...
        if (!meshFrom) {
            Tcl_SetResult(interp,
                          "Missing 'from' mesh in PlvCorrespRegistrationCmd",
                          TCL_STATIC);
            return TCL_ERROR;
        }
        ...
        if (mode == from2all)
            ...
            if (!meshTo) {
                Tcl_SetResult(interp,
                              "Missing 'to' mesh in PlvCorrespRegistrationCmd",
                              TCL_STATIC);
                return TCL_ERROR;
            }
    ...
    Tcl_SetResult(interp, "Bad mode passed to PlvCorrespRegistrationCmd",
                  TCL_STATIC);
    return TCL_ERROR;
}
...
static int CorrespRegOneMesh(AlignmentOverviewInfo *aoi,
    ...
    for (int i = 0; i < aoi->correspondences.size(); i++) {
        ...
        if (cpThis != cl->points.end()) { // references this mesh
            for (vector<CorrespondencePt>::iterator cpOther =
                     cl->points.begin();
                 cpOther < cl->points.end(); cpOther++) {
    ...
    for (int iFrom = 0; iFrom < numSets; iFrom++) {
        for (int iTo = iFrom + 1; iTo < numSets; iTo++) {
            ...
            for (int i = 0; i < aoi->correspondences.size(); i++) {
                ...
                // if correspondence mentions both meshes currently under
                // consideration
                vector<CorrespondencePt>::iterator cpFrom =
                    cl->FindPoint(mdFrom);
            ...
            // add to globalreg
            if (pntFrom.size() != 0) {
                gr.addPair(msFrom, msTo, pntFrom, vector<Pnt3>(), pntTo,
                           vector<Pnt3>());
                cout << pntFrom.size();
            } else {
                cout << "no";
            ...
            cout << " correspondences between " << mdFrom->getName() << " and "
                 << mdTo->getName() << endl;
        }
    }
...
int PlvDragRegisterCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                       char *argv[]) {
    // argument: name of mesh being registered (to all visible meshes)
    if (argc < 2) {
        Tcl_SetResult(interp, "Bad arguments to PlvDragRegisterCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (ms == NULL) {
        Tcl_SetResult(interp, "Missing meshset in PlvDragRegisterCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
// should merge code
SceneBuf *GetScenePoints(DisplayableMesh *meshWithout = NULL) {
    ...
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    unsigned int zBackground;
    glReadPixels(0, 0, 1, 1, GL_DEPTH_COMPONENT, GL_UNSIGNED_INT, &zBackground);
...
void GetMeshSceneOverlapPoints(DisplayableMesh *meshDisplay,
                               const SceneBuf &sceneBuf, vector<Pnt3> &ptMesh,
                               vector<Pnt3> &ptScene, vector<int> *ptSceneOfs) {
    ...
    for (int x = 0; x < width; x++) {
        for (int y = 0; y < height; y++) {
            int ofs = y * width + x;
            if (!ISBACKGROUND(sceneBuf.z[ofs]) && !ISBACKGROUND(bufMesh[ofs])) {
                // time to unproject
                float tx = (2. * x / width) - 1;
                float ty = (2. * y / height) - 1;
...
static void calculateNormals(vector<Pnt3> &normals, SceneBuf *sceneBuf,
    ...
    for (int i = 0; i < normals.size(); i++) {
        ...
        for (int ix = -1; ix <= 1; ix++) {
            for (int iy = -1; iy <= 1; iy++) {
                ...
                rays.push_back(sceneBuf->pt[x][y] - sceneBuf->pt[ptX][ptY]);
            }
        }
...
static void dragAlign(DisplayableMesh *mesh, AlignmentMethod method) {
    ...
    GetMeshSceneOverlapPoints(mesh, *sceneBuf, ptOrigMesh, ptOrigScene,
                              method == alignChenMedioni ? &ptSceneOfs : NULL);
    ...
    for (int iter = 0; iter < nIterations; iter++) {
        ...
        GetMeshSceneOverlapPoints(mesh, *sceneBuf, ptMesh, ptScene, NULL);
        scan->setXform(orig);
...
int PlvGlobalRegistrationCmd(ClientData clientData, Tcl_Interp *interp,
                             int argc, char *argv[]) {
    if (argc < 2) {
        Tcl_SetResult(interp, "Bad argument to PlvGlobalRegistrationCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (!strcmp(argv[1], "reset")) {
        ...
        if (argc > 4) {
            ...
            if (scanToMoveTo == NULL) {
                ...
                Tcl_SetResult(interp, "Bad mesh passed to GlobalReg 1->2 align",
                              TCL_STATIC);
                return TCL_ERROR;
            }
        }
        ...
        gr->align(atof(argv[2]), scanToMove, scanToMoveTo);
        ...
        if (!dm1 || !dm2) {
            // Tcl_SetResult(interp, "Bad mesh specified -
            // sczRegCmds.cc:PlvGlobalRegistrationCmd pairstatus", TCL_STATIC);
            //      rihan 7/9/01
        ...
        if (!dm) {
            Tcl_SetResult(interp, "Bad mesh specified - "
                                  "sczRegCmds.cc:PlvGlobalRegistrationCmd "
                                  "listpairsfor",
                          TCL_STATIC);
            return TCL_ERROR;
        }
        ...
        if (argc > 3) {
            ...
            tcl_cmd = gr->dump_meshpairs(atoi(argv[2]), showThresh);
        } else {
        ...
        if (dm2)
            gr->deletePair(dm1->getMeshData(), dm2->getMeshData(), true);
        else
        ...
        if (argc < 4) {
            Tcl_SetResult(
                interp, "Bad args to PlvGlobalRegistrationCmd point_pair_count",
                TCL_STATIC);
            return TCL_ERROR;
        }
        DisplayableMesh *dm1 = FindMeshDisplayInfo(argv[2]);
        ...
        if (!dm1 || (!dm2 && argv[3][0] != '*')) {
            Tcl_SetResult(
                interp, "Bad mesh in PlvGlobalRegistrationCmd point_pair_count",
                TCL_STATIC);
            return TCL_ERROR;
        }
        if (dm2)
            gr->showPointpairCount(dm1->getMeshData(), dm2->getMeshData());
        else
            gr->showPointpairCount(dm1->getMeshData());

    } else if (!strcmp(argv[1], "deleteautopairs")) {
        if (argc < 3 || argc > 4) {
            Tcl_SetResult(
                interp, "Bad args to PlvGlobalRegistrationCmd deleteautopairs",
                TCL_STATIC);
            return TCL_ERROR;
        }
        TbObj *tb = NULL;
        ...
        sprintf(szStats, "%c %d %d %.3f %.3f %.3f %.3f %.3f %4d/%02d/%02d",
                bManual ? 'M' : 'A', iQuality, nPoints, errs[0], errs[1],
                errs[2], errs[3], errs[4], 1900 + pTime->tm_year,
        ...
        if (!dm && 0 != strcmp(argv[2], "*")) {
            Tcl_SetResult(interp, "Bad mesh passed to getstatsummary",
                          TCL_STATIC);
            return TCL_ERROR;
        }
        ...
        if (!gr->getPairingSummary(tb, metric, count, err, qual)) {
            Tcl_SetResult(interp, "Can't get summary for given mesh",
                          TCL_STATIC);
            return TCL_ERROR;
        }

        char szStats[200];
        sprintf(szStats, "%d %d %d %.3f %.3f %.3f %d %d %d %d", count[0],
                count[1], count[2], err[0], err[1], err[2], qual[0], qual[1],
                qual[2], qual[3]);
        Tcl_SetResult(interp, szStats, TCL_VOLATILE);
...
int PlvRegIcpCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                 char *argv[]) {
    if (argc != 14) {
        Tcl_SetResult(interp, "Usage: plv_icpregister \n"
    ...
    float avgError =
        icp.align(atof(argv[1]), atoi(argv[2]), atoi(argv[3]), atoi(argv[4]),
                  argv[6][1] == 'o', argv[9][0] == 'a', atof(argv[10]));
    ...
    if (bSaveForGlobal) {
        ...
        if (ptSrc.size() || ptTrg.size()) {
            cout << "Aligned pairs saved for global registration: "
                 << mdSrc->getName() << " and " << mdTrg->getName() << endl;

            int max_pairs = atoi(argv[12]);
            int quality = atoi(argv[13]);

            theScene->globalReg->addPair(
                mSrc, mTrg, ptSrc, nrmSrc, ptTrg, nrmTrg,
                mTrg->getXform().fast_invert() * mSrc->getXform(), true,
                max_pairs, true, quality);
        }
    }
...
int PlvRegIcpMarkQualityCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                            char *argv[]) {
    if (argc != 4) {
        Tcl_SetResult(interp, "Bad args to PlvRegIcpMarkQualityCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (quality < 0 || quality > 3) {
        Tcl_SetResult(interp, "Quality out of range in PlvRegIcpMarkQualityCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }

    if (!theScene->globalReg->markPairQuality(mSrc, mTrg, quality)) {
        Tcl_SetResult(interp, "General failure in PlvRegIcpMarkQualityCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
int PlvShowIcpLinesCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                       char *argv[]) {
    if (argc < 2) {
        Tcl_SetResult(interp, "Bad arguments to PlvShowIcpLinesCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
int SczAutoRegisterCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                       char *argv[]) {
    if (argc < 7) {
        Tcl_SetResult(
            interp, "Usage: scz_auto_register \n"
                    "\t[visible | current] [visible | all] [name_of_curr_mesh] "
                    "[error_thresh] [nTargetPairs] [bPreserveExisting] "
                    "[bNormSSample]\n",
            TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    // ctor
    AutoICP<RigidScan *> auto_align(theScene->globalReg, final_error_thresh);
    ...
    for (first = begin; first != end; first++) {
        if ((bFromVisible && (*first)->getVisible()) ||
            ...
            for (second = begin; second != end; second++) {
                ...
                if (bToVisible && !(*second)->getVisible())
                    continue;

                if (bFromVisible && (*second)->getVisible() && second < first)
                    continue;
                ...
                if (rs1->num_vertices() == 0 || rs2->num_vertices() == 0) {
                    continue;
                }
    ...
    for (first = begin; first != end; first++) {
        if ((bFromVisible && (*first)->getVisible()) ||
            ...
            for (second = begin; second != end; second++) {
                ...
                bool manual;
                if (theScene->globalReg->pairRegistered(rs1, rs2, manual)) {
                    if (manual) {
