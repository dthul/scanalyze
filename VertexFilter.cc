...
VertexFilter *filterFromSelection(TbObj *ms, const Selection &sel) {
    ...
    if (sel.type == Selection::rect) {
        filter = new ScreenBox(ms, sel[0].x, sel[2].x, sel[0].y, sel[2].y);
...
ScreenBox::ScreenBox(const ScreenBox *source) { *this = *source; }
...
bool ScreenBox::accept(const Pnt3 &crd) const {
    ...
    bool in = (screen[0] <= xmax && screen[0] >= xmin && screen[1] <= ymax &&
               screen[1] >= ymin);
...
bool ScreenBox::accept(const Bbox &bbox) const {
...
#ifdef VERBOSE
    cout << projBbox << endl;
    cout << "Screen bounds: (" << xmin << ", " << ymin << ")  (" << xmax << ", "
         << ymax << ")" << endl;
#endif
    ...
    // this macro works, it actually ends up meaning ACCEPT in the inverted
    // case.
    if (inverted) {
        if (projBbox.min()[0] >= xmin && projBbox.max()[0] <= xmax &&
            projBbox.min()[1] >= ymin && projBbox.max()[1] <= ymax) {
            if (g_verbose)
                cerr << "Wholly within bounding box, rejecting" << endl;
            ACCEPT;
        } else {
            REJECT;
        }
    ...
    if (projBbox.min()[0] >= xmax || projBbox.max()[0] <= xmin ||
        projBbox.min()[1] >= ymax || projBbox.max()[1] <= ymin) {
        REJECT;
    }
    ...
    for (i = 0; i < 6; i++) {
        ...
        face.normal = normal(verts[face.vIndex[0]], verts[face.vIndex[1]],
                             verts[face.vIndex[2]]);
    }
    ...
    for (i = 0; i < numEdges; i++) {
        ...
        silEdgeEq =
            cross(verts[silEdges[i].vIndex[0]], verts[silEdges[i].vIndex[1]]);
...
void ScreenBox::invert(void) { inverted = !inverted; }
...
ScreenPolyLine::~ScreenPolyLine() { delete pixels; }
...
ScreenPolyLine::ScreenPolyLine(const ScreenPolyLine *source)
    ...
    *this = *source;
    ...
    memcpy(pixels, source->pixels, width * height * sizeof(unsigned char));
}
