...
// factory:
class KDindtree *CreateKDindtree(const Pnt3 *pts, const short *nrms, int nPts);
...
class KDindtree {
  private:
    ...
    int _search(const Pnt3 *pts, const short *nrms, const Pnt3 &p,
                const Pnt3 &n, int &ind, float &d) const;
    int _search(const Pnt3 *pts, const Pnt3 &p, int &ind, float &d) const;
  ...
  public:
    ...
    // ind is modified,
    KDindtree(const Pnt3 *pts, const short *nrms, int *ind, int n,
              int first = 1);
    ~KDindtree();

    // use normals
    int search(const Pnt3 *pts, const short *nrms, const Pnt3 &p, const Pnt3 &n,
    ...
    int search(const Pnt3 *pts, const Pnt3 &p, int &ind, float &d) const {
        float _d = d;
        _search(pts, p, ind, d);
        return (d != _d);
    }
