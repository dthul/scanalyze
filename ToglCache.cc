...
DisplayCache::~DisplayCache(void) { Free(); }

bool DisplayCache::Render(void) {
    if (!mEnabled)
        return false;
...
bool DisplayCache::Invalidate(void) {
    mValid = false;
    return true;
}

void DisplayCache::Enable(bool bEnable) { mEnabled = bEnable; }

bool DisplayCache::Allocate(void) {
    Free();
...
void DisplayCache::InvalidateToglCache(struct Togl *togl) {
    DisplayCache *dc = FindToglDC(togl);
    if (dc != NULL)
        dc->Invalidate();
}

void DisplayCache::UpdateToglCache(struct Togl *togl) {
    DisplayCache *dc = FindToglDC(togl);
    if (dc != NULL)
        dc->Update();
}

void DisplayCache::EnableToglCache(struct Togl *togl, bool bEnable) {
    DisplayCache *dc = FindToglDC(togl);
    assert(dc);
...
void DisplayCache::DC_ToglDestroy(struct Togl *togl) {
    DisplayCache *dc = FindToglDC(togl);
    assert(dc);
...
void DisplayCache::DC_ToglReshape(struct Togl *togl) {
    DisplayCache *dc = FindToglDC(togl);
    assert(dc);
...
void DisplayCache::DC_ToglDisplay(struct Togl *togl) {
    DisplayCache *dc = FindToglDC(togl);
    assert(dc);
...
void DisplayCache::AddToglDC(struct Togl *togl, DisplayCache *dc) {
    unsigned long hash = (unsigned long)togl;
    dcHash[hash] = dc;
}

DisplayCache *DisplayCache::FindToglDC(struct Togl *togl) {
    unsigned long hash = (unsigned long)togl;
    return dcHash[hash];
}
