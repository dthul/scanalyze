...
Tk_RestrictAction BailDetector::filterproc(ClientData clientData,
                                           XEvent *eventPtr) {
    if (!s_bBail) {
        if (eventPtr->type == KeyPress) {
            ...
            if ((((XKeyPressedEvent *)eventPtr)->keycode & 0xFF) == 16) {
                /* could also use:
                   ...
                   && ((XMotionEvent*)eventPtr)->state & (Button1Mask |
                   Button2Mask)))

                   to bail out of a render that's already unwanted because
                   you've
                   moved the trackball since it started.
                   but we'd want to be more careful than that, maybe look for a
                   few in
...
int PlvBailDetectCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                     char *argv[]) {
    BailDetector bail;
