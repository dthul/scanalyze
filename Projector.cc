...
Pnt3 Projector::operator()(const Pnt3 &world) const {
    Pnt3 screen = world;
    xfForward(screen);
...
void Projector::xformBy(const Xform<float> &xfRel) {
...
Unprojector::Unprojector(Trackball *tb, TbObj *tbobj) : Projector(tb, tbobj) {
    xfBack = xfForward;
    xfBack.invert();
}

Pnt3 Unprojector::operator()(int x, int y, unsigned int z) const {
...
Pnt3 Unprojector::operator()(int x, int y, float z) const {
    float tx = (2. * x / width) - 1;
    float ty = (2. * y / height) - 1;
...
Pnt3 Unprojector::forward(const Pnt3 &world) const {
    return Projector::operator()(world);
}

void Unprojector::xformBy(const Xform<float> &xfRel) {
    Projector::xformBy(xfRel);

    // BUGBUG suspect code!  needs testing.
    Xform<float> xfRelInv = xfRel;
    xfRelInv.invert();
