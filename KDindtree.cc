...
// factory
KDindtree *CreateKDindtree(const Pnt3 *pts, const short *nrms, int nPts) {
...
static Pnt3 GetNormalAsPnt3(const short *nrms, int ofs) {
    ofs *= 3;
...
static int divisionsort(const Pnt3 *data, int *p, int n, int dim, float med) {
    // move values <= med to left, the rest to right
    int left = 0, right = n - 1;
    while (1) {
        while (data[p[left]][dim] <= med && right > left)
            left++;
        while (data[p[right]][dim] > med && right > left)
            right--;
        int tmp = p[left]; // swap
        p[left] = p[right];
        p[right] = tmp;
        if (left == right) {
            if (data[p[right]][dim] <= med)
                right++;
            break;
        }
        left++;
...
void merge_normal_cones(const Pnt3 &n1, float th1, const Pnt3 &n2, float th2,
                        Pnt3 &n3, float &th3) {
    ...
    qim *= sin(half_ang);
    // apply rotation
    (((n3 = n1) *= (qr *qr - qim.norm2())) += (cross(qim, n1) *= (2.0 *qr))) +=
        (qim *= (2.0 * dot(n1, qim)));
    // n3 = (qr*qr-qim.norm2())*n1 + (2*dot(n1,qim))*qim
    //  + 2*qr*cross(qim,n1);
}

KDindtree::KDindtree(const Pnt3 *pts, const short *nrms, int *ind, int n,
                     int first)
    ...
    if (dist == 0.0)
        n = 1; // a single point several times
...
#undef DATA
...
#if MEDIAN_SPLIT
    if (first)
        med.zap(); // release memory after the tree's done
#endif
    ...
    if (Nhere) {
        ...
        theta = 0.0;
        for (i = 1; i < Nhere; i++) {
            merge_normal_cones(normal, theta, GetNormalAsPnt3(nrms, element[i]),
                               0, normal, theta);
        }
    } else {
        // a non-terminal node
        merge_normal_cones(child[0]->normal, child[0]->theta, child[1]->normal,
                           child[1]->theta, normal, theta);
    }
    tmp = theta + M_PI * .25;
    if (tmp > M_PI)
        cos_th_p_pi_over_4 = -1.0;
    else
        cos_th_p_pi_over_4 = cos(tmp);
}
...
int KDindtree::_search(const Pnt3 *pts, const short *nrms, const Pnt3 &p,
                       const Pnt3 &n, int &ind, float &d) const {
    assert(this);
...
int KDindtree::_search(const Pnt3 *pts, const Pnt3 &p, int &ind,
                       float &d) const {
    assert(this);
