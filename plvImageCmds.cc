...
int PlvWriteIrisCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                    char *argv[]) {
    ...
    if (!img.write(argv[2])) {
        ...
        // Linux
        Tcl_SetResult(interp, "Image saving not supported under Linux",
                      TCL_STATIC);
#else
...
int PlvCacheBufferCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                      char *argv[]) {
    GLint lastBuffer;
    ...
    glReadPixels(0, 0, theWidth, theHeight, GL_RGBA, GL_UNSIGNED_BYTE,
                 theColorBuf);
    glReadPixels(0, 0, theWidth, theHeight, GL_DEPTH_COMPONENT, GL_UNSIGNED_INT,
                 theZBuf);
...
int PlvReloadBufferCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                       char *argv[]) {
    GLint lastBuffer;
    ...
    glGetIntegerv(GL_DRAW_BUFFER, &lastBuffer);
    glDrawBuffer(GL_FRONT);
    glDrawPixels(theWidth, theHeight, GL_RGBA, GL_UNSIGNED_BYTE, theColorBuf);
    glDrawPixels(theWidth, theHeight, GL_DEPTH_COMPONENT, GL_INT, theZBuf);
    glDrawBuffer(GLenum(lastBuffer));
...
int countPixInRange(int x, int y, int w, int h, int lowR, int lowG, int lowB,
    ...
    for (int i = 0; i < w * h; i = i + 4) {
        ...
        if (p.r >= lowR && p.r <= highR && p.g >= lowG && p.g <= highG &&
            p.b >= lowB && p.b <= highB) {
            count++;
        }
    }
...
int PlvCountPixelsCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                      char *argv[]) {
    int x, w, y, h;
    ...
    if (argc == 4 || argc == 7) {
        ...
        strcat(
            buf,
            "Usage: plv_countPixels RGB_min [RGB_max] [Bounding_Rect_LTRB]\n");
        strcat(buf, "  plv_countPixels 0 0 0 - counts all black pixels in "
                    "current selection\n");
        strcat(buf, "  plv_countPixels 0 0 200 255 255 255 - counts pixels "
                    "with high blue values\n");
        strcat(buf, "  plv_countPixels 0 0 0 0 0 0 100 150 200 200 - counts "
                    "black pixels in the\n    rect bounded by (100,150) and "
                    "(200,200).\n");
        strcat(buf, "\n");
