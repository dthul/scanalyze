...
template <class T> class ICP : public DrawObj {
  public:
    bool allow_bdry;
  ...
  private:
    ...
    float cull_pairs(int p, // percentage to cull, [0,100]
        ...
        med.set_percentage((100 - p) / 100.0);
        med.clear();
        int n = pP.size();
        vector<double> d(n);
    ...
    void find_pairs(float thr) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            xfP.apply(ssP[i], wp);
            xfP.apply_nrm(ssPn[i], wn); // world
            xfi.apply(wp, lp);
            xfi.apply_nrm(wn, ln); // local in Q
        ...
        for (int i = 0; i < n; i++) {
            // point to other mesh's coords
            xfQ.apply(ssQ[i], wp);
            xfQ.apply_nrm(ssQn[i], wn); // world
            xfi.apply(wp, lp);
            xfi.apply_nrm(wn, ln); // local in P
  ...
  public:
    ICP(void) : allow_bdry(0), firstend(0) { draw_other_things.add(this); }

    ~ICP(void) { draw_other_things.remove(this); }
    ...
    void sort_into_buckets(const vector<Pnt3> &n,
        ...
        for (int bucket = 0; bucket < normbuckets.size(); bucket++)
            random_shuffle(normbuckets[bucket].begin(),
                           normbuckets[bucket].end());
    }

    float align(float sample_rate, bool normspace_sample, int n_iter,
        ...
        if (normspace_sample) {
            // Sampling that tries to be somewhat uniform in the space of
            // normals
    ...
    bool auto_align(vector<Pnt3> &_pP, vector<Pnt3> &_nP, vector<Pnt3> &_pQ,
                    vector<Pnt3> &_nQ, Xform<float> &rel_xf,
                    float final_abs_thresh = FLT_MAX, // mm
                    float max_motion = FLT_MAX, bool normspace_sample = false) {
        ...
        if (max_motion < FLT_MAX) {
            ...
            for (vector<Pnt3>::const_iterator it = ssP.begin(); it != ssP.end();
                 it++) {
                Pnt3 p = *it;
                xf(p);
                d += dist(*it, p);
    ...
    void get_pairs_for_global(vector<Pnt3> &_pP, vector<Pnt3> &_nP,
                              vector<Pnt3> &_pQ, vector<Pnt3> &_nQ) {
        _pP = pP;
        _nP = nP;
        Xform<float> xfi = xfP;
        xfi.fast_invert();
...
template <class T> class AutoICP {
  ...
  public:
    ...
    AutoICP(GlobalReg *pGlobalReg = NULL, float _abs_threshold = FLT_MAX,
    ...
    bool add_pair(T a, T b, int max_pairs = 0, bool nss = false) {
        icp.set(a, b);
        Xform<float> rel_xf;
        bool success = icp.auto_align(pP, nP, pQ, nQ, rel_xf,
                                      final_abs_threshold, FLT_MAX, nss);

        if (success) {
            regall->addPair(a, b, pP, nP, pQ, nQ, rel_xf, false, max_pairs);
        }

        return success;
    }

    void operator()(void) { regall->align(1.e-3); }
};
