...
static bool getXformFilename(const char *meshName, char *xfName) {
...
void MMScan::absorb_xforms(char *basename) {
    ...
    for (int i = 0; i < scans.size(); i++) {
        char temp[15];
        if (i < 10)
            sprintf(temp, "_00%d.xf", i);
        else if (i < 100)
            sprintf(temp, "_0%d.xf", i);
        else
            sprintf(temp, "_%d.xf", i);
        strcpy(pExt, temp);
        TbObj tempObj;
        Xform<float> newXF;
...
int MMScan::write_xform_ply(int whichRes, char *directory, string &confLines,
                            bool applyXform) {
    ...
    */
    ...
    for (int i = 0; i < scans.size(); i++) {
        ...
        if (scans[i].stripes.size() < 3) {
            ...
            if (i < 10)
                sprintf(pExt, "_00%d.ply", i);
            else if (i < 100)
                sprintf(pExt, "_0%d.ply", i);
            else
                sprintf(pExt, "_%d.ply", i);
	 ...
	 scans[i].viewDir = -scans[i].viewDir;
            ...
            // cout << "rotating " << rotAngle << " rads around " << axis <<
            // endl;
            ...
            if (applyXform) {
                ...
                sprintf(line, "{bmesh %s %g %g %g %g %g %g %g}", buffer,
                        comboTrans[0], comboTrans[1], comboTrans[2],
            ...
            write_ply_file(full_path, scanVtx, scans[i].meshes[whichRes].tris,
                           false);
            scanVtx.clear();
            ...
            if (xffile.fail()) {
                cerr << "Scan " << buffer << " failed to write xform!" << endl;
                break;
            }
...
MeshTransport *MMScan::mesh(bool perVertex, bool stripped, ColorSource color,
                            int colorSize) {
    ...
    if (!resolutions[resNum].in_memory)
        load_resolution(resNum);
    ...
    for (int i = 0; i < scans.size(); i++) {
        if (scans[i].isVisible) {
            ...
            if (perVertex) {
                ...
                /* no support now
                if (!res->triNrm.size()) {
                  if (!res->tris.size()) strips_to_tris(res->tstrips,
                res->tris);
...
void MMScan::setMTColor(MeshTransport *mt, int iScan, bool perVertex,
...
int MMScan::num_tris(int res) {
    if (scans.size() == 0)
        return 0;
    int numResolutions = scans[0].meshes.size();
    if (numResolutions == 0)
        return 0;
    if ((res >= numResolutions) || (res < 0))
        return 0;
...
int MMScan::num_vertices(int resNum) {
    int total = 0;
...
int MMScan::create_resolution_absolute(int budget, Decimator dec) {
    if (budget == 0)
        return 0;
...
void MMScan::flipNormals(void) {
    for (int i = 0; i < scans.size(); i++) {
        for (int j = 0; j < scans[i].meshes.size(); j++) {
            // triangle normals
            for (int k = 0; k < scans[i].meshes[j].tris.size(); k += 3) {
                swap(scans[i].meshes[j].tris[k],
                     scans[i].meshes[j].tris[k + 1]);
            }
            // tstrip normals
        ...
        for (int j = 0; j < scans[i].stripes.size(); j++) {
            ...
            if (scans[i].stripes[j].scanDir == 1)
                scans[i].stripes[j].scanDir = 2;
            else
                scans[i].stripes[j].scanDir = 1;
        }
        scans[i].viewDir *= -1;
    }
}

void MMScan::flipNormals(int scanNum) {
    ...
    for (int j = 0; j < scans[scanNum].stripes.size(); j++) {
        ...
        if (scans[scanNum].stripes[j].scanDir == 1)
            ...
            scans[scanNum].stripes[j].scanDir = 1;
    }
    scans[scanNum].viewDir *= -1;
}

bool MMScan::isValidPoint(int scanNum, int stripeNum, int colNum)
// returns value of bitmap of specified stripe in specified column
// of a specified scan
{
    if (scanNum >= scans.size())
        return false;
    if (stripeNum >= scans[scanNum].stripes.size())
        return false;
    if (colNum > STRIPE_PTS)
        return false;
    return (scans[scanNum].stripes[stripeNum].validMap[colNum / 8] &
            (1 << (colNum % 8)));
}

static void addTri(vector<int> &tris, int v1, int v2, int v3, char scanDir) {
        ...
        for (j = 0; j < scans[k].stripes.size() - 1; j++) {
            ...
            for (i = 0; i < STRIPE_PTS - 1; i++) {
                ...
                if (isValidPoint(k, j, i) && isValidPoint(k, j + 1, i)) {
                    scanDir +=
                        mesh->vtx[scan->indices[i + (j + 1) * STRIPE_PTS]];
            ...
            if (numPts == 0) {
                ...
                // if the actual doesn't match expected, we need to flip the
                // triangles
                // by marking the scanDir flag as 2 instead of 1
                if (dot(crossDir, scanDir) > 0)
                    scan->stripes[j].scanDir = 1;
                else
                    scan->stripes[j].scanDir = 2;
            }
        }
...
bool MMScan::load_resolution(int iRes) {
    if (resolutions[iRes].in_memory)
        return true;

    int subSamp = 1 << iRes;

    Progress progress(scans.size(), "%s: build mesh", name.c_str());
...
void MMScan::triangulate(mmScanFrag *scan, mmResLevel *res, int subSamp) {
    int i, j, ii, jj;
    ...
    if (subSamp != 1) {
        ...
        if (!alreadySubSampled) {
            ...
            res->confidence.reserve(scan->meshes[0].confidence.size() /
                                    subSamp);
                ...
                if (in1 >= 0) {
                    ...
                    if (!alreadySubSampled) {
                        ...
                        if (scan->meshes[0].confidence.size())
                            res->confidence.push_back(
                                scan->meshes[0].confidence[in1]);
                        if (scan->meshes[0].intensity.size())
                            res->intensity.push_back(
                                scan->meshes[0].intensity[in1]);
                    }
                    count++;
                }
            }
        ...
        for (i = 0; i < STRIPE_PTS - subSamp; i += subSamp) {
            ...
            if (count == 4) { // all 4 vertices okay, so make 2 tris
                ...
                if (dist2(res->vtx[vin1], res->vtx[vin4]) > maxLen1)
                    badTri = true;
                // check to make sure that inter-stripe distances aren't too
                // long
                if (dist2(res->vtx[vin1], res->vtx[vin2]) > maxLen2)
                    badTri = true;
                if (dist2(res->vtx[vin3], res->vtx[vin4]) > maxLen2)
                    badTri = true;
                ...
                if (!badTri) {
                    ...
                    if (len1 < len2) {
                        addTri(res->tris, vin2, vin1, vin3,
                               scan->stripes[j].scanDir);
                        addTri(res->tris, vin1, vin4, vin3,
                               scan->stripes[j].scanDir);
                    } else {
                        addTri(res->tris, vin2, vin1, vin4,
                               scan->stripes[j].scanDir);
                        addTri(res->tris, vin2, vin4, vin3,
                               scan->stripes[j].scanDir);
                    }
                }
...
void MMScan::make_tstrips(mmScanFrag *scan, mmResLevel *res, int subSamp) {
    int i, j, ii, jj;
    int in1, in2, in3, in4;
            ...
            if (count == 4) { // all 4 vertices okay, so make 2 tris
                ...
                if (badTri) {
                    ...
                    if (len1 < len2) {
                        ...
                        // (meaning diagonal lies between 1 and 3, instead of 2
                        // and 4)

                        if (!(whichDiag || newStrip)) {
                            // if we're in the middle of a reverse-diagonal
                            // strip...
                            // end the strip
                            res->tstrips.push_back(-1);
                            newStrip = true;
                        ...
                        // if we're starting a new strip or ending a
                        // normal-diagonal,
                        // we add a single triangle and begin again
                        if (whichDiag || newStrip) {
                            if (newStrip) {
                                // 1st 2 vertices of triangle needed for new
                                // strip
                                res->tstrips.push_back(vin2);
                                res->tstrips.push_back(vin1);
                            }
                            res->tstrips.push_back(vin4);
                            res->tstrips.push_back(-1);
                            // end the strip and the triangle, start the new
                            // strip
                            res->tstrips.push_back(vin2);
                            numStrips++;
                            newStrip = false;
                ...
                // we need to end any current strips because there is going to
                // be
...
void MMScan::subsample_points(float rate, vector<Pnt3> &p, vector<Pnt3> &n) {
...
MMScan::mergedRegData &MMScan::getRegData(void) {
    mergedRegData &reg = regData[curr_res];
    if (!isDirty_mem && (reg.vtx.size() > 0))
        return reg;

    cout << "Making new global vertex array" << endl;
    int numVerts = num_vertices(curr_res);
    reg.vtx.clear();
    reg.vtx.reserve(numVerts);
    reg.nrm.clear();
    reg.nrm.reserve(numVerts * 3);
    reg.tris.clear();
    reg.tris.reserve(num_tris(curr_res) * 3);
...
void MMScan::mark_boundary(mergedRegData &reg) {
    if (!isDirty_mem && (reg.boundary.size() > 0))
        return;
    reg.boundary.clear();
...
KDindtree *MMScan::get_kdtree() {
    ...
    delete kdtree[curr_res];
    kdtree[curr_res] =
        CreateKDindtree(reg.vtx.data(), reg.nrm.data(), reg.vtx.size());
...
bool MMScan::closest_point(const Pnt3 &p, const Pnt3 &n, Pnt3 &cp, Pnt3 &cn,
...
void MMScan::update_res_ctrl() {
    for (int i = 0; i < resolutions.size(); i++) {
        resolutions[i].abs_resolution = num_tris(i);
    }
}

bool MMScan::filter_inplace(const VertexFilter &filter) {
    int i, j, k;
    ...
    for (j = 0; j < scans.size(); j++) {
        ...
        if (scan->isVisible) {
            ...
            if (scan->stripes.size() == 0) {
                ...
                for (res = scan->meshes.begin(); res != scan->meshes.end();
                     res++) {
                    ...
                    if (count == 0) {
                        // no vertices made it... only applicable in lower res's
                        // i guess
                        cout << "count equalled 0; scan bites the dust "
                             << endl;
                    ...
                    // delete the tstrips from the res since they are hard to
                    // clip
                    res->tstrips.clear();
...
RigidScan *MMScan::filtered_copy(const VertexFilter &filter) {
    int i, j, k;
        ...
        if (scans[j].isVisible) {
            ...
            if (newScan.stripes.size() > 0) {
                ...
                for (k = 0; k < scans[j].meshes.size(); k++) {
                    ...
                    // #3a: figure out which verts make the cut, except on the
                    // first
                    ...
                    for (i = 0; i < oldRes->vtx.size(); i++) {
                        if (newIndices[i] > 0) {
                            ...
                            if (oldRes->confidence.size())
                                newRes.confidence.push_back(
                                    oldRes->confidence[i]);
                            if (oldRes->intensity.size())
                                newRes.intensity.push_back(
                                    oldRes->intensity[i]);
                    ...
                    for (i = 0; i < oldRes->tris.size(); i += 3) {
                        if (newIndices[oldRes->tris[i]] >= 0 &&
                            ...
                            newRes.tris[count + 1] =
                                newIndices[oldRes->tris[i + 1]];
                            newRes.tris[count + 2] =
                                newIndices[oldRes->tris[i + 2]];
...
// of the file...
bool MMScan::filter_vertices(const VertexFilter &filter, vector<Pnt3> &p) {
    ...
    for (vector<mmScanFrag>::iterator curScanFrag = scans.begin();
        ...
        for (vector<Pnt3>::iterator pnt = curPtVector->begin(); pnt < vtxend;
             pnt++) {
...
void MMScan::calcConfidence() {
    ...
    for (int k = 0; k < scans.size(); k++) {
        ...
        for (i = 0; i < scans[k].num_vertices(); i++) {
          float conf = 1.0;
          if (boundPlus[i] > 0)
            conf = (1.0 - FEARANGE) + (boundPlus[i] / (float)FEASTEPS) *
        FEARANGE;
...
static void remove_unused_vtxs(vector<Pnt3> &vtx, vector<int> &tri,
                               vector<int> &vtx_ind) {
...
static void CopyReverseWord(void *dest, void *src, int nWords)
...
static void ReversePoints(MMSio *points, int numPoints) {
...
static void ReversePoints(mmIO *points, int numPoints) {
...
static void triSet(float *dest, Pnt3 *src)
...
bool MMScan::stripeCompare(int strNum, mmScanFrag *scan) {
    if (strNum < 0)
        return false;
    ...
    if (abs(str1->numPoints - str2->numPoints) > 5)
        return false;
    ...
    if (numPts < (0.95 * str1->numPoints))
        return false;

    avgDist /= numPts;

    if ((avgDist < 0.1) && (numLargeDist < 5))
        return true;
    return false;
}
...
bool MMScan::read_mms(const string &fname) {
    mmStripeInfo stripe;
    mmScanFrag *scan;
    ...
    if ((strcmp(szFileID, "BinVer1.0") == 0) ||
        ...
        for (int i = 6; i < 9; i++) {
            ...
            default:
                cerr << endl
                     << "Unrecognized flag in MMScan string: " << szFileID[i]
                     << endl;
                return false;
            }
        }
    ...
    cout << "Reading " << numPoints << " points, " << numStripes << " stripes, "
         << numScans << " scans." << endl;
    ...
    if (stripes == NULL) {
        cerr << "Couldn't allocate enough memory for " << numStripes
             << " stripes" << endl;
        return false;
    }
    ...
    for (i = 0; i < numStripes; i++) {
        if (stripes[i].numPoints > 0) {
            ...
            // reserve geometry and intensity space for our soon-to-arrive
            // points
            ...
            // shouldn't ever be here if filtered properly... no points in
            // stripe
    ...
    if (!points) {
        cerr << "Couldn't allocate memory for " << numPoints << " points."
             << endl;
        return false;
    }
...
bool MMScan::read_cta(const string &fname) {
    ...
    bool doFilter = true;
    char *result = Tcl_GetVar(theScene->interp, "noMMFilter", TCL_GLOBAL_ONLY);
    ...
    if (numSkipped)
        cout << "SKIPPED " << numSkipped << "STRIPES while loading." << endl;
    return true;
}

bool MMScan::read(const string &fname) {
    set_name(fname);
...
bool MMScan::write(const string &fname) {
    ...
    for (i = 0; i < numScans; i++) {
        for (j = 0; j < scans[i].stripes.size(); j++, numSoFar++) {
            ...
            for (k = 0; k < 3; k++)
                stripes[numSoFar].sensorVec[k] =
                    scans[i].stripes[j].sensorVec[k];
