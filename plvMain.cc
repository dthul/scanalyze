...
static bool isReadableFile(char *name) {
    ...
    if (filename_has_ending(name, ".sd") || filename_has_ending(name, ".sd.gz"))
        return true;
...
int main(int argc, char **argv) {
    ...
    // probably won't hurt, can't think of any reason to have escaped wildcards
    // that
...
int Tcl_AppInit(Tcl_Interp *interp) {
    ...
    if (filenames.size()) {
        ...
        for (int i = 0; i < filenames.size(); i++) {
            ...
            if (strncmp(filenames[i] + strlen(filenames[i]) - 3, ".gp", 3) ==
                0) {
