...
class CubeTreeBase {
  ...
  public:
    ...
    int boundary() const { return type == BOUNDARY || type == SILHOUETTE; }
    int go_on() const {
        return type == BOUNDARY || type == SILHOUETTE || type == INDETERMINATE;
    }

    // spatial info
    inline const Pnt3 &ctr(void) const { return mCtr; }
    inline const float &size(void) const { return mSize; }
...
class CubeTree : public CubeTreeBase {
  ...
  private:
    ...
    CubeTreeBase *recursive_find(int face, stack<int> &path, bool bReachedTop);
  ...
  protected:
    ...
    void carve_help(vector<RigidScan *> &views, int levels,
                    vector<int> &tri_inds, float perc_min, float perc_max);

  public:
    CubeTree(Pnt3 c, float s, CubeTree *parent = NULL, int iParIdx = -1);
    ~CubeTree(void);

    void carve(vector<RigidScan *> &views, int levels, vector<Pnt3> &coords,
               vector<int> &tri_inds);
};
...
class CubeTreeLeaf : public CubeTreeBase {
  public:
    CubeTreeLeaf(const Pnt3 &c, float s, CubeTree *parent, int iParIdx,
