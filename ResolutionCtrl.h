...
class ResolutionCtrl {
  public:
    struct res_info {
        ...
        res_info(void)
            : abs_resolution(0), in_memory(false), desired_in_mem(true) {}

        friend bool
        operator<(const res_info &r1,
                  const res_info &r2) { // descending sort by number of vertices
  ...
  protected:
    ...
    void insert_resolution(int abs, string filename, bool in_mem = true,
