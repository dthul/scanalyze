...
static void quaternion2matrix(double *q, double m[3][3]) {
    double q00 = q[0] * q[0];
    double q11 = q[1] * q[1];
    double q22 = q[2] * q[2];
    double q33 = q[3] * q[3];
...
static void characteristicPol(double Q[4][4], double c[4]) {
    // squares
    ...
    // b
    c[1] = -q01_2 - q02_2 - q03_2 + q0011 - q12_2 - q13_2 + q0022 + q1122 -
           q23_2 + q0033 + q1133 + q2233;
...
void maxEigenVector(double Q[4][4], double ev[4]) {
    ...
    */
    ...
    double len = best[0] * best[0] + best[1] * best[1] + best[2] * best[2] +
                 best[3] * best[3];
    for (int i = 1; i < 4; i++) {
        curr = 0;
        curr[i] = 1;
        LU_solve(N, ipiv, curr);
        double tlen = curr[0] * curr[0] + curr[1] * curr[1] +
...
// model
void horn_align(Pnt3 *p,     // the model points  (source)
                ...
                double q[7]) // the reg. vector 0..3 rot, 4..6 trans
{
    ...
    q[4] = x_mean[0] - m[0][0] * p_mean[0] - m[0][1] * p_mean[1] -
           m[0][2] * p_mean[2];
    q[5] = x_mean[1] - m[1][0] * p_mean[0] - m[1][1] * p_mean[1] -
           m[1][2] * p_mean[2];
    q[6] = x_mean[2] - m[2][0] * p_mean[0] - m[2][1] * p_mean[1] -
           m[2][2] * p_mean[2];
}

/**************************************************/
static int qudrtc(double b, double c, double rts[4])
/*
...
static void get_transform(double correction[6], double m[3][3], double t[3]) {
...
static bool cholesky_solve(double A[6][6], double b[6], double x[6]) {
    int i, j, k;
    double sum;
...
void chen_medioni(Pnt3 *ctr,   // control points (source)
                  ...
                  double q[7]) // registration quaternion
{
    ...
    for (int i = 0; i < 6; i++) {
        HtH[i][0] = HtH[i][1] = HtH[i][2] = HtH[i][3] = HtH[i][4] = HtH[i][5] =
            HtP[i] = 0;
    }
    ...
    cout << "Sqrt of average squared error before transform " << sqrtf(sum / n)
         << endl;
