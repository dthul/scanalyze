...
class Median {
    ...
    void divide(void) {
        ...
        while (left < right) {
            ...
            while (inleft <= inright) {
                while (data[inleft] < curr_med)
                    inleft++;
                while (data[inright] > curr_med)
                    inright--;
                if (inleft <= inright) {
                    temp = data[inleft];
  ...
  public:
    ...
    void set_percentage(float p) { perc = p; }
    void zap(void) {
        delete[] data;
        data = new T[size = 256];
        assert(data != NULL);
    ...
    T avrg_sum_until_median(void) { return sum_until_median() / T(div_point); }
