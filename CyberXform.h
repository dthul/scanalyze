...
class CyberXform {
  ...
  public:
    ...
    CyberXform(unsigned config, float hor_trans, float screw,
               float vert_trans = 0) {
        setup((config & 0x00000001), (config & 0x00000002) >> 1,
              (config & 0x0000000c) >> 2, (config & 0x00000010) >> 4, hor_trans,
              screw, vert_trans);
    }
    CyberXform(bool vscan, bool tup, int tconf, bool tright, float hor_trans,
               float screw, float vert_trans = 0) {
        setup(vscan, tup, tconf, tright, hor_trans, screw, vert_trans);
    }

    void setup(bool vscan, bool tup, int tconf, bool tright, float hor_trans,
               float screw, float vert_trans = 0);
    ...
    bool back_project(const Pnt3 &p_in, Pnt3 &p_out, bool check_frustum = true);

    int sphere_status(const Pnt3 &ctr, float r, float &screw, float ax_min,
                      float ax_max, short ywin[2], short zwin[2],
                      float swin[2]);
