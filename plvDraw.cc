...
static bool setupViewing(bool bFirstPass) {
    ...
    if (gRP->antiAlias) {
        if (!tbView->getOrthographic()) {
            ...
            // and b) it draws the whole scene with the same blurriness,
            // regardless
            // of how far individual objects are from camera.

            accFrustum(left, right, top, bottom, znear, zfar, gRP->jitterX,
                       gRP->jitterY, gRP->jitterX * gRP->dofJitterX,
                       gRP->jitterY * gRP->dofJitterY, zFocus);
        } else {
            // orthographic
            // have to modify projection matrix with jitter code
            float heightAngle, aspectRatio, nearDistance, farDistance;
            tbView->getProjection(heightAngle, aspectRatio, nearDistance,
                                  farDistance);

            float cameraHeight = tbView->getOrthoHeight();
            glTranslatef(gRP->jitterX * cameraHeight * aspectRatio / theWidth,
                         gRP->jitterY * cameraHeight / theHeight, 0.0);
        }
    }

    // Prepare viewing orientation/trans
    if (bFirstPass)
        tbView->applyXform(); // apply spin()
    else
        tbView->reapplyXform(); // no spin
...
static void setupShading() {
    ...
    // theRenderParams->backFaceEmissive);
...
bool setupSceneDrawing() {
    ...
    if (theRenderParams->accelerateWithBbox) {
        ...
        ScreenBox *filter = new ScreenBox(NULL, 0, Togl_Width(toglCurrent) - 1,
...
static void initTextureParmsForShadowMap(void) {
...
static bool drawMeshesFromLightView(void) {
    ...
    if (theRenderParams->fromLightPOV || !GetTclGlobalBool("shadowFastPass")) {
        drawMeshes();
    ...
    float sceneCenter[3];
    tbView->getRotationCenter(sceneCenter);
    float cameraCenter[3];
    tbView->getCameraCenter(cameraCenter);
    ...
    if (!findZBufferNeighborExt(curWidth / 2, curHeight / 2, objectCenter,
                                toglCurrent, tbView, 100, NULL, false,
                                bReadFromFront)) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        Pnt3 corner;
        if (findZBufferNeighborExt(x, y, corner, toglCurrent, tbView, 100, NULL,
                                   false, bReadFromFront)) {
    ...
    glOrtho(-orthoHeight / 2 * aspect, orthoHeight / 2 * aspect,
            -orthoHeight / 2, orthoHeight / 2, -halfScene, +halfScene);
...
static bool generateShadowMap(void) {
#ifdef GL_SGIX_shadow
    int x, y;
    ...
    if (ret) {
        ...
        glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT16_SGIX, 0, 0, x,
                         y, 0);
        dumpGLerror();
    }
...
bool drawSceneIndexColored(int scale = 1) {
    PushRenderParams();
    theRenderParams->polyMode = GL_FILL;
    theRenderParams->hiddenLine = false;
...
static bool drawMeshes(bool bDrawAnnotations) {
    int k;
...
static void glvtx(const Pnt3 &p) { glVertex3fv(&p[0]); }

void drawBoundingBox(const Bbox &box, bool bUseDefaultColor) {
...
void PushRenderParams(void) { renderParamsStack.push_back(*theRenderParams); }
