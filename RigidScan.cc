...
bool RigidScan::render_self(ColorSource color) { return false; }

int RigidScan::num_vertices(void) { return 0; }

void RigidScan::subsample_points(float rate, vector<Pnt3> &p, vector<Pnt3> &n) {
}

void RigidScan::flipNormals(void) {}

void RigidScan::computeBBox(void) {}

RigidScan *RigidScan::filtered_copy(const VertexFilter &filter) { return NULL; }

bool RigidScan::filter_vertices(const VertexFilter &filter, vector<Pnt3> &p) {
    return false;
}

bool RigidScan::filter_inplace(const VertexFilter &filter) { return false; }

string RigidScan::getInfo(void) {
    float q[4], t[3];
    ...
    sprintf(info, "filename: %s\n"
                  ...
                  "  Y: %.3f to %.3f\n"
                  "  Z: %.3f to %.3f",
            get_name().c_str(), t[0], t[1], t[2], q[0], q[1], q[2], q[3],
            wb.min()[0], wb.max()[0], wb.min()[1], wb.max()[1], wb.min()[2],
            wb.max()[2]);

    return string(info);
}

OccSt RigidScan::carve_cube(const Pnt3 &ctr, float side) {
    return INDETERMINATE;
}

OccSt RigidScan::carve_sphere(const Pnt3 &ctr, float radius) {
    return INDETERMINATE;
}

bool RigidScan::closest_point(const Pnt3 &p, const Pnt3 &n, Pnt3 &cl_pnt,
                              Pnt3 &cl_nrm, float thr, bool brdy_ok) {
    return 0;
}
...
float RigidScan::closest_point_on_mesh(const Pnt3 &p, Pnt3 &cl_pnt,
                                       OccSt &status_p) {
    return 0.0f;
}

float RigidScan::closest_along_line_of_sight(const Pnt3 &p, Pnt3 &cp,
                                             OccSt &status_p) {
    return 0.0f;
}

float RigidScan::closest_along_line(const Pnt3 &p, const Pnt3 &dir, Pnt3 &cp,
                                    OccSt &status_p) {
    return 0.0f;
}

float RigidScan::color_along_line_of_sight(const Pnt3 &p, float rgb[3]) {
    return 0.0f;
}

float RigidScan::color_along_line_of_sight(const Pnt3 &p, uchar rgb[3]) {
    return 0.0f;
}

bool RigidScan::read(const string &fname) { return false; }

bool RigidScan::is_modified(void) { return false; }

bool RigidScan::write(const string &fname) { return false; }

bool RigidScan::write_resolution_mesh(int nPolys, const string &fname,
                                      Xform<float> xfBy) {
    ...
    if (mt) {
        ...
        if (mt->vtx.size() == 1 && xfBy.isIdentity() &&
            ...
            if (mt->color.size() == 1 &&
                ...
                cerr << "(in color)... " << flush;
                write_ply_file(fname.c_str(), *(mt->vtx[0]), *(mt->tri_inds[0]),
                               bStrips, *(mt->color[0]));
            } else {
                write_ply_file(fname.c_str(), *(mt->vtx[0]), *(mt->tri_inds[0]),
                               bStrips);
            ...
            for (int i = 0; i < mt->vtx.size(); i++) {
                ...
                tris.insert(tend, mt->tri_inds[i]->begin(),
                            mt->tri_inds[i]->end());
                ...
                if (i < mt->color.size() && mt->color[i])
                    color.insert(color.end(), mt->color[i]->begin(),
                                 mt->color[i]->end());

                Xform<float> xfThis = xfBy * mt->xf[i];
                if (!xfThis.isIdentity()) {
                    cerr << "applying xform... " << flush;
                    for (vector<Pnt3>::iterator pi = newVtxStart;
                         pi < vtx.end(); pi++) {
                        xfThis(*pi);
                    }
                }
...
bool RigidScan::write_metadata(MetaData data) {
    bool success = false;
...
bool RigidScan::get_children(vector<RigidScan *> &children) const {
    return false;
}
