...
#if defined(WIN32)
    HDC
    tglGLHdc; /* Device context of device that OpenGL calls will be drawn on */
 ...
 */
static int Togl_Cmd(ClientData clientData, Tcl_Interp *interp, int argc,
                    char **argv);
...
static Tk_ConfigSpec configSpecs[] = {
    { TK_CONFIG_PIXELS, "-height", "height", "Height", DEFAULT_HEIGHT,
      Tk_Offset(struct Togl, Height), 0, NULL },
    { TK_CONFIG_PIXELS, "-width", "width", "Width", DEFAULT_WIDTH,
      Tk_Offset(struct Togl, Width), 0, NULL },
    { TK_CONFIG_BOOLEAN, "-rgba", "rgba", "Rgba", "true",
      Tk_Offset(struct Togl, RgbaFlag), 0, NULL },
    { TK_CONFIG_INT, "-redsize", "redsize", "RedSize", "1",
      Tk_Offset(struct Togl, RgbaRed), 0, NULL },
    { TK_CONFIG_INT, "-greensize", "greensize", "GreenSize", "1",
      Tk_Offset(struct Togl, RgbaGreen), 0, NULL },
    { TK_CONFIG_INT, "-bluesize", "bluesize", "BlueSize", "1",
      Tk_Offset(struct Togl, RgbaBlue), 0, NULL },
    { TK_CONFIG_BOOLEAN, "-double", "double", "Double", "false",
      Tk_Offset(struct Togl, DoubleFlag), 0, NULL },
    { TK_CONFIG_BOOLEAN, "-depth", "depth", "Depth", "false",
      Tk_Offset(struct Togl, DepthFlag), 0, NULL },
    { TK_CONFIG_INT, "-depthsize", "depthsize", "DepthSize", "1",
      Tk_Offset(struct Togl, DepthSize), 0, NULL },
    { TK_CONFIG_BOOLEAN, "-accum", "accum", "Accum", "false",
      Tk_Offset(struct Togl, AccumFlag), 0, NULL },
    { TK_CONFIG_INT, "-accumredsize", "accumredsize", "AccumRedSize", "1",
      Tk_Offset(struct Togl, AccumRed), 0, NULL },
    { TK_CONFIG_INT, "-accumgreensize", "accumgreensize", "AccumGreenSize", "1",
      Tk_Offset(struct Togl, AccumGreen), 0, NULL },
    { TK_CONFIG_INT, "-accumbluesize", "accumbluesize", "AccumBlueSize", "1",
      Tk_Offset(struct Togl, AccumBlue), 0, NULL },
    { TK_CONFIG_INT, "-accumalphasize", "accumalphasize", "AccumAlphaSize", "1",
      Tk_Offset(struct Togl, AccumAlpha), 0, NULL },
    { TK_CONFIG_BOOLEAN, "-alpha", "alpha", "Alpha", "false",
      Tk_Offset(struct Togl, AlphaFlag), 0, NULL },
    { TK_CONFIG_INT, "-alphasize", "alphasize", "AlphaSize", "1",
      Tk_Offset(struct Togl, AlphaSize), 0, NULL },
    { TK_CONFIG_BOOLEAN, "-stencil", "stencil", "Stencil", "false",
      Tk_Offset(struct Togl, StencilFlag), 0, NULL },
    { TK_CONFIG_INT, "-stencilsize", "stencilsize", "StencilSize", "1",
      Tk_Offset(struct Togl, StencilSize), 0, NULL },
    { TK_CONFIG_INT, "-auxbuffers", "auxbuffers", "AuxBuffers", "0",
      Tk_Offset(struct Togl, AuxNumber), 0, NULL },
    { TK_CONFIG_BOOLEAN, "-privatecmap", "privateCmap", "PrivateCmap", "false",
      Tk_Offset(struct Togl, PrivateCmapFlag), 0, NULL },
    { TK_CONFIG_BOOLEAN, "-overlay", "overlay", "Overlay", "false",
      Tk_Offset(struct Togl, OverlayFlag), 0, NULL },
    { TK_CONFIG_BOOLEAN, "-stereo", "stereo", "Stereo", "false",
      Tk_Offset(struct Togl, StereoFlag), 0, NULL },

#ifndef NO_TK_CURSOR
    { TK_CONFIG_ACTIVE_CURSOR, "-cursor", "cursor", "Cursor", "",
      Tk_Offset(struct Togl, Cursor), TK_CONFIG_NULL_OK },
#endif
    { TK_CONFIG_INT, "-time", "time", "Time", DEFAULT_TIME,
      Tk_Offset(struct Togl, Time), 0, NULL },
    { TK_CONFIG_STRING, "-sharelist", "sharelist", "ShareList", NULL,
      Tk_Offset(struct Togl, ShareList), 0, NULL },
    { TK_CONFIG_STRING, "-sharecontext", "sharecontext", "ShareContext", NULL,
      Tk_Offset(struct Togl, ShareContext), 0, NULL },
    { TK_CONFIG_STRING, "-ident", "ident", "Ident", DEFAULT_IDENT,
      Tk_Offset(struct Togl, Ident), 0, NULL },
    { TK_CONFIG_END, (char *)NULL, (char *)NULL, (char *)NULL, (char *)NULL, 0,
      0, NULL }
};
...
#endif
            && visinfo->depth == 8) {
            status = XGetRGBColormaps(dpy, root, &standardCmaps, &numCmaps,
                                      hp_cr_maps);
            if (status) {
 ...
 */
#ifndef SOLARIS_BUG
    status = XmuLookupStandardColormap(dpy, visinfo->screen, visinfo->visualid,
                                       visinfo->depth, XA_RGB_DEFAULT_MAP,
...
static unsigned char threeto8[8] = { 0,         0111 >> 1, 0222 >> 1, 0333 >> 1,
...
static int defaultOverride[13] = { 0,   3,   24,  27,  64,  67, 88,
                                   173, 181, 236, 247, 164, 91 };
 ...
 */
void Togl_CreateFunc(Togl_Callback *proc) { CreateProc = proc; }

/*
 * Register a C function to be called when an Togl widget must be redrawn.
 */
void Togl_DisplayFunc(Togl_Callback *proc) { DisplayProc = proc; }

/*
 * Register a C function to be called when an Togl widget is resized.
 */
void Togl_ReshapeFunc(Togl_Callback *proc) { ReshapeProc = proc; }

/*
 * Register a C function to be called when an Togl widget is destroyed.
 */
void Togl_DestroyFunc(Togl_Callback *proc) { DestroyProc = proc; }

/*
 * Register a C function to be called from TimerEventHandler.
 */
void Togl_TimerFunc(Togl_Callback *proc) { TimerProc = proc; }
...
Togl_Callback *Togl_GetTimerFunc(struct Togl *togl) { return togl->TimerProc; }
...
void Togl_MakeCurrent(const struct Togl *togl) {
    ...
    assert(res == TRUE);
#elif defined(X11)
    glXMakeCurrent(Tk_Display(togl->TkWin), Tk_WindowId(togl->TkWin),
                   togl->GlCtx);
#if defined(__sgi) && defined(STEREO)
    stereoMakeCurrent(Tk_Display(togl->TkWin), Tk_WindowId(togl->TkWin),
                      togl->GlCtx);
#endif /*__sgi STEREO */
#endif /* WIN32 */
}
...
static void RenderOverlay(ClientData clientData) {
    ...
    if (togl->OverlayFlag && togl->OverlayDisplayProc) {
        ...
        assert(res == TRUE);
#elif defined(X11)
        glXMakeCurrent(Tk_Display(togl->TkWin), togl->OverlayWindow,
                       togl->OverlayCtx);
#if defined(__sgi) && defined(STEREO)
        stereoMakeCurrent(Tk_Display(togl->TkWin), togl->OverlayWindow,
                          togl->OverlayCtx);
#endif /*__sgi STEREO */
#endif /* WIN32 */
 ...
 */
int Togl_Configure(Tcl_Interp *interp, struct Togl *togl, int argc,
                   char *argv[], int flags) {
    ...
    if (Tk_ConfigureWidget(interp, togl->TkWin, configSpecs, argc, argv,
    ...
    if (togl->RgbaFlag != oldRgbaFlag || togl->RgbaRed != oldRgbaRed ||
        togl->RgbaGreen != oldRgbaGreen || togl->RgbaBlue != oldRgbaBlue ||
        togl->DoubleFlag != oldDoubleFlag || togl->DepthFlag != oldDepthFlag ||
        togl->DepthSize != oldDepthSize || togl->AccumFlag != oldAccumFlag ||
        togl->AccumRed != oldAccumRed || togl->AccumGreen != oldAccumGreen ||
        togl->AccumBlue != oldAccumBlue || togl->AccumAlpha != oldAccumAlpha ||
        togl->AlphaFlag != oldAlphaFlag || togl->AlphaSize != oldAlphaSize ||
...
int Togl_Widget(ClientData clientData, Tcl_Interp *interp, int argc,
                char *argv[]) {
    struct Togl *togl = (struct Togl *)clientData;
    int result = TCL_OK;
    Tcl_HashEntry *entry;
...
static int Togl_Cmd(ClientData clientData, Tcl_Interp *interp, int argc,
    ...
    togl->widgetCmd = Tcl_CreateCommand(
        interp, Tk_PathName(tkwin), Togl_Widget, (ClientData)togl,
...
static int SetupOverlay(struct Togl *togl) {
    ...
    static int ovAttributeList[] = {
        GLX_BUFFER_SIZE,          2,                         GLX_LEVEL, 1,
        GLX_TRANSPARENT_TYPE_EXT, GLX_TRANSPARENT_INDEX_EXT, None
    };
#else
    static int ovAttributeList[] = { GLX_BUFFER_SIZE, 2, GLX_LEVEL, 1, None };
#endif
    ...
    */
    ...
    togl->OverlayCtx = glXCreateContext(dpy, visinfo, togl->GlCtx, GL_TRUE);
    ...
    togl->OverlayWindow = XCreateWindow(
        dpy, Tk_WindowId(togl->TkWin), 0, 0, togl->Width, togl->Height, 0,
...
#define MAX_ATTEMPTS 12
    static int ci_depths[MAX_ATTEMPTS] = { 8, 4, 2, 1, 12, 16,
                                           8, 4, 2, 1, 12, 16 };
    ...
    hwnd = CreateWindow(
        TOGL_CLASS_NAME, NULL, WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, 0,
        0, togl->Width, togl->Height, parentWin, NULL, hInstance, NULL);
    ...
    pfd.cAccumBits = togl->AccumFlag ? (togl->AccumRed + togl->AccumGreen +
                                        togl->AccumBlue + togl->AccumAlpha)
                                     : 0;
...
/* need read/write colormap so user can store own color entries */
    ...
    swa.event_mask = ALL_EVENTS_MASK;
    winPtr->window =
        XCreateWindow(dpy, parent, 0, 0, togl->Width, togl->Height, 0,
                      visinfo->depth, InputOutput, visinfo->visual,
                      CWBorderPixel | CWColormap | CWEventMask, &swa);
...
char *Togl_Ident(const struct Togl *togl) { return togl->Ident; }

int Togl_Width(const struct Togl *togl) { return togl->Width; }

int Togl_Height(const struct Togl *togl) { return togl->Height; }

Tcl_Interp *Togl_Interp(const struct Togl *togl) { return togl->Interp; }

Tk_Window Togl_TkWin(const struct Togl *togl) { return togl->TkWin; }
...
static UINT Win32AllocColor(const struct Togl *togl, float red, float green,
                            float blue) {
    ...
    */
    ...
    /*
        * If this is not a duplicate and colormap is not full, allocate a new
     * entry.
        */
...
static void Win32SetColor(const struct Togl *togl, unsigned long index,
...
unsigned long Togl_AllocColor(const struct Togl *togl, float red, float green,
                              float blue) {
    XColor xcol;
    int exact;
...
#if defined(X11)
    XFreeColors(Tk_Display(togl->TkWin), Tk_Colormap(togl->TkWin), &pixel, 1,
                0);
#elif defined(WIN32)
...
void Togl_SetColor(const struct Togl *togl, unsigned long index, float red,
...
void Togl_PostOverlayRedisplay(struct Togl *togl) {
    if (!togl->OverlayUpdatePending && togl->OverlayWindow &&
        togl->OverlayDisplayProc) {
        Tk_DoWhenIdle(RenderOverlay, (ClientData)togl);
        togl->OverlayUpdatePending = 1;
    }
}

void Togl_OverlayDisplayFunc(Togl_Callback *proc) { OverlayDisplayProc = proc; }

int Togl_ExistsOverlay(const struct Togl *togl) { return togl->OverlayFlag; }
...
void Togl_ClientData(ClientData clientData) { DefaultClientData = clientData; }
...
Screen *Togl_Screen(const struct Togl *togl) { return Tk_Screen(togl->TkWin); }
        ...
        for (r = 0; r < RLEVELS; r++)
            for (g = 0; g < GLEVELS; g++)
                for (b = 0; b < BLEVELS; b++) {
                    ...
                    noFaultXAllocColor(display, colormap, ncolors, &xcol,
                                       &exact);
...
static int generateEPS(const char *filename, int inColor, unsigned int width,
                       unsigned int height) {
    FILE *fp;
    GLvoid *pixels;
    ...
    for (i = 0; i < width * height * components;) {
        ...
        if (inColor) {
            double pix = 0.0;
            pix = 0.30 * (double)curpix[i++] + 0.59 * (double)curpix[i++] +
                  0.11 * (double)curpix[i++];
            if (pix > 127.0)
                bitpixel |= 0x80;
            pix = 0.30 * (double)curpix[i++] + 0.59 * (double)curpix[i++] +
                  0.11 * (double)curpix[i++];
...
void Togl_StereoDrawBuffer(GLenum mode) {
    if (stereo.useSGIStereo) {
        ...
        */
        ...
        if (stereo.currentDisplay && stereo.currentWindow) {
            ...
            XSGISetStereoBuffer(stereo.currentDisplay, stereo.currentWindow,
...
void Togl_StereoClear(GLbitfield mask) {
    GLenum drawBuffer;
    if (stereo.useSGIStereo) {
...
void Togl_StereoFrustum(GLfloat left, GLfloat right, GLfloat bottom,
