...
struct CyberCalib {
    ...
    double n_h4; // acos((dnssp*dnssp + dnsep*dnsep -
                 // offn*offn)/(2*dnssp*dnsep))
    double t_h1; // offt - minscrew
    double t_h2; // dtssp^2 + dtsep^2
    double t_h3; // 1 / (2 * dtssp * dtsep)
    double t_h4; // acos((dtssp*dtssp + dtsep*dtsep -
                 // offt*offt)/(2*dtssp*dtsep))
  ...
  private:
    ...
    void setDerived(); // set (compute) the derived values from the compiled-in
                       // values
