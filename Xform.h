...
template <class T> Xform<T> operator*(const Xform<T> &a, const Xform<T> &b);

template <class T> Xform<T> delta(Xform<T> a, const Xform<T> &b);

template <class T>
Xform<T> linear_interp(const Xform<T> &a, const Xform<T> &b, double t);

template <class T>
Xform<T> bilinear_interp(const Xform<T> &u0v0, const Xform<T> &u1v0,
                         const Xform<T> &u0v1, const Xform<T> &u1v1, double u,
                         double v);

template <class T> Xform<T> relative_xform(const Xform<T> &a, Xform<T> b);

template <class T> ostream &operator<<(ostream &out, const Xform<T> &xf);

template <class T> istream &operator>>(istream &in, Xform<T> &xf);
...
class Xform {
  ...
  public:
    ...
    Xform<T> &addQuaternion(T q0, T q1, T q2, T q3, T tx = 0.0, T ty = 0.0,
                            T tz = 0.0);
    ...
    Xform<T> &fromFrame(const Pnt3 &x, const Pnt3 &y, const Pnt3 &z,
                        const Pnt3 &org);
    ...
    Xform<T> &rot(T angle, T ax, T ay, T az, T ox, T oy, T oz); // M <- R*M
    ...
    friend Xform<T> linear_interp(const Xform<T> &a, // r=t*a+(1-t)*b
                                  const Xform<T> &b, double t);
    friend Xform<T> bilinear_interp(const Xform<T> &u0v0, const Xform<T> &u1v0,
                                    const Xform<T> &u0v1, const Xform<T> &u1v1,
                                    double u, double v);
    ...
    friend Xform<T> bilinear_interp<>(const Xform<T> &u0v0,
                                      const Xform<T> &u1v0,
                                      const Xform<T> &u0v1,
                                      const Xform<T> &u1v1, double u, double v);
    friend Xform<T> relative_xform<>(const Xform<T> &a,
                                     Xform<T> b); // c = b^-1 * a
...
template <class T> inline Xform<T>::Xform(void) {
    m[1][0] = m[2][0] = m[3][0] = m[0][1] = m[2][1] = m[3][1] = 0.0;
    m[0][2] = m[1][2] = m[3][2] = m[0][3] = m[1][3] = m[2][3] = 0.0;
    m[0][0] = m[1][1] = m[2][2] = m[3][3] = 1.0;
}

template <class T> inline Xform<T>::Xform(const double *a) {
    m[0][0] = a[0];
    m[0][1] = a[1];
    m[0][2] = a[2];
    m[0][3] = a[3];
...
template <class T> inline Xform<T>::Xform(const float *a) {
    m[0][0] = a[0];
    m[0][1] = a[1];
    m[0][2] = a[2];
    m[0][3] = a[3];
...
template <class T> inline Xform<T>::Xform(const T r[3][3], const T t[3]) {
    m[0][0] = r[0][0];
    m[0][1] = r[1][0];
    m[0][2] = r[2][0];
    m[0][3] = 0.0;
...
template <class T> inline Xform<T> &Xform<T>::operator=(const double *a) {
    m[0][0] = a[0];
    m[0][1] = a[1];
    m[0][2] = a[2];
    m[0][3] = a[3];
...
template <class T> inline Xform<T> &Xform<T>::operator=(const float *a) {
    m[0][0] = a[0];
    m[0][1] = a[1];
    m[0][2] = a[2];
    m[0][3] = a[3];
...
template <class T> inline Xform<T> &Xform<T>::operator=(const Xform<T> &xf) {
...
template <class T>
inline Xform<T> &Xform<T>::fromQuaternion(const double q[4], double tx,
                                          double ty, double tz) {
...
template <class T>
inline Xform<T> &Xform<T>::fromQuaternion(const float q[4], float tx, float ty,
                                          float tz) {
    // q doesn't have to be unit
    T norm2 = q[0] * q[0] + q[1] * q[1] + q[2] * q[2] + q[3] * q[3];
    T s = (norm2 > 0.0) ? 2.0 / norm2 : 0.0;
...
template <class T> inline void Xform<T>::toQuaternion(double q[4]) const {
...
template <class T> inline void Xform<T>::toQuaternion(float q[4]) const {
    T tr = m[0][0] + m[1][1] + m[2][2];
    if (tr >= 0.0) {
        T s = sqrt(tr + m[3][3]);
        q[0] = s * 0.5;
...
template <class T>
inline Xform<T> &Xform<T>::addQuaternion(T q0, T q1, T q2, T q3, T tx, T ty,
                                         T tz) {
    rotQ(q0, q1, q2, q3);
    translate(tx, ty, tz);
    return *this;
...
template <class T>
inline Xform<T> &Xform<T>::addQuaternion(const float *q, int n) {
...
template <class T>
inline Xform<T> &Xform<T>::addQuaternion(const double *q, int n) {
    rotQ(q[0], q[1], q[2], q[3]);
    if (n == 7)
        translate(q[4], q[5], q[6]);
    return *this;
...
template <class T> inline void Xform<T>::fromEuler(const T _ea[3], int order) {
...
template <class T> inline void Xform<T>::toEuler(T ea[3], int order) {
    int i, j, k, n, s, f;
    EulGetOrd(order, i, j, k, n, s, f);
    if (s == EulRepYes) {
...
template <class T>
inline Xform<T> &Xform<T>::fromFrame(const Pnt3 &x, const Pnt3 &y,
                                     const Pnt3 &z, const Pnt3 &org) {
    m[0][0] = x[0];
    m[0][1] = x[1];
    m[0][2] = x[2];
    m[0][3] = 0.0;
...
template <class T>
inline Xform<T> &Xform<T>::fromRotTrans(const T r[3][3], const T t[3]) {
    m[0][0] = r[0][0];
    m[0][1] = r[1][0];
    m[0][2] = r[2][0];
    m[0][3] = 0.0;
...
template <class T> inline bool Xform<T>::isIdentity(void) const {
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            if (m[i][j] != (float)(i == j))
                return false;
...
template <class T> inline Xform<T> &Xform<T>::identity(void) {
    m[1][0] = m[2][0] = m[3][0] = m[0][1] = m[2][1] = m[3][1] = 0.0;
    m[0][2] = m[1][2] = m[3][2] = m[0][3] = m[1][3] = m[2][3] = 0.0;
    m[0][0] = m[1][1] = m[2][2] = m[3][3] = 1.0;
    return *this;
...
template <class T> inline Xform<T> &Xform<T>::fast_invert(void) {
    T tmp;
...
template <class T> inline Xform<T> &Xform<T>::invert(void) {
...
template <class T>
inline void Xform<T>::translate(const double t[3]) // M <- T*M
{
    for (int i = 0; i < 4; i++)
        m[i][0] += t[0] * m[i][3];
    for (int i = 0; i < 4; i++)
...
template <class T>
inline void Xform<T>::translate(const float t[3]) // M <- T*M
{
    for (int i = 0; i < 4; i++)
        m[i][0] += t[0] * m[i][3];
    for (int i = 0; i < 4; i++)
...
template <class T>
inline void Xform<T>::translate(T x, T y, T z) // M <- T*M
{
    for (int i = 0; i < 4; i++)
        m[i][0] += x * m[i][3];
    for (int i = 0; i < 4; i++)
...
template <class T> inline void Xform<T>::getTranslation(double t[3]) const {
    t[0] = m[3][0];
    t[1] = m[3][1];
    t[2] = m[3][2];
}

template <class T> inline void Xform<T>::getTranslation(float t[3]) const {
    t[0] = m[3][0];
    t[1] = m[3][1];
    t[2] = m[3][2];
}

template <class T> inline void Xform<T>::removeTranslation(void) {
    m[3][0] = m[3][1] = m[3][2] = 0.0;
}

template <class T> inline void Xform<T>::removeTranslation(double t[3]) {
    t[0] = m[3][0];
    t[1] = m[3][1];
    t[2] = m[3][2];
    m[3][0] = m[3][1] = m[3][2] = 0.0;
...
template <class T> inline void Xform<T>::removeTranslation(float t[3]) {
    t[0] = m[3][0];
    t[1] = m[3][1];
    t[2] = m[3][2];
    m[3][0] = m[3][1] = m[3][2] = 0.0;
...
template <class T>
inline void Xform<T>::scale(const double s[3]) // M <- S*M
{
    m[0][0] *= s[0];
    m[1][0] *= s[0];
    m[2][0] *= s[0];
...
template <class T>
inline void Xform<T>::scale(const float s[3]) // M <- S*M
{
    m[0][0] *= s[0];
    m[1][0] *= s[0];
    m[2][0] *= s[0];
...
template <class T>
inline void Xform<T>::scale(T x, T y, T z) // M <- S*M
{
    m[0][0] *= x;
    m[1][0] *= x;
    m[2][0] *= x;
...
template <class T> inline void Xform<T>::get_e(int i, T e[3]) const {
    e[0] = m[i][0];
    e[1] = m[i][1];
    e[2] = m[i][2];
}
...
template <class T>
inline Xform<T> &Xform<T>::rotX(T a) // M <- R*M
{
    double sa = sin(a);
    double ca = cos(a);
    double row1[4];
    for (int i = 0; i < 4; i++)
        row1[i] = m[i][1]; // temp. copy
    for (int i = 0; i < 4; i++)
        m[i][1] = ca * row1[i] - sa * m[i][2];
    for (int i = 0; i < 4; i++)
        m[i][2] = sa * row1[i] + ca * m[i][2];
    return *this;
}

template <class T>
inline Xform<T> &Xform<T>::rotY(T a) // M <- R*M
{
    double sa = sin(a);
    double ca = cos(a);
    double row0[4];
    for (int i = 0; i < 4; i++)
        row0[i] = m[i][0]; // temp. copy
    for (int i = 0; i < 4; i++)
        m[i][0] = ca * row0[i] + sa * m[i][2];
    for (int i = 0; i < 4; i++)
        m[i][2] = -sa * row0[i] + ca * m[i][2];
    return *this;
}

template <class T>
inline Xform<T> &Xform<T>::rotZ(T a) // M <- R*M
{
    double sa = sin(a);
    double ca = cos(a);
    double row0[4];
    for (int i = 0; i < 4; i++)
        row0[i] = m[i][0]; // temp. copy
    for (int i = 0; i < 4; i++)
        m[i][0] = ca * row0[i] - sa * m[i][1];
    for (int i = 0; i < 4; i++)
        m[i][1] = sa * row0[i] + ca * m[i][1];
    return *this;
}

// rotate around (0,0,0)
template <class T>
inline Xform<T> &Xform<T>::rot(T angle, T ax, T ay, T az) // M <- R*M
{
    T q[4];
    q[0] = cos(angle / 2.0);
...
template <class T>
inline Xform<T> &Xform<T>::rot(T angle, T ax, T ay, T az, T ox, T oy,
                               T oz) // M <- R*M
{
    T q[4];
    q[0] = cos(angle / 2.0);
...
template <class T>
inline Xform<T> &Xform<T>::rotQ(T q0, T q1, T q2, T q3) // M <- R*M
{
    T q[4];
    q[0] = q0;
    q[1] = q1;
...
template <class T>
inline void Xform<T>::get_rot(T &angle, T &ax, T &ay, T &az) {
    T q[4];
    toQuaternion(q);
...
template <class T>
inline void Xform<T>::apply(const double i[3], double o[3]) const // o = M*i
{
    T invw = 1.0 / (m[0][3] * i[0] + m[1][3] * i[1] + m[2][3] * i[2] + m[3][3]);
...
template <class T>
inline void Xform<T>::apply(const float i[3], float o[3]) const // o = M*i
{
    T invw = 1.0 / (m[0][3] * i[0] + m[1][3] * i[1] + m[2][3] * i[2] + m[3][3]);
    o[0] = (m[0][0] * i[0] + m[1][0] * i[1] + m[2][0] * i[2] + m[3][0]) * invw;
    o[1] = (m[0][1] * i[0] + m[1][1] * i[1] + m[2][1] * i[2] + m[3][1]) * invw;
...
template <class T>
inline void Xform<T>::apply_inv(const double i[3],
                                double o[3]) const // o = M^-1*i
{
    T tx = i[0] - m[3][0];
    T ty = i[1] - m[3][1];
    T tz = i[2] - m[3][2];
...
template <class T>
inline void Xform<T>::apply_inv(const float i[3],
                                float o[3]) const // o = M^-1*i
{
    T tx = i[0] - m[3][0];
    T ty = i[1] - m[3][1];
    T tz = i[2] - m[3][2];
...
template <class T> inline void Xform<T>::operator()(double i[3]) const {
    T o[3];
    o[0] = i[0];
    o[1] = i[1];
    o[2] = i[2];
    T invw = 1.0 / (m[0][3] * o[0] + m[1][3] * o[1] + m[2][3] * o[2] + m[3][3]);
...
template <class T> inline void Xform<T>::operator()(float i[3]) const {
    T o[3];
    o[0] = i[0];
    o[1] = i[1];
    o[2] = i[2];
...
template <class T>
inline void Xform<T>::apply_nrm(const double i[3], double o[3]) const // o = M*i
{
    double invw =
...
template <class T>
inline void Xform<T>::apply_nrm(const float i[3], float o[3]) const // o = M*i
{
    double invw =
        1.0 / (m[0][3] * i[0] + m[1][3] * i[1] + m[2][3] * i[2] + m[3][3]);
    o[0] = (m[0][0] * i[0] + m[1][0] * i[1] + m[2][0] * i[2]) * invw;
...
template <class T> inline T &Xform<T>::operator()(int i, int j) {
    return m[j][i];
}

template <class T> inline T Xform<T>::operator()(int i, int j) const {
    return m[j][i];
}

template <class T>
inline Pnt3 Xform<T>::unproject(float u, float v, float z) const {
...
template <class T>
inline Pnt3 Xform<T>::unproject_fast(float u, float v, float z) const {
    float w = 1.0 / (z * m[2][3] + m[3][3]);
    return Pnt3((u * m[0][0] + m[3][0]) * w, (u * m[1][1] + m[3][1]) * w, -w);
}

template <class T>
inline Xform<T> &Xform<T>::
operator*=(const Xform<T> &a) // (*this) = a * (*this)
{
    Xform<T> b((T *)this);
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
...
// test how close to a rigid transformation the current
// transformation is, ideally returns 0
template <class T> inline float Xform<T>::test_rigidity(void) {
    float ans = 0.0;
...
// part to 0 0 0 1 and making sure rotation part is just a rotation
template <class T> inline Xform<T> &Xform<T>::enforce_rigidity(void) {
    m[0][3] = m[1][3] = m[2][3] = 0.0;
    m[3][3] = 1.0;
    T q[4];
    toQuaternion(q);
...
template <class T>
inline Xform<T> operator*(const Xform<T> &a, const Xform<T> &b) {
    Xform<T> out;
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            out.m[j][i] = a.m[0][i] * b.m[j][0] + a.m[1][i] * b.m[j][1] +
                          a.m[2][i] * b.m[j][2] + a.m[3][i] * b.m[j][3];
        }
    }
    return out;
...
// delta * a = b
template <class T> inline Xform<T> delta(Xform<T> a, const Xform<T> &b) {
...
template <class T>
inline Xform<T> linear_interp(const Xform<T> &a, const Xform<T> &b, double t) {
    Xform<T> d = delta(a, b);
...
template <class T>
inline Xform<T> bilinear_interp(const Xform<T> &u0v0, const Xform<T> &u1v0,
                                const Xform<T> &u0v1, const Xform<T> &u1v1,
                                double u, double v) {
...
template <class T>
inline Xform<T> relative_xform(const Xform<T> &a, Xform<T> b) {
    return b.fast_invert() * a;
}

template <class T>
inline ostream &operator<<(ostream &out, const Xform<T> &xf) {
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            // out << xf(i,j) << " ";
            out << xf.m[j][i] << " ";
...
template <class T> inline istream &operator>>(istream &in, Xform<T> &xf) {
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            in >> ws >> xf.m[j][i];
        }
