...
const char *type_names[] = { "invalid", "char", "short", "int",    "uchar",
                             "ushort",  "uint", "float", "double", };

const char *new_type_names[] = { "invalid", "int8",    "int16",
                                 "int32",   "uint8",   "uint16",
...
char PlyFile::read_char(void) {
    fbuf_need(1);
    return *(fb_s++);
}

unsigned char PlyFile::read_uchar(void) {
    fbuf_need(1);
    return *(fb_s++);
}

short PlyFile::read_short_BE(void) {
    fbuf_need(2);
    short s;
    char *p = (char *)&s;
    p[0] = fb_s[0];
...
unsigned short PlyFile::read_ushort_BE(void) {
    fbuf_need(2);
    unsigned short s;
    char *p = (char *)&s;
    p[0] = fb_s[0];
...
unsigned int PlyFile::read_uint_BE(void) {
    fbuf_need(4);
    unsigned int s;
    char *p = (char *)&s;
    p[0] = fb_s[0];
...
float PlyFile::read_float_BE(void) {
    fbuf_need(4);
    float s;
    char *p = (char *)&s;
    p[0] = fb_s[0];
...
double PlyFile::read_double_BE(void) {
    fbuf_need(8);
    double s;
    char *p = (char *)&s;
    p[0] = fb_s[0];
...
short PlyFile::read_short_LE(void) {
    fbuf_need(2);
    short s;
    char *p = (char *)&s;
    p[0] = fb_s[1];
...
unsigned short PlyFile::read_ushort_LE(void) {
    fbuf_need(2);
    unsigned short s;
    char *p = (char *)&s;
    p[0] = fb_s[1];
...
unsigned int PlyFile::read_uint_LE(void) {
    fbuf_need(4);
    unsigned int s;
    char *p = (char *)&s;
    p[0] = fb_s[3];
...
float PlyFile::read_float_LE(void) {
    fbuf_need(4);
    float s;
    char *p = (char *)&s;
    p[0] = fb_s[3];
...
double PlyFile::read_double_LE(void) {
    fbuf_need(8);
    double s;
    char *p = (char *)&s;
    p[0] = fb_s[7];
...
int PlyFile::to_int(char *item, int type) {
    switch (type) {
    case PLY_CHAR:
        return *item;
    case PLY_UCHAR:
...
int PlyFile::open_for_writing(const char *filename, int nelems,
...
int PlyFile::open_for_reading(const char *filename, int *nelems,
                              char ***elem_names) {
    char *name = new char[(strlen(filename) + 5)];
    strcpy(name, filename);
#if 0
...
void PlyFile::describe_element(char *elem_name, int nelems, int nprops,
...
void PlyFile::describe_property(char *elem_name, PlyProperty *prop) {
    /* look for appropriate element */
    PlyElement *elem = find_element(elem_name);
    if (elem == NULL) {
...
void PlyFile::element_count(char *elem_name, int nelems) {
    // look for appropriate element
    PlyElement *elem = find_element(elem_name);
    if (elem == NULL) {
...
void PlyFile::describe_other_elements(PlyOtherElems *o_elems) {
    int i;
    OtherElem *other;
...
void PlyFile::describe_other_properties(PlyOtherProp *other, int offset) {
    int i;
    PlyElement *elem;
...
void PlyFile::put_element_setup(char *elem_name) {
    PlyElement *elem = find_element(elem_name);
    if (elem == NULL) {
...
void PlyFile::put_element_static_strg(void *elem_ptr) {
    _put_element(elem_ptr, 1);
}

void PlyFile::put_element(void *elem_ptr) { _put_element(elem_ptr, 0); }

void PlyFile::_put_element(void *elem_ptr, int static_strg) {
    int j, k;
    char *item;
    int list_count;
    int item_size;
...
void PlyFile::put_comment(char *comment) {
    // add comment to list
...
void PlyFile::put_obj_info(char *o_info) {
    // add info to list
...
PlyProperty **PlyFile::get_element_description(char *elem_name, int *nelems,
                                               int *nprops) {
...
void PlyFile::get_element_setup(char *elem_name, int nprops,
                                PlyProperty *prop_list) {
    PlyProperty *prop;
    int index;
    ...
    for (int i = 0; i < nprops; i++) {
        ...
        if (prop == NULL) {
            cerr << "Warning:  Can't find property '" << prop_list[i].name
                 << "' in element '" << elem_name << "'" << endl;
            continue;
        }
...
void PlyFile::get_property(char *elem_name, PlyProperty *prop) {
    // find information about the element
    PlyElement *elem = find_element(elem_name);
    which_elem = elem;
    ...
    if (prop_ptr == NULL) {
        cerr << "Warning:  Can't find property '" << prop->name
             << "' in element '" << elem_name << "'" << endl;
        return;
    }
...
void PlyFile::get_element_noalloc(void *elem_ptr) {
    if (m_file_type == PLY_ASCII)
...
char **PlyFile::get_comments(int *n_comments) {
...
char **PlyFile::get_obj_info(int *n_obj_info) {
...
PlyOtherProp *PlyFile::get_other_properties(char *elem_name, int offset) {
    int i;
    PlyElement *elem;
    PlyOtherProp *other;
...
PlyOtherElems *PlyFile::get_other_element(char *elem_name, int elem_count) {
    int i;
    PlyElement *elem;
    ...
    other->other_props =
        get_other_properties(elem_name, offsetof(OtherData, other_props));
...
******************************************************************/
PlyElement *PlyFile::find_element(char *element) {
...
PlyProperty *PlyFile::find_property(PlyElement *elem, char *prop_name,
                                    int *index) {
    for (int i = 0; i < elem->props.size(); i++)
...
void PlyFile::ascii_get_element(char *elem_ptr, int use_provided_storage) {
    int j, k;
    int which_word;
    char *elem_data, *item;
    int int_val;
...
static void inv_memcpy(char *to, const char *from, int size, int n) {
    for (int i = 0; i < n; i++) {
        int off_to = size * i;
...
void PlyFile::get_and_store_binary_item(int type_ext, int type_int,
                                        char *item) {
    if (type_ext == type_int) {
        ...
        double double_val;
        if (m_file_type == NATIVE_ENDIANNES) {
            get_binary_item_BE(type_ext, &int_val, &uint_val, &double_val);
        } else {
            get_binary_item_LE(type_ext, &int_val, &uint_val, &double_val);
        }
        store_item(item, type_int, int_val, uint_val, double_val);
    }
}

void PlyFile::get_and_store_binary_list(int type_ext, int type_int, char *item,
                                        int n) {
    if (type_ext == type_int) {
        // external type and internal type is the same, short
        // circuit
...
void PlyFile::binary_get_element(char *elem_ptr, int use_provided_storage) {
    int j, k;
    char *elem_data, *item;
    int int_val;
    unsigned int uint_val;
    ...
    for (j = 0; j < elem->props.size(); j++) {
        ...
        if (elem->store_prop[j] | other_flag) {
            ...
            if (prop.is_list) { // a list
                ...
                get_binary_item_BE(prop.count_external, &list_count, &uint_val,
                                   &double_val);

                if (list_count) {
                    // read items and store them into the array
                    for (k = 0; k < list_count; k++) {
                        get_binary_item_BE(prop.external_type, &int_val,
                                           &uint_val, &double_val);
                    }
                }
            } else { // not a list
                get_binary_item_BE(prop.external_type, &int_val, &uint_val,
                                   &double_val);
            }
        }
    }
...
void PlyFile::write_scalar_type(int code) {
...
char **PlyFile::get_words(int *nwords, char **orig_line) {
...
double PlyFile::get_item_value(char *item, int type) {
...
void PlyFile::write_binary_item_BE(int int_val, unsigned int uint_val,
    ...
    case PLY_DOUBLE:
        ...
        break;
    default:
        cerr << "write_binary_item_BE: bad type = " << type << endl;
        exit(-1);
    }
}

static void swap(short i, char *c) {
    char *p = (char *)&i;
    c[0] = p[1];
    c[1] = p[2];
}

static void swap(unsigned short i, char *c) {
    char *p = (char *)&i;
    c[0] = p[1];
    c[1] = p[2];
}

static void swap(int i, char *c) {
    char *p = (char *)&i;
    c[0] = p[3];
    c[1] = p[2];
    c[2] = p[1];
...
static void swap(unsigned int i, char *c) {
    char *p = (char *)&i;
    c[0] = p[3];
    c[1] = p[2];
    c[2] = p[1];
...
static void swap(float i, char *c) {
    char *p = (char *)&i;
    c[0] = p[3];
    c[1] = p[2];
    c[2] = p[1];
...
static void swap(double i, char *c) {
    char *p = (char *)&i;
    c[0] = p[7];
    c[1] = p[6];
    c[2] = p[5];
...
void PlyFile::write_binary_item_LE(int int_val, unsigned int uint_val,
                                   double double_val, int type) {
    unsigned char uchar_val;
    char char_val;
    char c[8];
    ...
    case PLY_DOUBLE:
        ...
        break;
    default:
        cerr << "write_binary_item_LE: bad type = " << type << endl;
        exit(-1);
    }
}
...
void PlyFile::write_ascii_item(int int_val, unsigned int uint_val,
                               double double_val, int type) {
    switch (type) {
    case PLY_CHAR:
    case PLY_SHORT:
...
void PlyFile::get_stored_item(void *ptr, int type, int *int_val,
...
void PlyFile::get_binary_item_BE(int type, int *int_val, unsigned int *uint_val,
                                 double *double_val) {
    switch (type) {
    case PLY_CHAR:
        *int_val = read_char();
...
void PlyFile::get_binary_item_LE(int type, int *int_val, unsigned int *uint_val,
                                 double *double_val) {
    switch (type) {
    case PLY_CHAR:
        *int_val = read_char();
...
void PlyFile::get_ascii_item(char *word, int type, int *int_val,
                             unsigned int *uint_val, double *double_val) {
    switch (type) {
    case PLY_CHAR:
    case PLY_UCHAR:
...
void PlyFile::store_item(char *item, int type, int int_val,
                         unsigned int uint_val, double double_val) {
    unsigned char *puchar;
    short int *pshort;
    unsigned short int *pushort;
...
void PlyFile::add_element(char **words, int nwords) {
...
void PlyFile::add_property(char **words, int nwords) {
    PlyProperty prop;
...
void PlyFile::add_comment(char *line) {
    int i;
...
void PlyFile::add_obj_info(char *line) {
    int i;
...
int PlyFile::is_valid_property(char *elem_name, char *prop_name) {
    // find information about the element
    PlyElement *elem = find_element(elem_name);
    which_elem = elem;
    int index;
