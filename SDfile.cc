...
#include <zlib.h>
#endif
__median(const unsigned short &__a, const unsigned short &__b,
         const unsigned short &__c) {
...
#define FIX_SHORT(x)                                                           \
    (*(unsigned short *)&(x) = swap_short(*(unsigned short *)&(x)))
#define FIX_LONG(x) (*(unsigned *)&(x) = swap_long(*(unsigned *)&(x)))
#define FIX_FLOAT(x) FIX_LONG(x)

#define WRITE_UINT(x)                                                          \
    ((sg_swapbuf_int = x), FIX_LONG(sg_swapbuf_int),                           \
     (fwrite(&sg_swapbuf_int, sizeof(unsigned int), 1, sdfile) != 1))
#define WRITE_FLOAT(x)                                                         \
    ((sg_swapbuf_float = x), FIX_FLOAT(sg_swapbuf_float),                      \
     (fwrite(&sg_swapbuf_float, sizeof(float), 1, sdfile) != 1))
#define WRITE_USHORT(x)                                                        \
    ((sg_swapbuf_short = x), FIX_SHORT(sg_swapbuf_short),                      \
     (fwrite(&sg_swapbuf_short, sizeof(unsigned short), 1, sdfile) != 1))
...
#define WRITE_UINT(x) (fwrite(&x, sizeof(unsigned int), 1, sdfile) != 1)
#define WRITE_FLOAT(x) (fwrite(&x, sizeof(float), 1, sdfile) != 1)
#define WRITE_USHORT(x) (fwrite(&x, sizeof(unsigned short), 1, sdfile) != 1)

#endif

// These return true on error
#define READ_UINT(x)                                                           \
    ((gzread(sdfile, &x, sizeof(unsigned int)) != sizeof(unsigned int)) ||     \
     (FIX_LONG(x) && 0))
#define READ_FLOAT(x)                                                          \
    ((gzread(sdfile, &x, sizeof(float)) != sizeof(float)) ||                   \
     (FIX_FLOAT(x) && 0))
#define READ_USHORT(x)                                                         \
    ((gzread(sdfile, &x, sizeof(unsigned short)) != sizeof(unsigned short)) || \
     (FIX_SHORT(x) && 0))

void SDfile::prepare_axis_proj(void) {
    if (axis_proj_done)
        return;

    axis_proj_done = true;

    axis_proj_min = axis_proj_max = xf.axis_project(first_good[0], z_data[0]);

    for (int i = 0; i < n_frames; i++) {

        if (first_good[i] >= first_bad[i])
            continue;

        float pr = xf.axis_project(first_good[i], z_data[row_start[i]]);
        if (pr < axis_proj_min)
            axis_proj_min = pr;
        else if (pr > axis_proj_max)
            axis_proj_max = pr;

        int cnt = first_bad[i] - first_good[i];
        pr = xf.axis_project(first_bad[i] - 1, z_data[row_start[i] + cnt]);
        if (pr < axis_proj_min)
            axis_proj_min = pr;
        else if (pr > axis_proj_max)
            axis_proj_max = pr;
...
static void create_strip(vector<int> &tstrips, const vector<int> &p_inds_0,
                         const vector<int> &p_inds_1,
                         bool upper_row_starts = true, bool flipped = false) {
    bool str_on = false;
    // 1 *---*
    //    \1/2\
  // 0   *---*
...
inline void mark_all_empty(const vector<int> &curr, vector<char> &bdry) {
...
inline void mark_maybe_empty(const vector<int> &curr, const vector<int> &prev,
                             const vector<int> &next, vector<char> &bdry) {
    int n = curr.size();
    if (curr[0] != -1)
        bdry[curr[0]] = 1;
    int end = n - 1;
    ...
    for (int j = 1; j < end; j++) {
        int k = curr[j];
        if (k != -1 &&
            (curr[j - 1] == -1 || curr[j + 1] == -1 || prev[j] == -1 ||
             prev[j - 1] == -1 || next[j] == -1 || next[j - 1] == -1)) {
            bdry[k] = 1;
        }
    }
}

void SDfile::make_tstrip_horizontal(vector<int> &tstrips, vector<char> &bdry) {
...
static int get_index(int col, int idx, int n, const vector<int> &indices,
                     int good, int bad) {
...
void SDfile::make_tstrip_vertical(vector<int> &tstrips, vector<char> &bdry) {
    ...
    // initialize the point indices
    for (i = 0; i < n; i++) {
        c_0[i] = get_index(0, row_start[i], n_pts, indices, first_good[i],
                           first_bad[i]);
        c_1[i] = get_index(1, row_start[i] + 1, n_pts, indices, first_good[i],
                           first_bad[i]);
    }
    ...
    for (i = 2; i < pts_per_frame; i += 2) {
        ...
        for (j = 0; j < n; j++) {
            ...
            c_0[j] = get_index(i, tmp + i, n_pts, indices, first_good[j],
                               first_bad[j]);
            c_1[j] = get_index(oddi, tmp + oddi, n_pts, indices, first_good[j],
                               first_bad[j]);
...
void SDfile::fill_index_array(int step, int row, SubSampMode subSampMode,
                ...
                if (k % step == 0) {
                    if (subSampMode == conf || subSampMode == filterAvg) {
                        ...
                        for (int irow = firstrow; irow < lastrow; irow++) {
                            ...
                            int lastcol =
                                min(maxcol, int(first_bad[irow])) + rs - fg;
...
bool SDfile::find_row_range(float screw, float radius, const Pnt3 &ctr,
                            int rowrange[2]) {
...
SDfile::SDfile(void)
    : row_start(NULL), first_good(NULL), first_bad(NULL), z_data(NULL),
      intensity_data(NULL), n_valid_pts(0), axis_proj_done(false) {}
...
bool SDfile::read(const string &fname) {
    if (g_verbose)
        cout << "SDfile::read(" << fname << ")... ";
//
...
//
    ...
    if (version == 7 || version == 6) {
        ...
        if (READ_UINT(header_size) || READ_UINT(n_frames) ||
            READ_UINT(pts_per_frame) || READ_UINT(n_pts) ||
            READ_FLOAT(frame_pitch) || READ_FLOAT(scan_screw) ||
            READ_FLOAT(other_screw) || READ_FLOAT(scanner_trans)) {
        ...
        if (READ_UINT(scanner_config) || READ_FLOAT(laser_intensity) ||
        ...
        for (int i = 0; i < n_frames; ++i) {
            if (READ_USHORT(first_good[i]) || READ_USHORT(first_bad[i])) {
        ...
        if (READ_UINT(n_frames) || READ_UINT(pts_per_frame) ||
            READ_UINT(n_pts) || READ_FLOAT(frame_pitch) ||
            READ_FLOAT(scan_screw) || READ_FLOAT(other_screw) ||
            READ_FLOAT(scanner_trans) || READ_UINT(scanner_config)) {
            cerr << "Can't read sd file header" << endl;
            return false;
        }
...
bool SDfile::write(const string &fname) {
    FILE *sdfile;
    ...
    if (WRITE_UINT(magic_num) || WRITE_UINT(header_size) ||
        WRITE_UINT(n_frames) || WRITE_UINT(pts_per_frame) ||
        WRITE_UINT(n_pts) || WRITE_FLOAT(frame_pitch) ||
        WRITE_FLOAT(scan_screw) || WRITE_FLOAT(other_screw) ||
        WRITE_FLOAT(scanner_trans) || WRITE_FLOAT(scanner_vert) ||
        WRITE_UINT(scanner_config) || WRITE_FLOAT(laser_intensity) ||
    ...
    for (int i = 0; i < n_frames; ++i) {
        if (WRITE_USHORT(first_good[i]) || WRITE_USHORT(first_bad[i])) {
...
void SDfile::fill_holes(int max_missing, int thresh) {
    ...
    for (int row = 0; row < n_frames; row++) {
        ...
        for (int col = max_missing; col < numcols - max_missing; col++) {
            ...
            unsigned short med;
            if (max_missing == 1) {
                med = __median(z_data[index - 1], me, z_data[index + 1]);
#if 1
            } else if (max_missing == 2) {
                // Fast approx. to median of 5
                med = __median(z_data[index - 1], me, z_data[index + 1]);
                med = __median(z_data[index - 2], med, z_data[index + 2]);
#endif
            } else {
...
void SDfile::make_tstrip(vector<int> &tstrips, vector<char> &bdry) {
...
void SDfile::subsampled_tstrip(int step, char *behavior, vector<int> &tstrips,
                               vector<char> &bdry, vector<Pnt3> &pnts,
                               vector<uchar> &intensity,
                               vector<uchar> &confidence, vector<int> &ssmap) {
    int n_s = pts_per_frame / step;
    int end = 2 * n_frames / step;

    SubSampMode subSampMode =
        !strcmp(behavior, "holes")
            ? holesGrow
            : !strcmp(behavior, "conf") ? conf : !strcmp(behavior, "filter")
                                                     ? filterAvg
                                                     : fast;
    ...
    if (n_s > 1 && end > 1) {
        ...
        // first row
        fill_index_array(step, 0, subSampMode, p_inds[0], pnts, ssmap,
        ...
        fill_index_array(step, 1, subSampMode, p_inds[1], pnts, ssmap,
                         intensity, confidence);
        create_strip(tstrips, p_inds[0], p_inds[1], true, flip);
        // rest of the rows
        int i, j;
        for (i = 2, j = 0; i < end; i++, j ^= 1) {
            copy(p_inds[j].begin(), p_inds[j].end(), old_inds.begin());
            fill(p_inds[j].begin(), p_inds[j].end(), -1);
            fill_index_array(step, i, subSampMode, p_inds[j], pnts, ssmap,
                             intensity, confidence);
            mark_maybe_empty(p_inds[!j], old_inds, p_inds[j], tmp_bdry);
            create_strip(tstrips, p_inds[!j], p_inds[j], true, flip);
        }
...
void SDfile::set_xf(int row, bool both) {
    if (both) {
...
void SDfile::get_pnts_and_intensities(vector<Pnt3> &pnts,
                                      vector<uchar> &intensity) {
    int cnt = 0;
    for (int i = 0; i < n_frames; ++i) {
        set_xf(i);
...
bool SDfile::get_point(int row, int col, Pnt3 &p, bool transformed) {
    // row within range?
...
Xform<float> SDfile::vrip_reorientation_frame(void) {
    set_xf(n_frames / 2);
...
bool SDfile::find_data(float screw, float y_in, int &row, unsigned short &y,
                       unsigned short &z) {
    if (y_in < 0.0) {
        // SHOW(y_in);
        return false;
...
int SDfile::count_valid_pnts(void) {
    if (n_valid_pts == 0) {
...
void SDfile::filtered_copy(const VertexFilter &filter, SDfile &newsd) {
...
void SDfile::get_piece(int firstFrame, int lastFrame, SDfile &newsd) {
    // copy the data, first shallow copy
    newsd = *this;
    ...
    newsd.scan_screw =
        scan_screw +
        (firstFrame + (int(newsd.n_frames) - int(n_frames)) * 0.5) *
            frame_pitch;
    ...
    if (newsd.n_pts) {
        ...
        if (!g_bNoIntensity) {
            ...
            memcpy(newsd.intensity_data, intensity_data + row_start[firstFrame],
                   newsd.n_pts);
        }
    }
}

Bbox SDfile::get_bbox(void) {
    int cnt = 0;
    Bbox bb;
    for (int i = 0; i < n_frames; ++i) {
        set_xf(i);
...
int SDfile::sphere_status(const Pnt3 &ctr, float r) {
    float screw, swin[2];
    short ywin[2], zwin[2];
    prepare_axis_proj();
    int ans = xf.sphere_status(ctr, r, screw, axis_proj_min, axis_proj_max,
                               ywin, zwin, swin);
    ...
    cout << ywin[0] << " " << ywin[1] << " " << zwin[0] << " " << zwin[1] << " "
         << swin[0] << " " << swin[1] << " " << r << endl;
    ...
    for (int i = start_row; i <= end_row; i++) {
        if (first_good[i] > ywin[0])
            return 1; // invalid, force subdiv
        if (first_bad[i] <= ywin[1])
            return 1; // invalid, force subdiv
        unsigned short *z = &z_data[row_start[i] + ywin[0] - first_good[i]];
        for (int j = 0; j < span; j++) {
            if (z[j] < zwin[0]) {
                all_front = all_within = false;
            } else if (z[j] > zwin[1]) {
                all_behind = all_within = false;
            } else {
                all_front = all_behind = false;
            }
        }
...
int SDfile::dump_pts_laser_subsampled(ofstream &out, int nPnts) {
        ...
        for (int j = first_good[i]; j < first_bad[i]; j++) {
            ...
            if (z_data[k]) {
                ...
                if (rnd() < float(still_need) / float(valids_left)) {
                    ...
                    if (j % 2)
                        out.write((char *)&oddscr, sizeof(float));
                    else
                        out.write((char *)&evenscr, sizeof(float));
...
Pnt3 SDfile::raw_for_ith(int j, sd_raw_pnt &data) {
    assert(j < n_pts);
...
Pnt3 SDfile::raw_for_ith_valid(int i, sd_raw_pnt &data) {
