...
TriOctree *TriOctree::recursive_find(int face, stack<int> &path,
                                     bool bReachedTop) {
    if (path.empty())
        return this;
...
bool TriOctree::is_neighbor_cell(int face, int &iChild) {
...
TriOctree *TriOctree::find_neighbor(int face) {
    if (!parent)
        return NULL;
...
TriOctree::TriOctree(const Pnt3 &c, float r, const Pnt3 *p,
                     const vector<int> &tris, float minRadius, int pidx,
                     TriOctree *parent)
    ...
    for (int i = 0; i < 2; i++) {
        ...
        for (int j = 0; j < 2; j++) {
            ...
            childCtr.set(mCtr[0] + (i ? halfR : -halfR),
                         mCtr[1] + (j ? halfR : -halfR), mCtr[2] - halfR);
            it = uk.begin();
            while (it != uk.end()) {
                v1 = *it++;
                v2 = *it++;
                v3 = *it++;
                if (!tri_outside_of_cube(childCtr, mRadius, p[v1], p[v2],
                                         p[v3])) {
                    zm.push_back(v1);
                    zm.push_back(v2);
                    zm.push_back(v3);
            ...
            if (zm.size()) {
                child[k] =
                    new TriOctree(childCtr, halfR, p, zm, minRadius, k, this);
            } else {
                child[k] = NULL;
            }
            ...
            if (zp.size()) {
                child[k] =
                    new TriOctree(childCtr, halfR, p, zp, minRadius, k, this);
            } else {
                child[k] = NULL;
            }
        }
...
bool TriOctree::search(const Pnt3 &p, Pnt3 &cp, float &d2) {
    // is this a leaf node?
    if (pts) {
        // look for a new closest distance
        vector<int>::const_iterator it = tind.begin();
        while (it != tind.end()) {
            closer_on_tri(p, cp, pts[*(it++)], pts[*(it++)], pts[*(it++)], d2);
        }
    ...
    if (child[iChild] && child[iChild]->search(p, cp, d2))
        return true;
...
static void cube_bdry(const Pnt3 &ctr, float r, vector<Pnt3> &p,
                      vector<int> &i) {
    int s = p.size();
...
void TriOctree::visualize(vector<Pnt3> &p, vector<int> &ind) {
