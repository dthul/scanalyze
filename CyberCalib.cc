...
CyberCalib::CyberCalib() {
    ...
    if (calibFN) {
        ...
        if (in.fail() || !readNumbers(in, r2dv) || !setMap(r2dv)) {
            cerr << "Couldn't read calibration file: " << calibFN << endl;
            exit(1);
        } else {
...
bool setval(double (&d)[3], vector<double> &vd)
...
bool setval(double (&d)[16], vector<double> &vd) {
    if (vd.size() != 16)
        return 0;
...
template <int N>
bool setval(double (&d)[N], vector<double> &vd)
// Set the size contents of the size n array d to be the same as the
// contents of the vector, if possible.
//
// If not possible return false.
...
bool setval(double &d, vector<double> &vd)
...
bool CyberCalib::setMap(RopeDV &r2dv)
// Set all of the input calibration values using the map r2dv.  If
// something doesn't work return false, otherwise return true.
{
    return setval(axturn, r2dv["axturn"]) && setval(axturn0, r2dv["axturn0"]) &&
           setval(axnod, r2dv["axnod"]) && setval(axnod0, r2dv["axnod0"]) &&
           setval(A, r2dv["A"]) && setval(opt_frm_v, r2dv["opt_frm_v"]) &&
           ...
           setval(dnssp, r2dv["dnssp"]) && setval(dnsep, r2dv["dnsep"]) &&
           setval(offn, r2dv["offn"]) && setval(dtssp, r2dv["dtssp"]) &&
           setval(dtsep, r2dv["dtsep"]) && setval(offt, r2dv["offt"]) &&
           setval(minscrew, r2dv["minscrew"]);
}

//#define OLD_DEFAULTS
#ifndef OLD_DEFAULTS
void CyberCalib::setDefault()
...
// computations in CyberXform.cc).
{
    ...
    n_h4 = acos((dnssp * dnssp + dnsep * dnsep - offn * offn) /
                (2 * dnssp * dnsep));
    t_h1 = offt - minscrew;
    t_h2 = dtssp * dtssp + dtsep * dtsep;
    t_h3 = 1.0 / (2.0 * dtssp * dtsep);
    t_h4 = acos((dtssp * dtssp + dtsep * dtsep - offt * offt) /
                (2 * dtssp * dtsep));
}

bool CyberCalib::readNumbers(istream &in, RopeDV &r2dv)
