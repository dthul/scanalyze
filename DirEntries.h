...
class DirEntries {
  ...
  public:
    ...
    DirEntries(std::string dirname, std::string end_filter = "") : count(0) {
#ifdef WIN32
    ...
    bool done(void) { return (count >= m_entry.size()); }

    std::string &entry(void) { return m_entry[count]; }

    std::string path(void) { return m_path + m_entry[count]; }

    int size(void) { return m_entry.size(); }
};
