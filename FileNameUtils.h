...
inline bool get_filename_new_ending(const char *inName, const char *ext,
                                    char *outName) {
...
inline bool filename_has_ending(const char *filename, const char *ending) {
...
inline void remove_trailing_slash(char *fname) {
    int n = strlen(fname);
    if (fname[n - 1] == '/')
        fname[n - 1] = 0;
}

inline bool check_file_access(const char *fname, bool existence = true,
                              bool readable = true, bool writable = false,
                              bool executable = false, bool directory = false) {
    int res = true;
    ...
    if (res && directory) {
        struct stat st;
        if (stat(fname, &st)) {
            cerr << "Failed checking whether a directory for " << fname << endl;
            perror(NULL);
            res = false;
        }
...
inline bool unlink_if_exists(const char *fname) {
...
inline int portable_mkdir(const char *dname, int permissions = 00775) {
#ifdef WIN32
    return mkdir(dname);
#else
...
inline int portable_symlink(const char *s1, const char *s2) {
#ifndef WIN32
