...
PlyProperty vert_std_props[] = {
    ...
    { "confidence", PLY_FLOAT, PLY_FLOAT, offsetof(PlyVertex, confidence), 0, 0,
      0, 0 },
    { "intensity", PLY_FLOAT, PLY_FLOAT, offsetof(PlyVertex, intensity), 0, 0,
      0, 0 },
    { "diffuse_red", PLY_UCHAR, PLY_UCHAR, offsetof(PlyVertex, red), 0, 0, 0,
      0 },
    { "diffuse_green", PLY_UCHAR, PLY_UCHAR, offsetof(PlyVertex, grn), 0, 0, 0,
      0 },
    { "diffuse_blue", PLY_UCHAR, PLY_UCHAR, offsetof(PlyVertex, blu), 0, 0, 0,
      0 },
    { "std_dev", PLY_FLOAT, PLY_FLOAT, offsetof(PlyVertex, confidence), 0, 0, 0,
      0 },
};
...
PlyProperty range_props[] = { { "vertex_indices", PLY_INT, PLY_INT,
                                offsetof(RangePnt, pts), 1, PLY_UCHAR,
                                PLY_UCHAR, offsetof(RangePnt, num_pts) }, };
...
int RangeGrid::readRangeGrid(const char *name) {
        ...
        if (equal_strings("vertex", elem_name)) {
            ...
            for (j = 0; j < num_elems; j++) {
                ...
                if (get_std_dev && is_warped) {
                    ...
                    if (get_intensity)
                        if (vert.intensity < RANGE_DATA_MIN_INTENSITY)
                            conf = 0.0;
...
Mesh *RangeGrid::toMesh(int subSamp, int hasTexture) {
    int i, j;
    int ii, jj;
    ...
    if (!isLinearScan) {
        for (j = _ltMin; j <= ltMax; j += subSamp) {
            ...
            if (in1 >= 0) {
                ...
                Pnt3 temp(mesh->nrm[3 * vin1], mesh->nrm[3 * vin1 + 1],
                          mesh->nrm[3 * vin1 + 2]);
                temp /= 32767;
                temp.normalize();
                temp *= 32767;
    ...
    if (isModelMakerScan)
        mesh->remove_stepedges(50, 8);
    else
        mesh->remove_stepedges(50, 4);

    return mesh;
}

void RangeGrid::addTriangleCheckNormal(Mesh *mesh, int vin1, int vin2, int vin3,
                                       Pnt3 &viewDir, float minDot) {
    float _dot = fabs(dot(
        viewDir, normal(mesh->Vtx(vin1), mesh->Vtx(vin2), mesh->Vtx(vin3))));
...
void RangeGrid::addTriangleCheckLength(Mesh *mesh, int vin1, int vin2, int vin3,
                                       float maxLength) {
...
int RangeGrid::readCyfile(const char *name) {
    int xx, yy;
    float x, y, z;
    int count;
    long range;
