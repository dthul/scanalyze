...
class GlobalReg {
  ...
  public:
    class mapEntry {
      ...
      public:
        ...
        mapEntry(TbObj *xa, TbObj *xb, const vector<Pnt3> &a,
                 const vector<Pnt3> &b, Xform<float> rxf, int max_pairs = 0)
            : xfa(xa), xfb(xb), ptsa(a), ptsb(b), manually_aligned(false),
              rel_xf(rxf) {
        ...
        int getQuality(void) {
            if (quality_grade > qual_Good)
                return qual_Good;
            if (quality_grade < qual_Unknown)
                return qual_Unknown;
    ...
    struct equal_tbobj {
        bool operator()(const TbObj *a, const TbObj *b) const { return a == b; }
    };

    struct hash_tbobj {
        size_t operator()(const TbObj *a) const { return (unsigned long)a; }
    };

  private:
    // unordered_multimap<Key, Data, HashFcn, EqualKey, Alloc>
    typedef unordered_multimap<TbObj *, mapEntry *, hash_tbobj, equal_tbobj>
    HMM;
    ...
    bool should_delete_point(Pnt3 pt, VolumeBucket *vb, int bucketInd);
    int init_vol_buckets(VolumeBucket **vb, Bbox worldBbox, TbObj *scanToMoveTo,
                         float bucket_side = 1.0);
    int find_bucket(Pnt3 pt, VolumeBucket *vb, float bucket_side,
                    int numBuckets);
    void get_pts(TbObj *x, vector<Pnt3> &P, vector<Pnt3> &Q,
                 TbObj *partner = NULL);
    void get_pts_within_group(TbObj *x, vector<Pnt3> &P, vector<Pnt3> &Q,
                              vector<TbObj *> &group);
    ...
    void move_back(const vector<TbObj *> &group, const vector<XF_F> &old_xf);
    ...
    bool getPairError(TbObj *a, TbObj *b, float &pointError, float &planeError,
                      float &globalError, bool &manual);
  ...
  public:
    ...
    // manipulate pairs
    mapEntry *addPair(TbObj *a, TbObj *b, const vector<Pnt3> &ap,
                      const vector<Pnt3> &nrma, const vector<Pnt3> &bp,
                      const vector<Pnt3> &nrmb,
                      Xform<float> rel_xf = Xform<float>(),
                      bool manually_aligned = false, int max_pairs = 0,
                      bool save = true, int saveQual = mapEntry::qual_Unknown);
    mapEntry *addPair(TbObj *a, TbObj *b, const vector<Pnt3> &ap,
                      const vector<Pnt3> &bp, float pw_point_rmsErr,
                      float pw_plane_rmsErr,
                      Xform<float> rel_xf = Xform<float>(),
                      bool manually_aligned = false, int max_pairs = 0,
                      bool save = true, int saveQual = mapEntry::qual_Unknown);
    ...
    void align(float ftol, TbObj *scanToMove = NULL,
               TbObj *scanToMoveTo = NULL);
    void align(float ftol, Bbox worldBbox, TbObj *scanToMove = NULL,
               TbObj *scanToMoveTo = NULL);

    void normalize_samples(Bbox worldBbox, TbObj *scanToMoveTo);

    // status and debugging info
    bool pairRegistered(TbObj *a, TbObj *b, bool &manual,
                        bool transitiveAllowed = false);
    int count_partners(TbObj *mesh);
    string list_partners(TbObj *mesh, bool transitiveAllowed = false);

    std::string dump_meshpairs(int choice, float listThresh = FLT_MAX);
    ...
    bool getPairingStats(TbObj *from, TbObj *to, // between these two meshes
                         bool &bManual, int &iQuality, int &nPoints,
                         time_t &date, float err[5]);
};
