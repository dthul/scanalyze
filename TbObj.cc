...
Pnt3 TbObj::worldCenter(void) {
    Pnt3 wctr = rot_ctr;
    xf(wctr);
    return wctr;
}

Bbox TbObj::worldBbox(void) { return bbox.worldBox(xf); }

float TbObj::radius(void) { return bbox.max_dist(rot_ctr); }
...
void TbObj::clear_undo(TbObj *objToRemove) {
    if (objToRemove) {
        int top = 0;
        for (int i = 0; i < real_size; i++) {
...
void TbObj::rotate(float q0, float q1, float q2, float q3, bool undoable) {
    if (undoable)
        save_for_undo();
    Pnt3 wc = worldCenter();
...
void TbObj::translate(float t0, float t1, float t2, bool undoable) {
    if (undoable)
        save_for_undo();
    xf.translate(t0, t1, t2);
}

void TbObj::new_rotation_center(const Pnt3 &wc) {
    // have to apply inverse transformation because
    // the rotation center is in local coordinates!
    xf.apply_inv(wc, rot_ctr);
}

Xform<float> TbObj::getXform(void) const { return xf; }

void TbObj::setXform(const Xform<float> &x, bool undoable) {
    if (undoable)
        save_for_undo();
    xf = x;
}

bool TbObj::readXform(istream &in) {
    save_for_undo();
    in >> xf;
    return (!in.fail());
}

bool TbObj::writeXform(ostream &out) {
    out << xf;
    return !out.fail();
}

void TbObj::resetXform(void) {
    save_for_undo();
    xf.identity();
}

bool TbObj::readXform(const string &baseFile) {
...
bool TbObj::writeXform(const string &baseFile) {
    string name = (baseFile + ".xf");
