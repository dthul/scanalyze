...
class RegistrationStatistics {
  public:
    ...
    bool test(RegistrationStatistics t) {
        return ((percentageOverlapping > t.percentageOverlapping) &&
    ...
    float getResult(int resultType) {
        switch (resultType) {
        case PERCENTAGE_OVERLAPPING:
            return percentageOverlapping;
        case PAIR_STABILITY:
        ...
        case RMS_POINT_TO_PLANE_ERROR:
            return -RMSPointToPlaneError;
        case RMS_POINT_TO_POINT_ERROR:
            return -RMSPointToPointError;
        }
        return 0;
    }
    ...
    void print() {
        if (discarded) {
            cout << "this pairing was discarded (overlapping area is to "
                    "small)\n";
            return;
        }
        ...
        sprintf(str1, "projections: %9.3f %9.3f %9.3f\n", mainDirProjections[3],
                mainDirProjections[2], mainDirProjections[1]);
        sprintf(str2, "             %9.3f           %9.3f\n",
                mainDirProjections[4], mainDirProjections[0]);
        sprintf(str3, "             %9.3f %9.3f %9.3f\n", mainDirProjections[5],
                mainDirProjections[6], mainDirProjections[7]);
        cout << str1 << str2 << str3 << "\n" << flush;
    }

    static void computeMainDirections(vector<Pnt3> pts, Pnt3 *directions,
                                      Pnt3 &centroid) {
        Pnt3 norm;
        float d;
