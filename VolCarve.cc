...
class CTL_memhandler {
  ...
  public:
    ...
    void store(CubeTreeLeafData *p) { free.push(p); }
};
...
void CubeTreeBase::set_leaf_depth(int ld) {
    if (ld >= 0)
        leafDepth = ld;
    else
        leafDepth = defaultLeafDepth;
}

CubeTreeBase *CubeTreeBase::find_neighbor(int face) {
    if (!parent)
        return NULL;
...
bool CubeTreeBase::is_neighbor_cell(int face, int &iChild) {
...
//   INDETERMINATE
//
OccSt view_query_status(vector<RigidScan *> &views,
                        vector<RigidScan *> &remaining_views, const Pnt3 &ctr,
                        float size, OccSt type = INDETERMINATE) {
...
float view_query_distance(vector<RigidScan *> &views, const Pnt3 &ctr,
                          float &distance) {
    float totalConf = 0;
    OccSt occ;
    Pnt3 closest;
...
void CubeTree::delete_children(void) {
    for (int i = 0; i < 8; i++) {
        delete child[i];
        child[i] = NULL;
    }
...
CubeTreeBase *CubeTree::recursive_find(int face, stack<int> &path,
                                       bool bReachedTop) {
    if (path.empty()) {
        /*
...
void CubeTree::release_space(int i) {
    if (i & 1) {
...
CubeTree::~CubeTree(void) { delete_children(); }

// recursively descend up to given level
// then if the neighboring cube is outside,
// create a face
void CubeTree::extract_faces(vector<int> &tri_inds) {
...
void CubeTree::keep_walls(bool x, bool y, bool z) {
    int l = 0;
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
            for (int k = 0; k < 2; k++, l++) {
...
void CubeTree::carve_help(vector<RigidScan *> &views, int levels,
                          vector<int> &tri_inds, float perc_min,
                          float perc_max) {
    ...
    type = ::view_query_status(views, remaining_views, mCtr, mSize + leafSize,
                               type);
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (levels > leafDepth) {
            ...
            ct->carve_help(remaining_views, levels, tri_inds, _perc_min,
                           _perc_min + perc_frac);
            cout << "Carved " << _perc_min + perc_frac << " percent" << endl;
        } else {
            CubeTreeLeaf *ctl =
                new CubeTreeLeaf(childCtr, childSize, this, i, views);
            child[i] = ctl;
...
void CubeTree::carve(vector<RigidScan *> &views, int levels,
                     vector<Pnt3> &coords, vector<int> &tri_inds) {
#ifdef VIS_TRIOCTREE
    CyraScan *cs = dynamic_cast<CyraScan *>(views[0]);
    if (cs)
        cs->TriOctreeMesh(coords, tri_inds);
    return;
#endif
    ...
    */
    ...
    for (int i = 0; i < coords.size(); i++) {
        if (isnanf(coords[i][0]) || isnanf(coords[i][1]) ||
            isnanf(coords[i][2])) {
            SHOW(i);
            SHOW(coords[i]);
            break;
...
CubeTreeLeaf::CubeTreeLeaf(const Pnt3 &c, float s, CubeTree *_par, int _i,
    ...
    vector<RigidScan *> remaining_views;
    type = ::view_query_status(views, remaining_views, c, s + leafSize);
...
CubeTreeLeaf::~CubeTreeLeaf(void) {
    // delete[] data;
    if (data)
        mem_handler.store(data);
}

static void cube_bdry(const Pnt3 &ctr, float size, vector<Pnt3> &p,
                      vector<int> &i) {
    float w = .5 * size;
    ...
    static int triinds[] = { 0, 1, 3, 2, 0, 3, 0, 4, 5, 0, 5, 1,
                             2, 6, 4, 2, 4, 0, 3, 5, 7, 3, 1, 5,
                             2, 7, 6, 2, 3, 7, 7, 4, 6, 7, 5, 4 };
...
void CubeTreeLeaf::extract_faces(vector<int> &tri_inds) {
    // if (!boundary()) return;
    if (!go_on())
        return;
    ...
    for (int i = 0; i < nVoxels; i++) {
        ...
        if (x == 0) {
            ...
            if (nborX && nborX->data) {
                vp[0] =
                    ((j = i + lenx - ofsy - ofsz) < 0 ? NULL : &nborX->data[j]);
        ...
        if (y == 0) {
            ...
            if (nborY && nborY->data) {
                vp[0] =
                    ((j = i + leny - ofsx - ofsz) < 0 ? NULL : &nborY->data[j]);
        ...
        if (nborX && nborY) { // then out in both x and y
            ...
            if (nborXY && nborXY->data) {
                vp[0] = ((j = i + lenx + leny - ofsz) < 0 ? NULL
                                                          : &nborXY->data[j]);
        ...
        if (vp[0]->distance > 0.0)
            edgeTableIndex |= 1;
        if (vp[1]->distance > 0.0)
            edgeTableIndex |= 2;
        if (vp[2]->distance > 0.0)
            edgeTableIndex |= 4;
        if (vp[3]->distance > 0.0)
            edgeTableIndex |= 8;
        if (vp[4]->distance > 0.0)
            edgeTableIndex |= 16;
        if (vp[5]->distance > 0.0)
            edgeTableIndex |= 32;
        if (vp[6]->distance > 0.0)
            edgeTableIndex |= 64;
        if (vp[7]->distance > 0.0)
            edgeTableIndex |= 128;
...
void CubeTreeLeaf::keep_walls(bool, bool, bool) {}

OccSt CubeTreeLeaf::neighbor_status(int face, int child) {
...
CubeTreeBase *CubeTreeLeaf::recursive_find(int face, stack<int> &path,
                                           bool bReachedTop) {
    return this;
}

CubeTreeLeaf *CubeTreeLeaf::find_neighbor(int face) {
    return dynamic_cast<CubeTreeLeaf *>(CubeTreeBase::find_neighbor(face));
}

bool CubeTreeLeaf::is_neighbor_cell(int face, int &child) {
