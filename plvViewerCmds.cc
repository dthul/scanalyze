...
int PlvSelectScanCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                     char *argv[]) {
    if (argc < 2) {
...
int PlvSetVisibleCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                     char *argv[]) {
    if (argc < 3) {
        Tcl_SetResult(interp, "Wrong number of args", TCL_STATIC);
        return TCL_ERROR;
...
int PlvGetVisibleCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                     char *argv[]) {
    if (argc < 2) {
        Tcl_SetResult(interp, "Wrong number of args", TCL_STATIC);
        return TCL_ERROR;
...
static void PlvListScansHelper(Tcl_Interp *interp,
                               vector<DisplayableMesh *> meshes, bool bLeaf);
// NEW! IMPROVED!!
int PlvListScansCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                    char *argv[]) {
    // options:
    ...
    if (argc > 2) {
        Tcl_SetResult(interp, "Too many arguments to PlvListScansCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }

    if (argc == 1)
        bRoot = true; // i.e. default mode
    ...
    if (bRoot) {
        for (vector<DisplayableMesh *>::iterator pdm =
...
static void PlvListScansHelper(Tcl_Interp *interp,
                               vector<DisplayableMesh *> meshes, bool bLeaf) {
    for (vector<DisplayableMesh *>::iterator pdm = meshes.begin();
         pdm < meshes.end(); pdm++) {
...
int PlvLightCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                char *argv[]) {
    ...
    if (argc == 4) {
        ...
        sprintf(result, "plv_light %f %f %f", theRenderParams->lightPosition[0],
...
int PlvRotateLightCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                      char *argv[]) {
    float x, y, z;
...
int PlvOrthographicCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                       char *argv[]) {
...
int PlvPerspectiveCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                      char *argv[]) {
...
int PlvZoomAngleCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                    char *argv[]) {
...
int PlvObliqueCameraCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                        char *argv[]) {
    if (argc > 2)
...
int PlvViewAllCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                  char *argv[]) {
    redraw(true);
    return TCL_OK;
}

int PlvResetXformCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                     char *argv[]) {
...
static void resetTranslationScale(int x, int y) {
    Pnt3 clickWorld, clickScreen;
    if (findZBufferNeighborExt(x, y, clickWorld, toglCurrent, tbView, 100,
                               &clickScreen, false)) {
...
static Pnt3 screenRotationCenter(void) {
    int x = theWidth / 2;
    int y = theHeight / 2;
...
static bool passesOnscreenTest(int oldSize = -1, bool bOnlyQuery = false) {
    ...
    TbObj tbObjIdentity;
    ScreenBox sb(&tbObjIdentity, xmargin, theWidth - xmargin, ymargin,
                 theHeight - ymargin);
...
static void passMoveToTrackball(int x, int y, int t) {
    Xform<float> before;
    int oldSize = -1;
    if (s_bForceOnscreen) {
...
int PlvRotateXYViewMouseCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                            char *argv[]) {
...
int PlvTransXYViewMouseCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                           char *argv[]) {
...
int PlvTranslateInPlaneCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                           char *argv[]) {
    if (argc < 3) {
...
int PlvUndoXformCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                    char *argv[]) {
    // tbView->undo_last();
    TbObj::undo();
    theScene->computeBBox();
...
int PlvRedoXformCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                    char *argv[]) {
    // tbView->undo_last();
    TbObj::redo();
    theScene->computeBBox();
...
int PlvGetScreenToWorldCoords(ClientData clientData, Tcl_Interp *interp,
                              int argc, char *argv[]) {
    if (argc != 3) {
        Tcl_SetResult(interp, "Bad arguments to PlvGetScreenToWorldCoords",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (ScreenToWorldCoordinates(x, y, coords)) {
        ...
        sprintf(answer, "Hit background; nearest point: %g %g %g", coords[0],
                coords[1], coords[2]);
    } else {
...
int PlvSetThisAsCenterOfRotation(ClientData clientData, Tcl_Interp *interp,
                                 int argc, char *argv[]) {
    if (argc != 3) {
        Tcl_SetResult(interp, "Bad arguments to PlvSetThisAsCenterOfRotation",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (findZBufferNeighbor(x, y, newcenter)) {
        ...
        Tcl_SetResult(
            interp, "We hit a background pixel. You must click on the mesh.\n",
            TCL_STATIC);
        return TCL_ERROR;
    }
}
    ...
    if (!strcmp(argv[1], "screen")) {
        ...
        if (theActiveScan != NULL) {
            Tcl_SetResult(interp,
                          "Don't use screen-centered rotation for moveMesh",
                          TCL_STATIC);
            return TCL_ERROR;
        }
        ...
        Tcl_SetResult(interp, "Must specify object|screen to PlvResetCoR",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
int PlvCameraInfoCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                     char *argv[]) {
    char transform[100];
...
int PlvSetHomeCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                  char *argv[]) {
    if (argc != 2) {
    ...
    if (0 == strcmp(argv[1], "")) {
        ...
        if (ms == NULL) {
            Tcl_SetResult(interp, "Missing meshset in PlvSetHomeCmd",
                          TCL_STATIC);
            return TCL_ERROR;
        }
    }
...
int PlvGoHomeCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                 char *argv[]) {
    if (argc != 2) {
    ...
    if (0 == strcmp(argv[1], "")) {
        ...
        ms = FindMeshDisplayInfo(argv[1]);
        if (ms == NULL) {
            Tcl_SetResult(interp, "Missing meshset in PlvGoHomeCmd",
                          TCL_STATIC);
            return TCL_ERROR;
        }
    }
...
int PlvSetOverallResCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                        char *argv[]) {
    if (argc != 2) {
        Tcl_SetResult(interp, "Bad argument to PlvSetOverallResCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
int PlvFlattenCameraXformCmd(ClientData clientData, Tcl_Interp *interp,
                             int argc, char *argv[]) {
    if (argc != 1) {
        Tcl_SetResult(interp, "Bad argument to PlvFlattenCameraXformCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
int PlvSpaceCarveCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                     char *argv[]) {
...
int PlvConstrainRotationCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                            char *argv[]) {
...
int PlvSetManipRenderModeCmd(ClientData clientData, Tcl_Interp *interp,
    ...
    if (argc != 7) {
        Tcl_SetResult(interp, "Bad args to PlvSetManipRenderModeCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
...
int PlvManualRotateCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                       char *argv[]) {
    Pnt3 axis;
...
int PlvManualTranslateCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                          char *argv[]) {
    Pnt3 tp;
...
int PlvPickScanFromPointCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                            char *argv[]) {
    if (argc < 2) {
        return TCL_ERROR;
    }
...
int PlvGetVisiblyRenderedScans(ClientData clientData, Tcl_Interp *interp,
    ...
    if (argc == 1) {
        ...
        if (theSel.type == Selection::rect) {
            ...
            Tcl_SetResult(interp, "Need to have a rectangle selected",
                          TCL_STATIC);
            return TCL_ERROR;
        }
    } else if (argc == 5) {
        ...
        // Usage
        Tcl_SetResult(interp, "Usage: plv_getVisiblyRenderedScans [xstart "
...
int PlvPositionCameraCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                         char *argv[]) {
    Pnt3 c, o, t;
    float q[4];
    float fov;
...
#define PNT3Contents(p) p[0], p[1], p[2]
    if (argc == 1) {
        ...
        sprintf(buf, "%g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g\n",
                PNT3Contents(c), PNT3Contents(o), PNT3Contents(t), q[0], q[1],
                q[2], q[3], fov, oblique_x, oblique_y);
        ...
        Pnt3 up(0, 1, 0);
        tbView->setup(c, o, up, theScene->sceneRadius(), fov, oblique_x,
                      oblique_y);
        tbView->setState(t, q);
...
int PlvSortScanListCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                       char *argv[]) {
    ...
    theScene->sortSceneMeshes(sortedList, criteria, bDictionary,
                              bListInvisible);
...
int PlvZoomToRectCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                     char *argv[]) {
    int x1, x2, y1, y2;
...
bool manipulatedRenderDifferent(void) {
    ...
    if (theRenderParams->bRenderManipsPoints ||
        ...
        char *pszNum = Tcl_GetVar(Togl_Interp(toglCurrent), "visPolyCount",
                                  TCL_GLOBAL_ONLY);
...
static void autoResetProc(ClientData clientData = NULL) {
    theScene->centerCamera();
...
static void rescheduleAutoReset(void) {
    ...
    if (s_iAutoResetTime) {
        s_autoResetTimerToken =
            Tk_CreateTimerHandler(s_iAutoResetTime, autoResetProc, NULL);
    }
}

int PlvForceKeepOnscreenCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                            char *argv[]) {
    if (argc < 2) {
