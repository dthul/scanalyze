...
static void SetResultFromColorU(Tcl_Interp *interp, uchar *color) {
    char buffer[20];
    sprintf(buffer, "#%02x%02x%02x", (int)color[0], (int)color[1],
            (int)color[2]);
    Tcl_SetResult(interp, buffer, TCL_VOLATILE);
}

static void SetResultFromColorF(Tcl_Interp *interp, float *color) {
    char buffer[20];
    sprintf(buffer, "#%02x%02x%02x", (int)(255 * color[0]),
            (int)(255 * color[1]), (int)(255 * color[2]));
    Tcl_SetResult(interp, buffer, TCL_VOLATILE);
}

int PlvDrawStyleCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                    char *argv[]) {
    if (g_bNoUI)
        return TCL_OK;
    ...
    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "-shademodel")) {
            ...
            if (!strcmp(argv[i], "realflat")) {
                ...
                Tcl_SetResult(interp, "bad arg to plv_drawstyle -shademodel",
                              TCL_STATIC);
                return TCL_ERROR;
            }
            ...
            if (!strcmp(argv[i], "fill")) {
                ...
                Tcl_SetResult(interp, "bad arg to plv_drawstyle -polymode",
                              TCL_STATIC);
                return TCL_ERROR;
            }
            ...
            if (i == argc) { // query
                ...
                theRenderParams->background[0] = 255 * atof(argv[i]);
                i++;
                theRenderParams->background[1] = 255 * atof(argv[i]);
                i++;
          ...
          SetBoolFromArgIndex (++i, theRenderParams->backFaceEmissive);
            ...
            if (i + 4 > argc) {
                Tcl_SetResult(interp,
                              "bad # args to plv_drawstyle -depthoffield",
                              TCL_STATIC);
                return TCL_ERROR;
            }
...
int PlvFillPhotoCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                    char *argv[]) {
    uchar *cbuf;
    GLint lastBuffer;
    ...
    if (argc != 3) {
        Tcl_SetResult(interp,
                      "Usage: plv_fillphoto <togl-widget> <photo-widget>",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    Tk_PhotoPutBlock(interp, handle, &block, 0, 0, theWidth, theHeight,
...
int PlvDrawCmd(ClientData clientData, Tcl_Interp *interp, int argc,
               char *argv[]) {
    prepareDrawInWin(argv[1]);
...
int PlvClearWinCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                   char *argv[]) {
    prepareDrawInWin(argv[1]);
...
int PlvMaterialCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                   char *argv[]) {
    if (g_bNoUI)
        return TCL_OK;
    ...
    if (argc == 1) {
        ...
        case confidenceColor:
            colorModeName = "confidence";
            break;
        case boundaryColor:
            colorModeName = "boundary";
            break;
        case intensityColor:
            colorModeName = "intensity";
            break;
        case trueColor:
            colorModeName = "true";
            break;
        case textureColor:
            colorModeName = "texture";
            break;
        case registrationColor:
            colorModeName = "registration";
            break;
        }
        ...
        for (i = 1; i < argc; i++) {
            if (!strcmp(argv[i], "-shininess")) {
                ...
                theRenderParams->specular[0] = 255 * atof(argv[i]);
                i++;
                theRenderParams->specular[1] = 255 * atof(argv[i]);
                i++;
                ...
                theRenderParams->diffuse[0] = 255 * atof(argv[i]);
                i++;
                theRenderParams->diffuse[1] = 255 * atof(argv[i]);
                i++;
            ...
            // backDiffuse added by kberg 6/5/01
                ...
                theRenderParams->backDiffuse[0] = 255 * atof(argv[i]);
                i++;
                theRenderParams->backDiffuse[1] = 255 * atof(argv[i]);
                i++;
                ...
                theRenderParams->backSpecular[0] = 255 * atof(argv[i]);
                i++;
                theRenderParams->backSpecular[1] = 255 * atof(argv[i]);
                i++;
                ...
                theRenderParams->lightAmbient[0] = atof(argv[i]);
                i++;
                theRenderParams->lightAmbient[1] = atof(argv[i]);
                i++;
                ...
                theRenderParams->lightSpecular[0] = atof(argv[i]);
                i++;
                theRenderParams->lightSpecular[1] = atof(argv[i]);
                i++;
                ...
                theRenderParams->lightDiffuse[0] = atof(argv[i]);
                i++;
                theRenderParams->lightDiffuse[1] = atof(argv[i]);
                i++;
                ...
                if (!strcmp(argv[i], "gray")) {
                    ...
                    Tcl_SetResult(interp, "Bad argument to plv_material -color",
                                  TCL_STATIC);
                    return TCL_ERROR;
                }
...
int PlvLoadProjectiveTexture(ClientData clientData, Tcl_Interp *interp,
    ...
    if (argc < 2) {
        ...
        printf("       green : Don't actually load image, just use a green "
               "projection\n");
        return TCL_OK;
    }
    ...
    Ref<ProjectiveTexture> thetexture = ProjectiveTexture::loadImage(
        argv[1], NULL, NULL, NULL, NULL, actuallyLoad, use_mipmaps);
    ...
    for (mesh = 0; mesh < nMeshes; mesh++)
        if (theScene->meshSets[mesh]->getVisible()) {
            ...
            const float *meshxform =
                (float *)(themesh->getMeshData()->getXform());
            themesh->setTexture(MeshAffixedProjectiveTexture::AffixToMesh(
                thetexture, meshxform));
        }
...
void resizeMainTogl(struct Togl *togl) {
...
void drawInToglBuffer(struct Togl *togl, int buffer, bool bCacheable) {
...
int PlvSetSlowPolyCountCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                           char *argv[]) {
    if (argc != 2) {
...
int PlvRenderThicknessCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                          char *argv[]) {
    Togl_MakeCurrent(toglCurrent);
...
int PlvLastRenderTime(ClientData clientData, Tcl_Interp *interp, int argc,
                      char *argv[]) {
