...
static void MakeMipmapsWithBorder(unsigned char *image_in, int width,
                                  int height) {
    ...
    while (nextwidth || nextheight) {
        ...
        gluScaleImage(GL_RGB, oldwidth, oldheight, GL_UNSIGNED_BYTE, image1,
                      nextwidth, nextheight, GL_UNSIGNED_BYTE, image2);
        ...
        glTexImage2D(GL_TEXTURE_2D, level, GL_RGB, nextwidth, nextheight, 0,
...
try_to_load_alignxf:
    ...
    ProjectiveTexture *thetexture = new ProjectiveTexture(
        thecamera, camcalibxf, camxf, alignxf, xscale, yscale);
    ...
    if (use_mipmaps) {
        ...
        for (int row = 0; row < tex_y; row++) {
            memcpy(tempimage + 3 * row * tex_x,
                   image + 3 * (xoff + (row + yoff) * width), 3 * tex_x);
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, tex_x, tex_y, 0, GL_RGB,
...
void ProjectiveTexture::setupTexture() {
    bind();
    ...
    GLfloat fudge[] = { 0.5, 0, 0,   0, 0,   0.5, 0, 0,
                        0,   0, 1.0, 0, 0.5, 0.5, 0, 1.0 };
    glMultMatrixf(fudge);
    // Scale...
...
void MeshAffixedProjectiveTexture::setupTexture() {
