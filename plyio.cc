...
void CopyReverseWord(void *dest, void *src, int nWords)
...
Mesh *mattReadPlyFile(char *filename) {
    /*
      int nelems;
      char **elist;
      char **obj_info;
...
int writePlyFile(char *filename, MeshSet *meshSet, int level,
                 int useColorNotTexture, int writeNormals) {
#if 1
    return meshSet->meshes[level]->writePlyFile(filename, useColorNotTexture,
                                                writeNormals, meshSet);
#else
    int i;
    int hasIntensity;
    int hasColor;
    ...
    ply.describe_element("vertex", mesh->numVerts, vert_props.size(),
                         &vert_props[0]);
    ...
    for (i = 0; i < mesh->numVerts; i++) {
        ...
        if (hasTexture && !useColorNotTexture) {
            ...
            uchar *col =
                &meshSet->texture[3 * (xOff + meshSet->texYdim * yOff)];
