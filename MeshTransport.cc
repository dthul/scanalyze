...
void MeshTransport::setVtx(const vector<Pnt3> *in, TransportMode mode,
...
void MeshTransport::setBbox(const Bbox &in) {
...
void MeshTransport::setNrm(const vector<short> *in, TransportMode mode) {
...
#pragma message("Normals will be floats! ")
    ...
    float *pOut;
    const short *pIn;
    for (pIn = in->begin(), pOut = out->begin(); pIn != in->end();
         pIn++, pOut++) {
        *pOut = *pIn / 32767.0;
    }
    nrm.push_back(out);
...
#ifdef SCZ_NORMAL_FORCEFLOAT
void MeshTransport::setNrm(const vector<float> *in, TransportMode mode) {
...
void MeshTransport::setTris(const vector<int> *in, TransportMode mode) {
    switch (mode) {
    case copy:
...
void MeshTransport::setColor(const vector<uchar> *in, TransportMode mode) {
    switch (mode) {
    case copy:
...
void MeshTransport::appendMT(MeshTransport *in, const Xform<float> &xfBy) {
