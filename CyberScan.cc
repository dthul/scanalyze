...
KDindtree *CyberScan::get_current_kdtree() {
    ...
    kdtree[iTree] = CreateKDindtree(level->pnts->data(), level->nrms->data(),
                                    level->pnts->size());

    return kdtree[iTree];
}

void CyberScan::read_postprocess(const char *filename) {
    ...
    if (!strcmp(
            Tcl_GetVar(g_tclInterp, "subsamplePreserveHoles", TCL_GLOBAL_ONLY),
            "filter")) {
        for (int i = 0; i < sweeps.size(); i++) {
...
CyberScan::CyberScan() { bDirty = false; }
...
MeshTransport *CyberScan::mesh(bool perVertex, bool stripped, ColorSource color,
                               int colorSize) {
    ...
    */
    ...
    for (int iTurn = 0; iTurn < sweeps.size(); iTurn++) {
        MeshTransport *turn =
            sweeps[iTurn]->mesh(perVertex, stripped, color, colorSize);
        if (turn) {
...
int CyberScan::num_vertices(void) { return getCurrentRegLevel()->pnts->size(); }

regLevelData *CyberScan::getHighestRegLevel(void) {
...
regLevelData *CyberScan::getCurrentRegLevel(void) {
    return getRegLevelFor(current_resolution_index());
}

regLevelData *CyberScan::getRegLevelFor(int iRes) {
    ...
    if (!reglevels[iRes]) {
        ...
        if (sweeps.size() == 1) {
            ...
            cout << "Merging sweep data (" << nPts << " pts) for KDtree... "
                 << flush;
            rl->pnts = new vector<Pnt3>;
            rl->pnts->reserve(nPts);
            rl->nrms = new vector<short>;
            rl->nrms->reserve(nPts);
            rl->bdry = new vector<char>;
            rl->bdry->reserve(nPts);
            rl->bFree = true;
            ...
            for (int i = 0; i < sweeps.size(); i++) {
                ...
                rl->pnts->insert(rl->pnts->end(), ld->pnts.begin(),
                                 ld->pnts.end());
                rl->nrms->insert(rl->nrms->end(), ld->nrms.begin(),
                                 ld->nrms.end());
                rl->bdry->insert(rl->bdry->end(), ld->bdry.begin(),
                                 ld->bdry.end());
...
void CyberScan::subsample_points(float rate, vector<Pnt3> &p, vector<Pnt3> &n) {
    regLevelData *level = getCurrentRegLevel();

    int end = level->pnts->size();
    p.clear();
    p.reserve(end * rate * 1.1);
    n.clear();
    n.reserve(end * rate * 1.1);
    for (int i = 0; i < end; i++) {
        if (rnd() <= rate) {
...
RigidScan *CyberScan::filtered_copy(const VertexFilter &filter) {
    BailDetector bail;
...
RigidScan *CyberScan::get_piece(int sweepNumber, int frameStart,
                                int frameFinish) {
...
bool CyberScan::filter_inplace(const VertexFilter &filter) { return false; }

bool CyberScan::filter_vertices(const VertexFilter &filter, vector<Pnt3> &p) {
    for (int i = 0; i < sweeps.size(); i++) {
...
bool CyberScan::closest_point(const Pnt3 &p, const Pnt3 &n, Pnt3 &cp, Pnt3 &cn,
    ...
    regLevelData *level = getCurrentRegLevel();
    ans =
        tree->search(level->pnts->data(), level->nrms->data(), p, n, ind, thr);
    if (ans) {
        if (bdry_ok == 0) {
            // disallow closest points that are on the mesh boundary
            if (level->bdry->operator[](ind))
                return 0;
        }
...
void CyberScan::flipNormals(void) {
    for (int i = 0; i < sweeps.size(); i++)
        sweeps[i]->flipNormals();
}

void CyberSweep::flipNormals(void) {
    for (int i = 0; i < levels.size(); i++) {
...
bool CyberScan::read(const string &fname) {
    cout << "cs::read()" << endl;
    set_name(fname);
...
bool CyberScan::is_modified(void) { return bDirty; }

bool CyberScan::write(const string &fname) {
...
bool CyberScan::load_resolution(int iRes) {
    if (resolutions[iRes].in_memory)
        return true;

    int nSweeps = sweeps.size();
    if (g_verbose)
        cout << name << ": create mesh (~" << resolutions[iRes].abs_resolution
             << ") from " << nSweeps << " sweeps: " << flush;
...
int CyberScan::create_resolution_absolute(int budget, Decimator dec) {
    cerr << "Can't decimate CyberScan meshes for now" << endl;
    return 0;
}

bool CyberScan::release_resolution(int nPolys) {
...
void CyberSweep::init_leveldata(void) {
    levels.clear();
    levels.reserve(SUBSAMPS);
    kdtree.clear();
    kdtree.reserve(SUBSAMPS);
    for (int i = 0; i < SUBSAMPS; i++) {
...
void CyberSweep::insert_possible_resolutions(void) {
    for (int i = 0; i < SUBSAMPS; i++) {
...
bool CyberSweep::read(const string &fname) {
    if (!sd.read(fname))
        return false;
...
bool CyberSweep::write(const string &fname) { return sd.write(fname); }

// called by CyberScan::load_resolution(),
// which again is inherited from ResolutionCtrl
bool CyberSweep::load_resolution(int iRes) {
    if (resolutions[iRes].in_memory)
        return true;
    ...
    if (iRes == 0) {
        ...
        sd.get_pnts_and_intensities(levels[0]->pnts, levels[0]->intensity);
        ...
        sd.subsampled_tstrip(
            step,
            Tcl_GetVar(g_tclInterp, "subsamplePreserveHoles", TCL_GLOBAL_ONLY),
            level->tstrips, level->bdry, level->pnts, level->intensity,
            level->confidence, level->map_sampled_to_unsampled);
    }
    ...
    if (level->pnts.size()) {
        if (strcmp(Tcl_GetVar(g_tclInterp, "removeStepedges", TCL_GLOBAL_ONLY),
                   "0") != 0) {
        ...
        // cout << "normals... " << flush;
        getVertexNormals(level->pnts, level->tstrips, true, level->nrms, false);

        resolutions[iRes].abs_resolution = count_tris(levels[iRes]->tstrips);
    } else {
 ...
 */
void CyberSweep::simulateFaceNormals(vector<short> &faceNormals,
                                     int currentRes) {
    ...
    for (vert = levels[currentRes]->tstrips.begin();
         vert != levels[currentRes]->tstrips.end(); vert++) {
        while (*vert == -1) { // handle 0-length strips
            ++vert;
            if (vert == levels[currentRes]->tstrips.end())
                break;
        }
...
MeshTransport *CyberSweep::mesh(bool perVertex, bool stripped,
    ...
    if (perVertex)
        ...
        /* used GenericScan::mesh(...) as an example of how to simulate this.
           ...
           is displayed when intensity is selected.  It should have something to
           do
...
vector<CyberSweep *> CyberScan::get_sweep_list(void) { return sweeps; }

// double indexing: horizontal translation, turns
vector<vector<CyberSweep *> > CyberScan::get_ordered_sweeps(void) {
    ...
    for (int i = 0; i < sweeps.size(); i++) {
        ...
        for (iTrans = 0; iTrans < v.size(); iTrans++) {
            ...
            if (t <= tc) {
                if (t < tc)
                    v.insert(v.begin() + iTrans, vector<CyberSweep *>());
                break;
            }
        }
        if (iTrans == v.size())
            v.push_back(vector<CyberSweep *>());
...
void CyberScan::dump_pts_laser_subsampled(std::string fname, int nPnts) {
    // open the fstream
...
bool CyberScan::get_raw_data(const Pnt3 &p, sd_raw_pnt &data) {
    ...
    int ind;
    float thr = .1;
    if (!kdtree[lvl]->search(reglevels[lvl]->pnts->data(), p, ind, thr)) {
...
// returns confidence!
float CyberScan::closest_point_on_mesh(const Pnt3 &p, Pnt3 &cl_pnt,
                                       OccSt &status_p) {
    // cheat...
...
// returns confidence!
float CyberScan::closest_along_line_of_sight(const Pnt3 &p, Pnt3 &cp,
                                             OccSt &status_p) {
...
OccSt CyberScan::carve_cube(const Pnt3 &ctr, float side) {
    ...
    while (carveStack.size() && !carveStack.back().can_be_child(ctr, side)) {
        carveStack.pop_back();
    }
    ...
    for (int i = 0; i < sw.size(); i++) {
        ...
        if (tmp == BOUNDARY || tmp == SILHOUETTE || tmp == INDETERMINATE) {
...
bool CyberScan::switchToResLevel(int iRes) {
...
string CyberSweep::get_description(void) const {
    static char name[100];
    sprintf(name, "trans-%.2d-rot-%.2d", (int)sd.scanner_trans,
            (int)sd.other_screw);
    return string(name);
}

KDindtree *CyberSweep::get_current_kdtree() {
    int iTree = current_resolution_index();
    assert(iTree < kdtree.size());
    if (kdtree[iTree] != NULL)
        return kdtree[iTree];
...
void CyberSweep::subsample_points(float rate, vector<Pnt3> &p,
                                  vector<Pnt3> &n) {
    levelData *level = levels[current_resolution_index()];
...
RigidScan *CyberSweep::filtered_copy(const VertexFilter &filter) {
...
bool CyberSweep::closest_point(const Pnt3 &p, const Pnt3 &n, Pnt3 &cp, Pnt3 &cn,
                               float thr, bool bdry_ok) {
    KDindtree *tree = get_current_kdtree();
    if (!tree)
        return false;
    ...
    levelData *level = levels[current_resolution_index()];
    ans = tree->search(&level->pnts[0], &level->nrms[0], p, n, ind, thr);
    if (ans) {
        if (bdry_ok == 0) {
            // disallow closest points that are on the mesh boundary
            if (level->bdry[ind])
        ...
        cp = level->pnts[ind];
        ind *= 3;
        cn.set(level->nrms[ind] / 32767.0, level->nrms[ind + 1] / 32767.0,
               level->nrms[ind + 2] / 32767.0);
    }
    return ans;
}

string CyberScan::getInfo(void) {
    long verts = 0;
    long tris = 0;
    for (int i = 0; i < sweeps.size(); i++) {
    ...
    if (sweeps.size() == 1) {
        ...
        float turnMin = FLT_MAX;
        float transMin = FLT_MAX;
        float turnMax = -FLT_MAX;
        float transMax = -FLT_MAX;
        int lastShell = -1000;
        int nShells = 0;
...
unsigned int CyberScan::get_scanner_config(void) {
    return sweeps[0]->sd.scanner_config;
}

float CyberScan::get_scanner_vertical(void) {
...
//
bool CyberScan::worldCoordToSweepCoord(const Pnt3 &wc, int *sweepIndex,
                                       Pnt3 &sc) {
...
// sweep coordinates (screw length, y, and z for a specific sweep).
//
void CyberScan::sweepCoordToWorldCoord(int sweepIndex, const Pnt3 &sc,
                                       Pnt3 &wc) {
    CyberXform cxf;
...
RigidScan *CyberSweep::get_piece(int firstFrame, int lastFrame) {
    // Does firstFrame have to be even???
    if (IS_ODD(firstFrame)) {
        cout << "CyberSweep::get_piece() Not tested with odd start frames!!"
             << endl;
    }

    if (firstFrame < 0 || lastFrame >= sd.num_frames() ||
...
// return confidence!
float CyberSweep::closest_along_line_of_sight(const Pnt3 &p, Pnt3 &cp,
                                              OccSt &status_p) {
    Pnt3 bp;
    if (!sd.xf.back_project(p, bp)) {
...
OccSt CyberSweep::carve_sphere(const Pnt3 &ctr, float r) {
