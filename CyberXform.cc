...
#define TURN_SCREW_TO_ANGLE(scr)                                               \
    (acos((cc.t_h2 - ((scr) + cc.t_h1) * ((scr) + cc.t_h1)) * cc.t_h3) -       \
     cc.t_h4)

#define NOD_SCREW_TO_ANGLE(scr)                                                \
    (acos((cc.n_h2 - ((scr) + cc.n_h1) * ((scr) + cc.n_h1)) * cc.n_h3) -       \
     cc.n_h4)

#define SCREW_TO_ANGLE(scr)                                                    \
    (acos((_h2 - ((scr) + _h1) * ((scr) + _h1)) * _h3) - _h4)

#define ANGLE_TO_SCREW(ang) (sqrtf(_h2 - cos(ang + _h4) / _h3) - _h1)
...
Pnt3 CyberXform::laser_ctr_in_laser_coords(void) {
...
void CyberXform::bounds_for_circle_on_laser_in_raw(float ly, float lz, float r,
                                                   short ywin[2],
                                                   short zwin[2]) {
...
void CyberXform::setup(bool vscan, bool tup, int tconf, bool tright,
    ...
    // remove offset
    raw_to_laser.translate(0.0, -(2 * CorrRowOffset), -CorrZOffset);
    // scale to [0..scale]
    ...
    if (vertical_scan) {
        ...
        ax0[1] = cc.axnod0[1];
        ax0[2] = cc.axnod0[2];
        scanaxis_to_horz.rot(TURN_SCREW_TO_ANGLE(screw), cc.axturn[0],
                             cc.axturn[1], cc.axturn[2], cc.axturn0[0],
                             cc.axturn0[1], cc.axturn0[2]);
    } else {
        _h1 = cc.t_h1;
        _h2 = cc.t_h2;
        _h3 = cc.t_h3;
        ...
        laser_to_scanaxis.rot(NOD_SCREW_TO_ANGLE(screw), cc.axnod[0],
                              cc.axnod[1], cc.axnod[2], cc.axnod0[0],
                              cc.axnod0[1], cc.axnod0[2]);
    }
...
void CyberXform::set_screw(float even_scr, float odd_scr) {
    odd_xform = even_xform = raw_to_scanaxis;
    even_xform.rot(SCREW_TO_ANGLE(even_scr), axdir[0], axdir[1], axdir[2],
                   ax0[0], ax0[1], ax0[2]);
    odd_xform.rot(SCREW_TO_ANGLE(odd_scr), axdir[0], axdir[1], axdir[2], ax0[0],
                  ax0[1], ax0[2]);
    ...
    blah.rot(SCREW_TO_ANGLE(even_scr), axdir[0], axdir[1], axdir[2], ax0[0],
             ax0[1], ax0[2]);
...
void CyberXform::set_screw(float even_scr) {
...
Xform<double> CyberXform::set_and_get_geometric_xform(float screw) {
    Xform<double> xf = laser_to_scanaxis;
    xf.rot(SCREW_TO_ANGLE(screw), axdir[0], axdir[1], axdir[2], ax0[0], ax0[1],
           ax0[2]);
    xf *= scanaxis_to_horz;
    return xf;
}

Pnt3 CyberXform::apply_xform(short y, short z) {
    if (y % 2) {
        double invw = 1.0 / (odd_xf[7] * y + odd_xf[11] * z + odd_xf[15]);
        ...
        double invw = 1.0 / (even_xf[7] * y + even_xf[11] * z + even_xf[15]);
...
void CyberXform::apply_xform(short y, short z, Pnt3 &p) {
    if (y % 2) {
        double invw = 1.0 / (odd_xf[7] * y + odd_xf[11] * z + odd_xf[15]);
...
// used by CyberTurn...
Pnt3 CyberXform::apply_raw_to_laser(short y, short z) {
    Pnt3 p(0, y, z);
    raw_to_laser(p);
    return p;
}
...
float CyberXform::axis_project(short y, short z) {
    Pnt3 p(0, y, z);
    raw_to_scanaxis(p);
    return ((p[0] - ax0[0]) * axdir[0] + (p[1] - ax0[1]) * axdir[1] +
            (p[2] - ax0[2]) * axdir[2]);
}
...
//  but not on the side)
bool CyberXform::back_project(const Pnt3 &p_in, Pnt3 &p_out,
                              bool check_frustum) {
    ...
    // * project the point p onto the scan axis (call that pa)
    axis_proj_pos = ((p[0] - ax0[0]) * axdir[0] + (p[1] - ax0[1]) * axdir[1] +
                     (p[2] - ax0[2]) * axdir[2]);
    // within frustum?
    if (check_frustum &&
        (axis_proj_pos < axislimit_min || axis_proj_pos > axislimit_max))
        return false;
    double pa[3], r[3];
    ...
    double alpha = (n_dot_t - (laser_n[0] * pa[0] + laser_n[1] * pa[1] +
                               laser_n[2] * pa[2])) /
                   n_dot_q;
    ...
    p[1] =
        (toYZ[1] * rp[0] + toYZ[5] * rp[1] + toYZ[9] * rp[2] + toYZ[13]) * tmp;
    p[2] =
        (toYZ[2] * rp[0] + toYZ[6] * rp[1] + toYZ[10] * rp[2] + toYZ[14]) * tmp;

    // get the screw
    scan_angle = acos((alpha * (r[0] * q[0] + r[1] * q[1] + r[2] * q[2]) +
                       beta * (r[0] * s[0] + r[1] * s[1] + r[2] * s[2])) /
                      r2);
    // try to deal with slightly negative angles
    if (scan_angle > .75 * M_PI)
        scan_angle -= M_PI;
...
int CyberXform::sphere_status(const Pnt3 &ctr, float radius, float &screw,
                              float ax_min, float ax_max, short ywin[2],
                              short zwin[2], float swin[2]) {
    ...
    if (axis_proj_pos < ax_min - radius || axis_proj_pos > ax_max + radius) {
    ...
    bounds_for_circle_on_laser_in_raw(lzr[1], lzr[2], radius, ywin, zwin);
