...
CyraScan::CyraScan(void) : triKD(NULL), triKD_mesh(NULL) {
    // Clear arrays
    levels.clear();
}
...
MeshTransport *CyraScan::mesh(bool perVertex, bool stripped, ColorSource color,
                              int colorSize) {
...
int CyraScan::num_tris(int resNum) {
    if (resNum < levels.size()) {
...
void CyraScan::computeBBox(void) {
...
void CyraScan::flipNormals(void) {
    // Flip all res levels
...
bool CyraScan::ReadPts(const string &inname) {
    ...
    for (int i = 1; i < 6; i++) {
        ...
        cerr << "Telling scanalyze about approx. " << estimate << " tris."
             << endl;
    }
...
bool CyraScan::WritePts(const string &inname) {
    // write out the highest level to the .pts file
    return (levels[0].WritePts(inname));
}

bool CyraScan::load_resolution(int iRes) {
...
bool CyraScan::read(const string &inname) {
    set_name(inname);
...
bool CyraScan::write(const string &fname) {
...
bool CyraScan::filter_inplace(const VertexFilter &filter) {
    // Filter each level
    for (int i = 0; i < levels.size(); i++) {
...
RigidScan *CyraScan::filtered_copy(const VertexFilter &filter) {
    ...
    for (int i = 0; i < newScan->levels.size(); i++) {
        ...
        cerr << "Telling scanalyze about " << level->num_tris() << " tris."
             << endl;
    }
...
// for ICP...
void CyraScan::subsample_points(float rate, vector<Pnt3> &p, vector<Pnt3> &n) {
    CyraResLevel &res = levels[curr_res];
    res.subsample_points(rate, p, n);
}

bool CyraScan::closest_point(const Pnt3 &p, const Pnt3 &n, Pnt3 &cp, Pnt3 &cn,
...
float CyraScan::closest_point_on_mesh(const Pnt3 &p, Pnt3 &cl_pnt,
                                      OccSt &status_p) {
    ...
    d = sqrtf(d);
    triKD->search(triKD_mesh->vtx[0]->data(), triKD_mesh->tri_inds[0]->data(),
                  p, cl_pnt, d);
    prev_point = cl_pnt;
...
float CyraScan::closest_along_line_of_sight(const Pnt3 &p, Pnt3 &cp,
                                            OccSt &status_p) {
    ...
    int ofss[4] = { t + l * res.height, t + r * res.height, b + l * res.height,
                    b + r * res.height };
...
bool OriginRaySphereIntersect(const Pnt3 &rayD, const Pnt3 &center,
...
struct carve_cache {
    ...
    bool inside(const Pnt3 &p, float r) { return dist(p, ctr) + r <= radius; }

#if _MSC_VER == 1100
    // this compiler (MSVC5) won't compile without being able to compare
    // carve_caches!
...
OccSt CyraScan::carve_cube(const Pnt3 &ctr, float side) {
    build_vrip_accelerators();
    ...
    if (!first)
        oldinds = hist.top().pinds;
    hist.push(cc);
    if (first)
        hist.top().pinds.reserve(numpoints);
    else
        hist.top().pinds.reserve(oldinds.size());
#endif
...
// leave the rest of the points into cache
        ...
        cout << oldinds.size() << ": ";
        for (vector<int>::iterator it = oldinds.begin(); it != oldinds.end();
             it++) {
            float mag = pntmag[*it];
            if (!mag)
                continue;
            ...
            if (OriginRaySphereIntersect(dir, ctr, side, t1, t2)) {
                ...
                if (mag < t1)
                    ...
                    // SHOW (p); SHOW (t1); SHOW (mag); SHOW (t2);
                    cout << "BOUND for point " << *it << " of " << numpoints
                         << endl;
                    // leave the rest of the points into cache
                    hist.top().pinds.insert(hist.top().pinds.end(), ++it,
                                            oldinds.end());
                    return BOUNDARY;
                }
            }
...
void CyraScan::build_vrip_accelerators(void) {
    CyraResLevel &res = levels[0];
