...
void getVertexNormals(const vector<Pnt3> &vtx, const vector<int> &tri,
                      bool strips, vector<short> &nrm_s, int useArea) {
    ...
    if (!useArea) {
        if (strips) {
            ...
            for (int i = 0; i < ntris; i += 3) {
                Pnt3 norm =
                    normal(vtx[tri[i + 0]], vtx[tri[i + 1]], vtx[tri[i + 2]]);
                nrm[tri[i + 0]] += norm;
                nrm[tri[i + 1]] += norm;
                nrm[tri[i + 2]] += norm;
            }
        ...
        if (strips) {
            ...
            for (int i = 0; i < ntris; i += 3) {
                ...
                */
                Pnt3 areaNorm =
                    cross(vtx[tri[i + 0]], vtx[tri[i + 1]], vtx[tri[i + 2]]);
...
float median_edge_length(vector<Pnt3> &vtx, vector<int> &tri, bool strips,
                         int percentile) {
    int n = tri.size();
    if (n < 3)
        return 0.0;
...
void remove_stepedges(vector<Pnt3> &vtx, vector<int> &tri, int factor,
                      int percentile, bool strips) {
            ...
            if (strip_on) {
                ...
                if (end_here) {
                    if (cnt % 2) {
                        ...
                        if (i > n - 4)
                            break; // almost done, break...
                        if (tri[i + 1] != -1 && tri[i + 2] != -1 &&
...
void remove_unused_vtxs(vector<Pnt3> &vtx, vector<int> &tri) {
...
int count_tris(const vector<int> &strips) {
    int cnt = 0;
    int i = 0;
    while (i < strips.size()) {
...
struct _edge {
    ...
    bool operator==(const _edge &e) const { return a == e.a && b == e.b; }
};
...
struct hash_edge {
    size_t operator()(const _edge &e) const { return e.a * 1000000 + e.b; }
};

static void remove_this_triangle(vector<int> &ts, int i) {
    for (int j = 0; j < 3; j++) {
        int curr = ts[i + j]; // current neighbor
        if (curr >= 0) {
            if (ts[curr] == -2)
                continue; // nbor has been removed
...
static void old_tris_to_strips(const vector<int> &tris, vector<int> &tstripinds,
                               const char *name) {
    int nTris = tris.size();
    tstripinds.clear();
    ...
    cout << "Creating triangle strips from raw triangle data..." << endl;
    if (Verbose)
    ...
    for (i = 0; i < nNbors; i += 3) {
        ...
        if (nbor == -1) {
            ...
            while (curr != -1) {
                ...
                if (tri_nbors[curr] == prev)
                    nbor = 0;
                else if (tri_nbors[curr + 1] == prev)
                    nbor = 1;
                else if (tri_nbors[curr + 2] == prev)
                    nbor = 2;
                else
                    printf("Warning: really lost!\n");
...
adjacentfacelist *TriMesh_FindAdjacentFaces(int numvertices,
...
#define FOR_EACH_VERTEX_OF_FACE(i, j)                                          \
    for (int jtmp = 0, j = faces[i]; jtmp < 3; jtmp++, j = faces[i + jtmp])

#define FOR_EACH_ADJACENT_FACE(i, j)                                           \
    for (int jtmp = 0, j = adjacentfaces[i][0]; jtmp < numadjacentfaces[i];    \
...
static inline int Tstrip_Next_Tri(unsigned tri, unsigned v1, unsigned v2) {
...
static void Tstrip_Crawl(unsigned v1, unsigned v2, unsigned v3, unsigned next) {
    ...
    *stripsp++ = v3;
    ...
    do {
        ...
        if ((faces[next + 0] == vlast2) && (faces[next + 1] == vlast1) &&
            (faces[next + 2] == vnext))
            thisflipped = false;
        if ((faces[next + 2] == vlast2) && (faces[next + 0] == vlast1) &&
            (faces[next + 1] == vnext))
            thisflipped = false;
        if ((faces[next + 1] == vlast2) && (faces[next + 2] == vlast1) &&
            (faces[next + 0] == vnext))
            thisflipped = false;
...
static unsigned *Build_Tstrips(int numvertices, const vector<int> &tris,
                               unsigned *&endstrips, int &outNstrips) {
    adjacentfaces =
        TriMesh_FindAdjacentFaces(numvertices, tris, numadjacentfaces);
...
void tris_to_strips(int numvertices, const vector<int> &tris,
                    vector<int> &tstripinds, const char *name) {
#if 0
  old_tris_to_strips (tris, tstripinds, name);
#else
    cout << "Tstripping " << tris.size() / 3 << " triangles (" << tris.size()
         << " vertices)..." << endl;
...
void strips_to_tris(const vector<int> &tstrips, vector<int> &tris, int nTris) {
    if (!tstrips.size())
        return;
...
void flip_tris(vector<int> &inds, bool stripped) {
    int n = inds.size();
...
#define BEFORE(mod, var) ((mod == 0) ? tris[var + 2] : tris[var - 1])

#define AFTER(mod, var) ((mod == 2) ? tris[var - 2] : tris[var + 1])

// sort a ring of neighbors

static void sort_nbors(int *nbors, int n, const vector<int> &tris) {
    for (int i = 0; i < n; i++) {
...
static void nbors_tris(int n_vtx, const vector<int> &tris,
...
// zeroes
void mark_boundary_verts(vector<char> &bdry, const vector<int> &tris) {
...
static void build_nbor_map(vecvec &nlist, const vector<int> &tris) {
    // build an edge set
...
// assume bdry has the right size and has been initialized with
// zeroes
void distance_from_boundary(vector<float> &distances, const vector<Pnt3> &pnts,
                            const vector<int> &tris) {
    TIMER(dist);
        ...
        for (hcit = workset.begin(); hcit != workset.end(); hcit++) {
            ...
            for (int j = 0; j < n_nbors[i]; j++) {
                ...
                if (d < distances[i])
                    distances[i] = d;
                // should neighbor go to the nextset?
                if (workset.find(k) != workset.end())
                    continue;
                if (prevset.find(k) != prevset.end())
                    continue;
                nextset.insert(k);
                d = dij + distances[i];
                if (d < distances[k])
...
void quadric_simplify(const vector<Pnt3> &vtx_in, const vector<int> &tri_in,
                      vector<Pnt3> &vtx_out, vector<int> &tri_out, int goal,
                      int optLevel, float errLevel, float boundWeight) {
    ...
    if (!tmpnam(inName)) {
        cerr << "ERROR: Unable to get temporary filename for QSlim input"
             << endl;
        return;
    }
    if (!(f = fopen(inName, "w"))) {
    ...
    char qslimCmd[200];
    sprintf(qslimCmd, "qslim -t %d -B %f -o %s %s", goal, boundWeight, outName,
            inName);
    if (system(qslimCmd)) {
...
static PlyProperty vert_prop_x = { "x", PLY_FLOAT,      PLY_FLOAT,      0,
                                   0,   PLY_START_TYPE, PLY_START_TYPE, 0 };
static PlyProperty vert_prop_y = { "y", PLY_FLOAT,      PLY_FLOAT,      0,
                                   0,   PLY_START_TYPE, PLY_START_TYPE, 0 };
static PlyProperty vert_prop_z = { "z", PLY_FLOAT,      PLY_FLOAT,      0,
                                   0,   PLY_START_TYPE, PLY_START_TYPE, 0 };
static PlyProperty vert_prop_nx = { "nx", PLY_FLOAT,      PLY_FLOAT,      0,
                                    0,    PLY_START_TYPE, PLY_START_TYPE, 0 };
static PlyProperty vert_prop_ny = { "ny", PLY_FLOAT,      PLY_FLOAT,      0,
                                    0,    PLY_START_TYPE, PLY_START_TYPE, 0 };
static PlyProperty vert_prop_nz = { "nz", PLY_FLOAT,      PLY_FLOAT,      0,
                                    0,    PLY_START_TYPE, PLY_START_TYPE, 0 };
static PlyProperty vert_prop_intens = {
    "intensity", PLY_FLOAT, PLY_FLOAT, 0, 0, PLY_START_TYPE, PLY_START_TYPE, 0
};
static PlyProperty vert_prop_conf = { "confidence",   PLY_FLOAT, PLY_FLOAT,
                                      0,              0,         PLY_START_TYPE,
                                      PLY_START_TYPE, 0 };
...
static PlyProperty vert_prop_diff_g = { "diffuse_green", PLY_UCHAR,
                                        PLY_UCHAR,       0,
                                        0,               PLY_START_TYPE,
                                        PLY_START_TYPE,  0 };
static PlyProperty vert_prop_diff_b = { "diffuse_blue", PLY_UCHAR,
                                        PLY_UCHAR,      0,
                                        0,              PLY_START_TYPE,
                                        PLY_START_TYPE, 0 };

static PlyProperty face_props[] = { { "vertex_indices", PLY_INT, PLY_INT, 0,
                                      true, PLY_UCHAR, PLY_UCHAR, 0 }, };
...
static PlyProperty tstrips_props[] = { { "vertex_indices", PLY_INT, PLY_INT, 4,
                                         true, PLY_INT, PLY_INT, 0 }, };
...
static void write_ply_file(const char *fname, const vector<Pnt3> &vtx,
                           const vector<int> &tris, bool strips,
                           const vector<Pnt3> &nrm,
                           const vector<uchar> &intensity,
                           const vector<float> &confidence, bool hasNrm,
                           bool hasIntensity, bool hasConfidence) {
    int i;
    ...
    ply.describe_element("vertex", vtx.size(), vert_props.size(),
                         &vert_props[0]);
    if (strips)
...
void write_ply_file(const char *fname, const vector<Pnt3> &vtx,
                    const vector<int> &tris, bool strips,
                    const vector<Pnt3> &nrm, const vector<uchar> &intensity,
                    const vector<float> &confidence) {
    write_ply_file(fname, vtx, tris, strips, nrm, intensity, confidence, true,
                   true, true);
}
...
void write_ply_file(const char *fname, const vector<Pnt3> &vtx,
    ...
    vector<uchar> intensity;
    vector<float> confidence;
    write_ply_file(fname, vtx, tris, strips, nrm, intensity, confidence, false,
                   false, false);
}
...
void write_ply_file(const char *fname, const vector<Pnt3> &vtx,
                    const vector<int> &tris, bool strips,
                    const vector<Pnt3> &nrm, const vector<float> &confidence) {
    vector<uchar> intensity;
    write_ply_file(fname, vtx, tris, strips, nrm, intensity, confidence, true,
                   false, true);
}

void pushNormalAsShorts(vector<short> &nrms, Pnt3 n) {
    n *= 32767;
    nrms.push_back(n[0]);
    nrms.push_back(n[1]);
    nrms.push_back(n[2]);
...
void pushNormalAsPnt3(vector<Pnt3> &nrms, short *n, int i) {
    i *= 3;
