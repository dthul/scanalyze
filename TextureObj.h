...
#define DEFAULT_CAMERA_FILENAME                                                \
    "/usr/graphics/lib/plyshade/pastecolor/dmich.cameraparams"
#define DEFAULT_CAMCALIBXF_FILENAME                                            \
    "/usr/graphics/lib/plyshade/pastecolor/dmich.cameracalib.xf"
...
class GL_Bound_TextureObj : public TextureObj {
  ...
  protected:
    void bind() { glBindTexture(GL_TEXTURE_2D, textureobjnum); }
...
class ProjectiveTexture : public GL_Bound_TextureObj {
  ...
  protected:
    ProjectiveTexture(CameraParams *_cam, Xform<float> *_camcalibxf = NULL,
                      Xform<float> *_camxf = NULL,
                      Xform<float> *_alignxf = NULL, float _xscale = 1.0,
                      float _yscale = 1.0)
        : camcalibxf(_camcalibxf), camxf(_camxf), alignxf(_alignxf),
          thecamera(_cam), xscale(_xscale), yscale(_yscale) {}

  public:
    static Ref<ProjectiveTexture> loadImage(
        const char *filename, const char *camfilename = NULL,
        const char *camcalibxffilename = NULL, const char *camxffilename = NULL,
        const char *alignxffilename = NULL, const bool actuallyLoadImage = true,
