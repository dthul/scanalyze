...
ProxyScan::ProxyScan(const string &proxyForFileName, const Pnt3 &min,
                     const Pnt3 &max) {
...
MeshTransport *ProxyScan::mesh(bool perVertex, bool stripped, ColorSource color,
                               int colorSize) {
    ...
    if (perVertex) {
        nrm->reserve(24);
        for (int i = 0; i < 8; i++) {
            pushNormalAsShorts(*nrm,
                               (bbox.corner(i) - bbox.center()).normalize());
        }
    } else {
        nrm->reserve(36);
    }

    const int faceInd[12][3] = { { 0, 1, 2 },
                                 { 1, 3, 2 },
                                 { 2, 3, 6 },
                                 { 3, 7, 6 },
                                 { 6, 7, 4 },
    ...
    for (int i = 0; i < 12; i++) {
        ...
        if (!perVertex) {
            Pnt3 normal =
                cross(((*vtx)[t1] - (*vtx)[t2]), ((*vtx)[t1] - (*vtx)[t3]));
            normal.normalize();
