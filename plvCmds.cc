        ...
        for (int i = 1; i < argc; i++) {
            if (!strcmp(argv[i], "-warn")) {
                ...
                if (!strcmp(argv[i], "0") || !strcmp(argv[i], "off")) {
                    ...
                    Tcl_SetResult(interp, "bad arg to plv_param -warn",
                                  TCL_STATIC);
                    return TCL_ERROR;
                }
...
Tk_RestrictAction updateWindowFilter(ClientData clientData, XEvent *eventPtr) {
...
int PlvUpdateWindowCmd(ClientData clientData, Tcl_Interp *interp, int argc,
    ...
    Tk_Window tkwin = Tk_NameToWindow(interp, argv[1], Tk_MainWindow(interp));
    if (!tkwin)
    ...
    old_filter_proc =
        Tk_RestrictEvents(updateWindowFilter, tkwin, &old_filter_data);
