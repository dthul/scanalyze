...
Selection::Selection(void) { clear(); }
...
static bool ptOnLine(Selection::Pt pt, Selection::Pt start, Selection::Pt end);
...
int PlvClipToSelectionCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                          char *argv[]) {
    if (argc < 2) {
    ...
    for (vector<DisplayableMesh *>::iterator clippee = clippees.begin();
        ...
        if (bClipInPlace) {
            ...
            cerr << "attempting a clip of mesh: " << meshFrom->get_name()
                 << endl;
            ...
            if (meshTo) {
                ...
                if (meshTo->num_resolutions() > 0) {
                    ...
                    char tclCmdBuff[256];
                    sprintf(tclCmdBuff, "confirmDeleteMesh %s",
                            oldDisp->getName());
                    ...
                    sprintf(tclCmdBuff, "clipInplaceHelper %s %s",
                            newDisp->getName(), theMeshOld);
                    ...
                    Tcl_VarEval(interp, "changeVis ", oldDisp->getName(), " 0",
                                NULL);
                }
            } else {
            ...
            if (meshTo) {
                if (meshTo->num_resolutions() > 0) {
                    ...
                    if (!group) {
                        string name = meshFrom->get_basename() + "Clip." +
                                      meshFrom->get_nameending();
                        meshTo->set_name(name);

                        DisplayableMesh *newDisp =
                            theScene->addMeshSet(meshTo, false);
                        Tcl_VarEval(interp, "clipMeshCreateHelper ",
                                    oldDisp->getName(), " ", newDisp->getName(),
                                    NULL);
                    } else {
                        // since it is a group, it has already been named +
                        // added to the scene
                        Tcl_VarEval(
                            interp, "clipMeshCreateHelper ", oldDisp->getName(),
        ...
        if (!progress->updateInc()) {
            cerr << "Clip operation cancelled ("
                 << (clippee - clippees.begin() + 1) << " down; "
                 << (clippees.end() - clippee - 1) << " to go)" << endl;
            break;
        }
    }
...
static char *my_clip_strrstr(char **haystack) {
    char *str = *haystack;
    char *oldstr = str;
    ...
    if (g) {
        if (g->get_children_for_display(children)) {
            ...
            for (vector<DisplayableMesh *>::iterator it = children.begin();
                 it < children.end(); it++)
                vec.push_back(
                    Renamer(*it)); // rename the descendants of the group first
...
int PlvPrintVoxelsCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                      char *argv[]) {
    ...
    if (meshDisp == NULL) {
        Tcl_SetResult(interp,
                      "Attempting to print voxels for non-existant mesh",
                      TCL_STATIC);
        return (TCL_ERROR);
    }
    ...
    if (meshFrom == NULL) {
        Tcl_SetResult(interp, "PlvPrintVoxelsCmd: We don't have a mesh",
                      TCL_STATIC);
        return (TCL_ERROR);
    }
    ...
    if (meshTo->num_vertices() == 0) {
        ...
        gscanTo->PrintVoxelInfo();

    return (TCL_OK);
}

int PlvClearSelectionCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                         char *argv[]) {
...
int PlvDrawLineSelectionCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                            char *argv[]) {
    // args are %x %y start
    if (argc < 4)
        return TCL_ERROR;
    ...
    if (!strcmp(argv[3], "start")) {
        ...
        if (theSel.type != Selection::line) {
            Tcl_SetResult(interp, "Internal selection tracking error",
                          TCL_STATIC);
            return TCL_ERROR;
        }
...
int PlvDrawBoxSelectionCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                           char *argv[]) {
    if (argc < 5)
        return TCL_ERROR;
    ...
    if (argc > 4 && !strcmp(argv[4], "start")) {
        ...
        if (theSel.type == Selection::rect) {
            ...
            if (iHandle >= 0) {
                ...
                theSel[2] = pt;
                theSel[1].x = theSel[2].x;
                theSel[1].y = theSel[0].y;
                theSel[3].x = theSel[0].x;
                theSel[3].y = theSel[2].y;
            } else {
                if (findSelectionLine(pt, theSel) >= 0) {
                    bChangeExisting = true;
                    bMoveEntire = true;
...
// ------------------------------------------------------------
int PlvGetSelectionInfoCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                           char *argv[]) {
    char buff[256];
    ...
    case Selection::none:
        Tcl_SetResult(interp,
                      "PlvGetSelectionInfoCmd: error -- nothing selected",
                      TCL_STATIC);
        return TCL_ERROR;
    //    break;
    case Selection::line:
        sprintf(buff, "line %i %i %i %i", theSel[0].x, theSel[0].y, theSel[1].x,
                theSel[1].y);
        break;
    case Selection::rect:
        /*
    ...
    default:
        Tcl_SetResult(interp,
                      "PlvGetSelectionInfoCmd: error -- type not supported",
                      TCL_STATIC);
        return TCL_ERROR;
        //    break;
    }
...
int PlvMeshIntersectSelectionCmd(ClientData clientData, Tcl_Interp *interp,
                                 int argc, char *argv[]) {
    if (argc < 2) {
        Tcl_SetResult(interp, "Missing mesh argument in PlvShowBySelectionCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }

    if (theSel.type != Selection::rect) {
        Tcl_SetResult(interp, "Only rectangle selections supported",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (!dm) {
        Tcl_SetResult(interp, "Bad mesh argument in PlvShowBySelectionCmd",
                      TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (nPts) {
        ...
        if (!bManipulatingSel) {
            for (int i = 0; i < nPts; i++)
                drawHandle(theSel[i], theSel.type == Selection::shape &&
                                          ((i == 0) || (i == (nPts - 1))));
        }
...
void splTessError(GLenum er) {
    ...
    cout << "Tesselation error during polyline rasterization: " << str << endl;
}

vector<GLdouble *> hack_vert_alloc;

void splTessCombine(GLdouble coords[3], GLdouble *vertex_data[4],
...
int PlvGetSelectedMeshesCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                            char *argv[]) {
