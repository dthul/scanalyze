...
class Image {
  private:
    ...
    uchar *at(int x, int y) const { return &(data[y][dim * x]); }
  ...
  public:
    ...
    uchar getR(int x, int y) const { return at(x, y)[0]; }
    ...
    uchar getValue(int x, int y) { return *at(x, y); }
    uchar *getValues(int x, int y) { return at(x, y); }
    const uchar *getValuesConst(int x, int y) const { return at(x, y); }
    void setValue(int x, int y, uchar a, uchar b = 0, uchar c = 0, uchar d = 0);
...
inline void Image::setValue(int x, int y, uchar a, uchar b, uchar c, uchar d) {
    if (!inside(x, y))
        return;
    uchar *tmp = at(x, y);
    tmp[0] = a;
