...
struct PlyElement {            /* description of an element */
    ...
    PlyElement(char *n) : num(0) { name = strdup(n); }

    PlyElement() : num(0) { name = NULL; }
};
...
class PlyFile {
  private:
    ...
    void get_and_store_binary_item(int type_ext, int type_int, char *item);
    void get_and_store_binary_list(int type_ext, int type_int, char *item,
                                   int n);

    PlyElement *find_element(char *);
    void ascii_get_element(char *elem_ptr, int use_provided_storage);
    void binary_get_element(char *elem_ptr, int use_provided_storage);
  ...
  public:
    ...
    int open_for_writing(const char *filename, int nelems, char **elem_names,
                         int file_type);
    ...
    int open_for_reading(const char *filename, int *nelems, char ***elem_names);
